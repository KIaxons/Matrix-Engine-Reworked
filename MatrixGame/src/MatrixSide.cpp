...
inline bool IsUnitAlive(CMatrixMapStatic* obj);
...
void CMatrixSideUnit::LogicTact(int ms)
{
            ...
            case BUILDING_TURRET:
            {
                if(m_CannonForBuild.m_Cannon)
                {
                    ...
                    if(can_bld_flg >= 0)
                    {
                        ...
                        m_CannonForBuild.m_Cannon->m_Place = g_MatrixMap->m_RoadNetwork.FindInPL(places[can_bld_flg]);
                    }
                    else
                    {
    ...
    while(object)
    {
        if(object->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
        {
            ...
            if(robot->m_Side == m_Id && robot->m_CurrState != ROBOT_DIP)
            {
                ++m_RobotsCnt;
                if(robot->GetTeam() >= 0)
                {
...
void CMatrixSideUnit::OnRButtonDown(const CPoint&)
{
    ...
    if(!IS_PREORDERING)
    {
        ...
        if(m_CurrSel == GROUP_SELECTED || m_CurrSel == ROBOT_SELECTED || m_CurrSel == FLYER_SELECTED)
        {
            ...
            else if(IS_TRACE_STOP_OBJECT(pObject) && ((IsUnitAlive(pObject) && pObject->GetSide() != m_Id) || pObject->IsSpecial()))
            {
...
void CMatrixSideUnit::OnForward(bool down)
{
    ...
    else if(!down && is_ord) robot->StopMoving();
}
...
void SideSelectionCallBack(CMatrixMapStatic* ms, dword param)
{
    ...
    {
        if(((CMatrixRobotAI*)ms)->m_CurrState == ROBOT_DIP) return;
    }
    ...
    while(ms)
    {
        switch(ms->GetObjectType())
        {
            ...
            case OBJECT_TYPE_ROBOT_AI:
            {
                ...
                if(cur_robot->m_CurrState != ROBOT_DIP)
                {
    ...
    for(i = 0; i < m_TeamCnt; ++i)
    {
        ...
        if(m_Team[i].m_Action.m_Type != mlat_None && m_Team[i].m_Action.m_Region >= 0 && m_Team[i].m_Action.m_Region != cr && g_MatrixMap->m_RoadNetwork.IsNearestRegion(cr, m_Team[i].m_Action.m_Region))
        {
            ...
            while(ms)
            {
                ...
                if(IsUnitAlive(ms)) ObjPlaceData(ms, 1);
                ms = ms->GetNextLogic();
            }
...
void CMatrixSideUnit::TactTL()
{
    ...
    if (m_LastTactUnderfire == 0 || (g_MatrixMap->GetTime() - m_LastTactUnderfire) > 500)
    {
        ...
        while (obj)
        {
            if (IsUnitAlive(obj) && obj->GetSide() != m_Id)
            {
                tp = GetMapPos(obj);
...
void CMatrixSideUnit::WarTL(int group)
{
    ...
    for(i = 0; i < rl_cnt; ++i)
    {
        if(rl[i]->GetEnv()->m_TargetAttack == rl[i]) rl[i]->GetEnv()->m_TargetAttack = nullptr;
        ...
        if(!rl[i]->GetEnv()->m_TargetAttack)
        {
            ...
            while(enemy)
            {
                if(IsUnitAlive(enemy->m_Enemy) && enemy->m_Enemy != rl[i])
                {
                    ...
                    if(cd < min_dist)
                    {
                        ...
                        if(dist > 0.0f)
                        {
                            ...
                            while(obj)
                            {
                                if(IsUnitAlive(obj) && obj->GetSide() == m_Id && rl[i] != obj)
                                {
                                    p = PointOfAim(obj);
    ...
    */
            ...
            while(obj)
            {
                if (IsUnitAlive(obj)) ObjPlaceData(obj, 1);
                obj = obj->GetNextLogic();
            }
...
//                              }
                                ...
                                //{
//                                if(distfrom2 > POW2(300 + 100)) continue; // Робот не должен отклониться слишком далеко
                    }

                    int underfire = place->m_Underfire;
                    if(distplace2 <= POW2(enemy_fire_dist)) ++underfire;

                    CMatrixMapStatic* trace_res = g_MatrixMap->Trace(nullptr, D3DXVECTOR3(pcx, pcy, g_MatrixMap->GetZ(pcx, pcy) + 20.0f) /*rl[i]->GetGeoCenter()*/, PointOfAim(rl[i]->GetEnv()->m_TargetAttack), TRACE_OBJECT | TRACE_NONOBJECT | TRACE_OBJECTSPHERE | TRACE_SKIP_INVISIBLE, rl[i]);
    ...
    for(i = 0; i < rl_cnt; ++i)
    {
        if(rl[i]->GetEnv()->m_TargetAttack)
        {
            if(!IsUnitAlive(rl[i]->GetEnv()->m_TargetAttack))
            {
                rl[i]->StopFire();
                continue;
            }
            ...
            if(fireline)
            {
                ...
                fireline = D3DXVec3LengthSq(&temp) <= POW2(rl[i]->GetMaxFireDist());
            ...
            else
            {
                if (rl[i]->HaveRepair() && (g_MatrixMap->GetTime() - rl[i]->GetEnv()->m_TargetChangeRepair) > 1000) //Ищем робота для починки
                {
                    ...
                    if (rl[i]->GetEnv()->m_Target && IsUnitAlive(rl[i]->GetEnv()->m_Target) && rl[i]->GetEnv()->m_Target->GetSide() == m_Id && rl[i]->GetEnv()->m_Target->NeedRepair())
                    {
                        v = GetWorldPos(rl[i]);
                    ...
                    if(!rl[i]->GetEnv()->m_Target)
                    {
                        ...
                        while(obj)
                        {
                            if(obj != rl[i] && IsUnitAlive(obj) && obj->GetSide() == m_Id && obj->NeedRepair())
                            {
                                v = GetWorldPos(rl[i]);
                                v2 = GetWorldPos(obj);
                ...
                if(rl[i]->GetEnv()->TargetType() == 2) rl[i]->OrderFire(PointOfAim(rl[i]->GetEnv()->m_Target), 2);
                else rl[i]->StopFire();
...
void CMatrixSideUnit::RepairTL(int group)
{
    ...
    for(i = 0; i < rl_cnt; ++i)
    {
        ...
        if(env->m_Target && env->m_Target->IsAlive() && env->m_Target->NeedRepair())
        {
            v = GetWorldPos(rl[i]);
        ...
        while(obj)
        {
            if(obj != rl[i] && obj->IsAlive() && obj->GetSide() == m_Id && obj->NeedRepair())
            {
                v = GetWorldPos(rl[i]);
                v2 = GetWorldPos(obj);
                if((POW2(v.x - v2.x) + POW2(v.y - v2.y)) < POW2(rl[i]->GetRepairDist()))
...
void CMatrixSideUnit::AssignPlace(CMatrixRobotAI* robot, int region)
{
    ...
    while(obj)
    {
        if(IsUnitAlive(obj) && obj != robot) ObjPlaceData(obj, 1);
        obj = obj->GetNextLogic();
    }
...
void CMatrixSideUnit::SortRobotList(CMatrixRobotAI** rl, int rl_cnt)
{
    int i, u;
...
void CMatrixSideUnit::ChooseAndBuildAIRobot(int cur_side)
{
    ...
    {
        int overrated_reserve = roundf(pow(m_Resources[res], 1.1));
        ...
        res_sufficiency[res] = (overrated_reserve + 10) * (abs(income) + 10); //В теории, в настройках карты можно задать отрицательный прирост ресурсов с базы, поэтому перестраховываемся от такого через модуль, чтобы бот поскорее потратил иссякающий ресурс
    }

    //Определяем профицитность/дефицитность каждого ресурса
    int average_sufficiency = (res_sufficiency[TITAN] + res_sufficiency[ELECTRONICS] + res_sufficiency[ENERGY] + res_sufficiency[PLASMA]) / MAX_RESOURCES;
    for(int res = 0; res < MAX_RESOURCES; ++res)
    {
    ...
    //Считаем достаток
    int prosperity_lvl = (min_overrated_reserve + 180) * (abs(min_income) + 7);
    //Считаем актуальный достаток
    prosperity_lvl = roundf(prosperity_lvl * m_WaitResMultiplier) + roundf(prosperity_lvl * 0.3f);
    ...
    prosperity_lvl *= PortionInDiapason(base_in_danger, 0.6f, 10.0f, 1.0f, 0.1f);
    ...
    */
    ...
    for(int i = list_begin; i <= list_end; ++i)
    {
        ...
        else
        {
            ...
            {
                cur_template->m_TemplateWeight += 1 + ceil(cur_template->m_ResourcesExpendability[TITAN] - res_sufficiency[TITAN]);
            }
            ...
            {
                cur_template->m_TemplateWeight += 1 + ceil(cur_template->m_ResourcesExpendability[ELECTRONICS] - res_sufficiency[ELECTRONICS]);
            }
            ...
            {
                cur_template->m_TemplateWeight += 1 + ceil(cur_template->m_ResourcesExpendability[ENERGY] - res_sufficiency[ENERGY]);
            }
            ...
            {
                cur_template->m_TemplateWeight += 1 + ceil(cur_template->m_ResourcesExpendability[PLASMA] - res_sufficiency[PLASMA]);
            }
...
void CMatrixSideUnit::RepairPL(int group)
{
    ...
    if (m_PlayerGroup[group].Order() == mpo_Repair && m_PlayerGroup[group].m_Obj && m_PlayerGroup[group].m_Obj->IsAlive())
    {
        for (i = 0; i < rl_cnt; ++i)
        {
            if (m_PlayerGroup[group].m_Obj == rl[i]) rl[i]->GetEnv()->m_Target = nullptr;
...
bool CMatrixSideUnit::FirePL(int group)
{
                ...
                if(IsUnitAlive(enemy->m_Enemy) && enemy->m_Enemy != rl[i])
                {
                    ...
                    if (cd < min_dist)
                    {
                        ...
                        if(dist > 0.0f)
                        {
                            ...
                            while(obj)
                            {
                                if(IsUnitAlive(obj) && obj->GetSide() == m_Id && rl[i] != obj && env->m_TargetAttack != obj)
                                {
                                    p = PointOfAim(obj);
    ...
    for(i = 0; i < rl_cnt; ++i)
    {
        ...
        if(env->m_TargetAttack && env->SearchEnemy(env->m_TargetAttack))
        {
            ...
            des = PointOfAim(env->m_TargetAttack);
            ...
            {
                D3DXVECTOR3 v1 = rl[i]->GetGeoCenter();
                D3DXVECTOR3 v2 = PointOfAim(env->m_TargetAttack);
            ...
            else
            {
                if(rl[i]->HaveRepair() && (g_MatrixMap->GetTime() - env->m_TargetChangeRepair) > 1000) //Ищем робота для починки
                {
                    ...
                    if(env->m_Target && IsUnitAlive(env->m_Target) && env->m_Target->GetSide() == m_Id && env->m_Target->NeedRepair())
                    {
                        v = GetWorldPos(rl[i]);
                        v2 = GetWorldPos(env->m_Target);
                ...
                if(env->TargetType() == 2) rl[i]->OrderFire(PointOfAim(env->m_Target), 2);
                else rl[i]->StopFire();
                    ...
                    if(IsUnitAlive(enemy->m_Enemy) && enemy->m_Enemy != rl[i])
                    {
                        ...
                        if(cd < min_dist)
                        {
                            ...
                            if(dist > 0.0f)
                            {
                                ...
                                while(obj)
                                {
                                    if(IsUnitAlive(obj) && obj->GetSide() == m_Id && rl[i] != obj && rl[i]->GetEnv()->m_TargetAttack != obj)
                                    {
                                        p = PointOfAim(obj);
...
void CMatrixSideUnit::PGFindAttackTarget(int no)
{
    ...
    while(obj)
    {
        if(IsUnitAlive(obj) && (obj->GetSide() != m_Id) && GetRegion(obj) == regionmass)
        {
            m_PlayerGroup[no].m_Obj = obj;
            ...
            for (t = 0; t < g_MatrixMap->m_RoadNetwork.m_Region[m_RegionIndex[sme]].m_NearCnt; ++t)
            {
                ...
                if (withourgroup == 0)
                {
                    ...
                    for (k = 0; k < MAX_LOGIC_GROUP; ++k)
                    {
                        ...
                        if (!m_PlayerGroup[k].m_Obj) continue;
                        if (!IsUnitAlive(m_PlayerGroup[k].m_Obj)) continue;
                ...
                if(m_Region[u].m_EnemyRobotCnt > 0 || m_Region[u].m_EnemyCannonCnt > 0 || m_Region[u].m_NeutralCannonCnt > 0)
                {
                    ...
                    while(obj)
                    {
                        if(IsUnitAlive(obj) && (obj->GetSide() != m_Id) && GetRegion(obj) == u)
                        {
                            m_PlayerGroup[no].m_Obj = obj;
                            PGCalcRegionPath(m_PlayerGroup + no, u, mm);
...
void CMatrixSideUnit::PGFindDefenceTarget(int no)
{
    ...
    while(obj)
    {
        if(IsUnitAlive(obj) && (obj->GetSide() != m_Id) && GetRegion(obj) == region_mass)
        {
            m_PlayerGroup[no].m_Obj = obj;
