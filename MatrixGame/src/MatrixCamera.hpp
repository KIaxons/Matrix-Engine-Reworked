...
//#define CAM_RESTORE           SETBIT(11)
#define CAM_XY_LERP_OFF         SETBIT(12) // xy pos restore
//#define CAM_LANDRELATIVE      SETBIT(13)

#define CAM_SELECTED_TARGET      SETBIT(14)
#define CAM_NEED2END_DIALOG_MODE SETBIT(15)
...
struct SAutoFlyData
{
    struct SWarPair
    {
        ...
        SObjectCore* attacker = nullptr;
        float        ttl = 0.0f;
    };

    ETrajectoryStatus m_Status = TRJS_JUST_STAYING;

    //CTrajectory*  m_Traj = nullptr;

    D3DXVECTOR3     m_Cur = { 0.0f, 0.0f, 0.0f };

    float           m_CurDist = 0.0f;
    float           m_CurAngZ = 0.0f;
    float           m_CurAngX = 0.0f;

    //D3DXVECTOR3   m_NewActual = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3     m_New = { 0.0f, 0.0f, 0.0f };
    float           m_NewDist = 0.0f;
    float           m_NewAngZ = 0.0f;
    float           m_NewAngX = 0.0f;

    float           m_RotSpeedZ = 0.0f;
    float           m_RotSpeedX = 0.0f;
    
    float           m_Speed = 0.0f;
    //float         m_OnTrajectorySpeedNeed = 0.0f;
    //float         m_OnTrajectorySpeedCurrent = 0.0f;

    SWarPair        m_WarPairCurrent;
    SWarPair        m_WarPairs[MAX_WAR_PAIRS];
    int             m_WarPairsCnt = 0;

    int             m_LastStatTime = 0;
    int             m_LastObzorTime = 0;


    //float         m_CalcDestTime = 0.0f;
    float           m_CalcDestAngZTime = 0.0f;
    //float         m_OnTrajTime = 0.0f;
    float           m_ObzorTime = 0.0f;
    
    float           m_BeforeObzorAngX = 0.0f;
...
public:
    ...
    float CamAngleToCoordOffset(double dist, int par_type)
    {
        ...
        if(!par_type) return -sin(m_Ang_Strategy) * float(dist);
        else return cos(m_Ang_Strategy) * float(dist);
    }

    CMatrixCamera();
    ~CMatrixCamera();

    inline float GetResXInversed() const { return _res_x_inversed; }
    inline float GetResYInversed() const { return _res_y_inversed; }
    ...
    inline const D3DXVECTOR2& GetXYStrategy() const { return m_XY_Strategy; }
    ...
    inline const D3DXVECTOR3& GetDir() const { return *(D3DXVECTOR3*)&m_MatViewInversed._31; }
    inline const D3DXVECTOR3& GetRight() const { return *(D3DXVECTOR3*)&m_MatViewInversed._11; }
    inline const D3DXVECTOR3& GetUp() const { return *(D3DXVECTOR3*)&m_MatViewInversed._21; }
    ...
    void SpeedUp() { SETFLAG(m_Flags, CAM_ACTION_SPEED_UP); }
