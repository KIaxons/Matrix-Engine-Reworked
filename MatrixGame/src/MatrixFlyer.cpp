...
off_weapon:;
            ...
            for(int i = 0; i < m_StreamsCount; ++i)
            {
                ...
                m_Streams[i].effect = CMatrixEffect::CreateFireStream(D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(10.0f, 0.0f, 0.0f), 5.0f, 0xFFFFFFFF);
            }
        }
        //else {}
    }
...
void CMatrixFlyer::LogicTactStrategy(SFlyerTactData& td)
{
    ...
    if(pls->GetUnitUnderManualControl() != this)
    {
...
)
{
    ...
    if(order == FO_GIVE_BOT)
    {
        ...
        SRobotTemplate bot = SRobotTemplate::m_AIRobotTypesList[REINFORCEMENTS_TEMPLATES][robot_template];
...
void CMatrixFlyer::DrawPropeller()
{
    if(!IS_VB(m_VB)) return;
...
void CMatrixFlyer::DrawShadowStencil()
{
...
void CMatrixFlyer::FreeDynamicResources()
{
    for(int i = 0; i < m_ModulesCount; ++i)
    {
