...
void SAutoFlyData::AddWarPair(CMatrixMapStatic* tgt, CMatrixMapStatic* attacker)
{
    ...
    for(int i = 0; i < m_WarPairsCnt;)
    {
        ...
        if(m_WarPairs[i].target->m_Object == nullptr || m_WarPairs[i].attacker->m_Object == nullptr)
        {
            m_WarPairs[i].target->Release();
            m_WarPairs[i].attacker->Release();
...
void SAutoFlyData::Tact(float ms)
{
DTRACE();
    ...
    {
        m_WarPairs[i].ttl -= ms;
        if(m_WarPairs[i].ttl < 0 || m_WarPairs[i].target->m_Object == nullptr || m_WarPairs[i].attacker->m_Object == nullptr)
        {
            m_WarPairs[i].target->Release();
            m_WarPairs[i].attacker->Release();
    ...
    if(m_CalcDestAngZTime < 0)
    {
        ...
        if(m_CalcDestAngZTime < 0) m_CalcDestAngZTime = RECALC_DEST_ANGZ_PERIOD;
    ...
    */
    ...
    //    g_MatrixMap->Trace(&pos, g_MatrixMap->m_Camera.GetFrustumCenter(), pos, TRACE_LANDSCAPE, nullptr);
    ...
    if(m_ObzorTime > 0)
    {
        CMatrixMapStatic * ts = g_MatrixMap->Trace(nullptr, g_MatrixMap->m_Camera.GetFrustumCenter(), g_MatrixMap->m_Camera.GetFrustumCenter() + g_MatrixMap->m_Camera.GetDir() * 200, TRACE_LANDSCAPE, nullptr);
        if(ts != TRACE_STOP_NONE) m_ObzorTime = 0;
...
void SAutoFlyData::FindAutoFlyTarget(void)
{
    ...
    CMatrixCamera* cam = &g_MatrixMap->m_Camera;
    if(m_WarPairsCnt == 0 && m_WarPairCurrent.target == nullptr)
    {
seek_nothing:
...
check_again:
        ...
        if(!m_WarPairsCnt) goto seek_nothing;
...
void CMatrixCamera::ResetAngles()
{
...
void CMatrixCamera::BeforeDraw()
{
    ...
    //g_MatrixMap->m_DI.ShowScreenText(L"...1", CWStr(m_AngleZ));
    //g_MatrixMap->m_DI.ShowScreenText(L"...2", CWStr(az));
    //g_MatrixMap->m_DI.ShowScreenText(L"dist", CWStr(m_Dist));
    //g_MatrixMap->m_DI.ShowScreenText(L"land z", CWStr(m_LandRelativeZ));
}
...
float CMatrixCamera::GetFrustPlaneDist(EFrustumPlane plane, const D3DXVECTOR3& pos, const D3DXVECTOR3& dir)
{
    ...
    switch(plane)
    {
        case FPLANE_LEFT: pl = &GetFrustPlaneL(); break;
        case FPLANE_RIGHT: pl = &GetFrustPlaneR(); break;
        case FPLANE_BOTTOM: pl = &GetFrustPlaneB(); break;
        case FPLANE_TOP: pl = &GetFrustPlaneT(); break;
    }
...
//Здесь считается положение игровой камеры для стратегического и аркадного режимов
void CMatrixCamera::CalcLinkPoint(D3DXVECTOR3& lp, float& ang_z)
{
    ...
    if(g_MatrixMap->GetPlayerSide()->IsManualControlMode())
    {
        ...
        {
            CMatrixFlyer* flyer = arcade_object->AsFlyer();

            D3DXVECTOR3 pos = flyer->GetPos();// -arcade_object->GetGeoCenter();
            lp.x = pos.x;
            lp.y = pos.y;
            lp.z = pos.z;
            //lp.z = g_MatrixMap->GetZ(lp.x, lp.y); //Ground level
            //lp.z += g_Config.m_CamParams[CAMERA_ARCADE].m_CamHeight;

            //Здесь задаётся корректный угол поворота камеры в сторону направления носа вертолёта
            float flyer_speed = flyer->GetSpeedNorm();
            pos = flyer->GetForward(0.5f);
            pos.z = 0.0f;
            lp += pos * LERPFLOAT(flyer_speed, g_Config.m_CamInRobotForward0, g_Config.m_CamInRobotForward1);
            ang_z = (float)(atan2(-pos.x, pos.y) + M_PI_MUL(1));
        }
    }
    else
    {
...
void CMatrixCamera::Tact(float ms)
{
    ...
    if(FLAG(m_Flags, CAM_ACTION_MOVE_LEFT | CAM_ACTION_MOVE_RIGHT | CAM_ACTION_MOVE_UP | CAM_ACTION_MOVE_DOWN))
    {
        if(m_ModeIndex == CAMERA_STRATEGY)
        {
            ...
            float coef = 1.0f;
            if(FLAG(m_Flags, CAM_ACTION_MOVE_LEFT)) { m_XY_Strategy += coef * lDir; }
            if(FLAG(m_Flags, CAM_ACTION_MOVE_RIGHT)) { m_XY_Strategy += coef * rDir; }
            if(FLAG(m_Flags, CAM_ACTION_MOVE_UP)) { m_XY_Strategy += coef * tDir; }
            if(FLAG(m_Flags, CAM_ACTION_MOVE_DOWN)) { m_XY_Strategy += coef * bDir; }
        ...
        {
            RESETFLAG(m_Flags, CAM_ACTION_MOVE_LEFT | CAM_ACTION_MOVE_RIGHT | CAM_ACTION_MOVE_UP | CAM_ACTION_MOVE_DOWN | CAM_ACTION_SPEED_UP);
        }
    }
    ...
    RESETFLAG(m_Flags, CAM_ACTION_ROT_LEFT | CAM_ACTION_ROT_RIGHT | CAM_ACTION_ROT_UP | CAM_ACTION_ROT_DOWN | CAM_ACTION_MOVE_LEFT | CAM_ACTION_MOVE_RIGHT | CAM_ACTION_MOVE_UP | CAM_ACTION_MOVE_DOWN | CAM_ACTION_SPEED_UP);
}

void CMatrixCamera::RestoreCameraParams()
{
