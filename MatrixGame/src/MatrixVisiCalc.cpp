...
struct SVisGroup
{
    ...
    void BuildShadowFor(const D3DXVECTOR3 &pointsFrom, int pointsCount, STempPoints *points, CBuf &pe)
    {
        ...
        for (; edge < bufferEnd; ++edge)
        {
            ...
            for (int i = 0; i < pointsCount; ++i, ++tempPoints)
            {
                ...
                if (!(((!ppcam) ^ PointLineCatch(D3DXVECTOR2(pointsFrom.x,pointsFrom.y), p20, D3DXVECTOR2(tempPoints->p.x,tempPoints->p.y))) &&
                ((!ppcam) ^ PointLineCatch(p21, D3DXVECTOR2(pointsFrom.x,pointsFrom.y), D3DXVECTOR2(tempPoints->p.x,tempPoints->p.y))))) continue;
    ...
    void CalcVis(SVisGroup *visibilityGroup, int groupCount, int pointsCount, STempPoints *points, STempCalcs *calcs, CBuf &pe)
    {
...
struct CMatrixMap::SCalcVisRuntime
{
    ...
    CMatrixMapGroup** mapGroup = nullptr;
    int i = 0, j = 0;
};
    ...
    if(m_GroupVis != nullptr)
    {
        ...
        if(gx >= 0 && gx < m_GroupSize.x && gy >= 0 && gy < m_GroupSize.y)
        {
            ...
            if(level >= 0 && level < visibilityGroup->levels_cnt)
            {
                ...
                for(int i = 0; i < n; ++i)
                {
                    CMatrixMapGroup* g = visibilityGroup->vis[i];
