...
CMultiSelection::CMultiSelection(const Base::CPoint& pos) : m_LT(pos), m_RB(pos), m_SelItems(g_MatrixHeap) //Нельзя перенести конструктор в хэдер, т.к. там g_MatrixHeap undefined
{
    //m_Tex = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, L"Mods\\PlanetaryBattles\\PBEngine\\Matrix\\Textures\\selback"); //TEXTURE_PATH_SELBACK);
    //m_Tex->MipmapOff();
    //m_Tex->Preload();
}
...
{
    if(g_MatrixMap->IsPaused()) return nullptr;
...
bool CMultiSelection::DrawPass1(void)
{
    ...
    */
    ...
    r.x2 = m_RB.x;
    r.y2 = m_RB.y;
    if(r.x1 > r.x2) { r.x1 ^= r.x2; r.x2 ^= r.x1; r.x1 ^= r.x2; }
    if(r.y1 > r.y2) { r.y1 ^= r.y2; r.y2 ^= r.y1; r.y1 ^= r.y2; }
...
//Определяются объекты, которые могут быть выделены игроком
void CMultiSelection::Update(
    const Base::CPoint& pos,
    dword mask,
    SELECT_ENUM callback,
    dword param
)
{
    m_RB = pos;
    ...
    for(int i = 0; i < n; ++i)
    {
        ...
        if(o->IsFlyer())
        {
            ...
            if(o->AsFlyer()->IsDeliveryCopter()) continue;
        }
