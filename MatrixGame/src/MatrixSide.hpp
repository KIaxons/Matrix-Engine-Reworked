...
public:
    ...
    int m_Brave = 0;
    float m_BraveStrengthCancel = 0.0f;
...
struct SMatrixLogicRegion
{
    ...
    //Почти идентины m_EnemyForces и m_EnemyForcesAdd, но при расчёте постоянно перемножаются на модификатор угрозы, задаваемый в карте
    float m_Danger = 0.0f;    // Коэффициент опасности (это не коэффициент, блять, а суммарная сила вех врагов в регоние и вокруг него!)
    float m_DangerAdd = 0.0f; // Выращенная опасность (спасибо, даб, так стало очень понятно)

    //Можно использовать для расчёта точного баланса сил в регионах
    float m_EnemyForces = 0.0f;
    float m_EnemyForcesAdd = 0.0f;
    float m_OurForces = 0.0f;
    float m_OurForcesAdd = 0.0f;
...
private:
    ...
    //float m_WarSideStrengthCancel = 0.0f; //Перерассчитываем сторону с которой воюем, когда сила стороны упадет ниже критической
    ...
    //Потом как-нибудь навести порядок
    /*
    struct SUnitUnderManualControl
    {
        CMatrixMapStatic* m_Unit = nullptr;
        dword             m_available = 0;
        D3DXVECTOR3       m_cur = { 0.0f, 0.0f, 0.0f };
        D3DXVECTOR3       m_prevrp = { 0.0f, 0.0f, 0.0f };

        D3DXVECTOR3       m_ppos0 = { 0.0f, 0.0f, 0.0f };
        D3DXVECTOR3       m_ppos1 = { 0.0f, 0.0f, 0.0f };
        float             m_k = 0.0f;
    };
    SUnitUnderManualControl m_UnitUnderManualControl;
    */

    CMatrixMapStatic* m_UnitUnderManualControl = nullptr;
    dword             m_UnitUnderManualControl_available = 0;
    D3DXVECTOR3       m_UnitUnderManualControl_cur = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3       m_UnitUnderManualControl_prevrp = { 0.0f, 0.0f, 0.0f };

    D3DXVECTOR3       m_UnitUnderManualControl_ppos0 = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3       m_UnitUnderManualControl_ppos1 = { 0.0f, 0.0f, 0.0f };
    float             m_UnitUnderManualControl_k = 0.0f;
...
public:
    ...
    float m_StrengthMultiplier = 1.0f;
    float m_BraveMultiplier = 0.5f;
    int   m_TeamCnt = 3;
    float m_DangerMultiplier = 1.0f;
    float m_WaitResMultiplier = 1.0f;
    ...
    int m_LastBuildedRobot = -1;
    int m_LastBuildedRobot2 = -1;
    int m_LastBuildedRobot3 = -1;
    ...
    void SelectedGroupBreakAllOrders(); //Отменяем вообще все приказы роботов указанной группы
    void SelectedGroupBreakMoveOrders(); //Отменяем все приказы роботов указанной группы связанные с движением, не затрагивая приказы на стрельбу
