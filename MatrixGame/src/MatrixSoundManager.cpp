...
void CSound::Tact()
{
    ...
    if(delta < 0 || delta > 1000)
    {
        ...
        if(FLAG(g_Config.m_DIFlags, DI_ACTIVE_SOUNDS))
        {
            int sc = 0;
            for(int i = 0; i < MAX_SOUNDS; ++i)
            {
            ...
            g_MatrixMap->m_DI.ShowScreenText(L"Active sounds: ", CWStr(sc));
        }
    }
}
...
void CSound::StopPlay(dword id)
{
    ...
    if(g_RangersInterface)
    {
        //g_MatrixMap->m_DI.ShowScreenText(CWStr(L"sndoff") + (int)id, L"");
    ...
    if(g_RangersInterface)
    {
        ...
        for(; sb < se;)
        {
            ...
            if(idx >= 0)
            {
                ...
                //g_MatrixMap->m_DI.ShowScreenText(CWStr(vol).Get(), L"1212");
            }
            else
            {
dele:
