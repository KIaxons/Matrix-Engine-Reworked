...
typedef struct _SDebris
{
    ...
    //Структуры внутри union-ов считаются кодом подклассами _SDebris, а, следовательно, должны иметь доступ к родительскому деструктору, которого _SDebris как структура по умолчанию не имеет
    //Так что если убрать отсюда объявление деструктора, то компилятор будет постоянно спамить ошибку отсутствия деструктора у _SDebris
    ~_SDebris() {};
