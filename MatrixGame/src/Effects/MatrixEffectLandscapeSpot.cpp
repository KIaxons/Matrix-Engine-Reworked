...
void CMatrixEffectLandscapeSpot::MarkAllBuffersNoNeed(void)
{
    ...
    for (;ls;ls = ls->m_Next)
    {
	    if (ls->m_VB) DESTROY_VB(ls->m_VB);
        if (ls->m_IB) DESTROY_VB(ls->m_IB);
    }
}

bool CMatrixEffectLandscapeSpot::PrepareDX()
{
    if(IS_VB(m_VB) || IS_IB(m_IB)) return true;
    if(m_CntVerts == 0) return false;
...
void CMatrixEffectLandscapeSpot::BuildLand(const D3DXVECTOR2& pos, float angle, float scalex, float scaley, float addz, bool scale_by_normal)
{
    typedef struct
    {
        D3DXVECTOR3 p;
    ...
    if(mr.right >= g_MatrixMap->m_Size.x) mr.right = g_MatrixMap->m_Size.x - 1;
    ...
    try
    {
        buff = (byte*)_alloca((sizeof(STempVertex) + sizeof(SLandscapeSpotVertex) + sizeof(word) * 3) * da_size);;
    }
    catch(...)
    {
        ERROR_S(L"Not enough memory for buffer after _alloca() call in CMatrixEffectLandscapeSpot::BuildLand function!");
    }
...
CMatrixEffectLandscapeSpot::~CMatrixEffectLandscapeSpot()
{
    ...
    //    {
    //        b.StrNZ(CStr(CWStr(eff->Priority()) + L":" + eff->m_Texture->m_Name).Get());
    //        b.StrNZ("\r\n");
...
{
    if(m_Props->func == SpotTactAlways) return MAX_EFFECT_PRIORITY;
