...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
CMatrixEffectExplosion::CMatrixEffectExplosion(const D3DXVECTOR3& pos, const SExplosionProperties& props) : CMatrixEffect()
{
    ...
    {
        if(m_Debris[i].GetDebType() == m_Props->deb_type)
        {
            ++dt_cnt;
            if(dt_idx == -1) dt_idx = i;
        }
    }

    if(dt_idx < 0) ERROR_S(L"CMatrixEffectExplosion::CMatrixEffectExplosion Error!");

    m_Light.effect = nullptr;
    if(m_Props->light) CMatrixEffect::CreatePointLight(&m_Light, pos, m_Props->light_radius1, m_Props->light_color1, false);
    ...
    for(; fire_cnt > 0; ++i)
    {
        m_Deb[i].fire.effect = nullptr;
        m_Deb[i].light.effect = nullptr;

        CMatrixEffect::CreateFire(&m_Deb[i].fire, pos, 1000000, 1000, 50, 5, true);
        CMatrixEffect::CreatePointLight(&m_Deb[i].light, pos, 20, 0x22222202, false);

        if(m_Deb[i].light.effect == nullptr || m_Deb[i].fire.effect == nullptr)
        {
    ...
    for(; sparks_cnt > 0; ++i)
    {
        ...
        m_Deb[i].bline = (CSpriteSequence*)HAlloc(sizeof(CSpriteSequence), m_Heap);
        m_Deb[i].bline->CSpriteSequence::CSpriteSequence(TRACE_PARAM_CALL pos, pos + m_Deb[i].v * (1.0f / 20.0f), EXPLOSION_SPARK_WIDTH, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_SPARK));
        m_Deb[i].ttl = (float)RND(m_Props->deb_min_ttl, m_Props->deb_max_ttl);
...
#if defined _TRACE
        if(deb->type == DEB_DEAD) _asm int 3
#endif
...
DTRACE();
    ...
    while(m_DebCnt > 0) RemoveDebris(m_DebCnt - 1);
    HFree(m_Deb, m_Heap);
...
{
    if(FLAG(m_before_draw_done, BDDF_EXPLOSION)) return;

DTRACE();

    for(int i = 0; i < m_DebrisCnt; ++i) m_Debris[i].BeforeDraw();
...
DTRACE();
    ...
    while(i < m_DebCnt)
    {
...
#if defined _DEBUG || defined _TRACE
        DCP();
        if(m_Deb[i].type == DEB_DEAD) _asm int 3
        DCP();
#endif
        ...
        D3DXMatrixRotationYawPitchRoll(&mat, m_Deb[i].angley * m_Time, m_Deb[i].anglep * m_Time, m_Deb[i].angler * m_Time);

        mat._11 *= m_Deb[i].scale; mat._12 *= m_Deb[i].scale;   mat._13 *= m_Deb[i].scale;
        mat._21 *= m_Deb[i].scale; mat._22 *= m_Deb[i].scale;   mat._23 *= m_Deb[i].scale;
        mat._31 *= m_Deb[i].scale; mat._32 *= m_Deb[i].scale;   mat._33 *= m_Deb[i].scale;
    ...
    for(i = 0; i < m_DebCnt; ++i)
    {
        ...
        *pos += deb->v * dtime;
        ...
        if(z > pos->z)
        {
            ...
            SMatrixMapUnit *mu = g_MatrixMap->UnitGetTest(Float2Int(pos->x * INVERT(GLOBAL_SCALE)), Float2Int(pos->y * INVERT(GLOBAL_SCALE)));
            if(mu == nullptr || mu->m_MapUnitBase != nullptr)
            {
                deb->ttl = 1;
            }
        }
