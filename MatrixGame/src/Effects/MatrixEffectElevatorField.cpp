...
CMatrixEffectElevatorField::CMatrixEffectElevatorField(const D3DXVECTOR3 &pos0, const D3DXVECTOR3 &pos1, float radius, const D3DXVECTOR3 & fwd):
CMatrixEffect(), m_AllBBCnt(0), m_Time(0), m_NextTime(0), m_Activated(false), m_Angle(0.0f)
{
    DTRACE();
    m_EffectType = EFFECT_ELEVATOR_FIELD;
...
void CMatrixEffectElevatorField::UpdateData(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, float r, const D3DXVECTOR3& fwd)
{
    ...
    {
        static i1 = 0;
        if(g_MatrixMap->m_KeyDown && g_MatrixMap->m_KeyScan == KEY_PGDN) { g_MatrixMap->m_KeyDown = false; i1--; if(i1 < 0) i1 = 4; }
        if(g_MatrixMap->m_KeyDown && g_MatrixMap->m_KeyScan == KEY_PGUP) { g_MatrixMap->m_KeyDown = false; i1++; if(i1 > 4) i1 = 0; }
...
void CMatrixEffectElevatorField::Tact(float t)
{
    m_Angle += t * 0.001f;
    if(m_Angle > M_PI_MUL(2)) { m_Angle -= M_PI_MUL(2); }
    //BuildRotateMatrix(m_Rot, m_Pos, m_Dir, m_Angle);

    if(m_Activated) m_Sound = CSound::Play(m_Sound, S_EF_CONTINUE, m_Pos, SL_ELEVATORFIELD);
    else m_Sound = CSound::Play(m_Sound, S_EF_START, m_Pos, SL_ELEVATORFIELD);
    ...
    while(m_NextTime < m_Time)
    {
        ...
        if(m_AllBBCnt < ELEVATORFIELD_BB_CNT)
        {
            ...
            m_BBoards[m_AllBBCnt].bb = CSpriteSequence(TRACE_PARAM_CALL p, p, ELEVATORFIELD_BB_SIZE, 0x00FFFFFF, GetSingleBrightSpriteTex(SPR_DISCHARGER_SPOT));
