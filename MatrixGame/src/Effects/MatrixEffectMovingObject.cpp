...
void MO_Homing_Missile_Tact(D3DXMATRIX& m, SMOProps& props, float tact)
{
    ...
    if(props.hm.homing_speed > 0.0f) //Если у ракеты включено самонаведение (при нулевом или минусовом значении оно отключено)
    {
        ...
        g_MatrixMap->FindObjects(seek_center, HOMING_RADIUS, 1, TRACE_ROBOT | TRACE_FLYER | TRACE_TURRET | TRACE_BUILDING, props.skip, HMEnum, (dword)&data); //props нихуя не CMatrixMapStatic, но за это говно скажем спасибо дабу
    ...
    if(hit || distance < (MISSILE_IMPACT_RADIUS * MISSILE_IMPACT_RADIUS)) //Сравниваем площади
    {
        ...
        {
            CMatrixEffect::CreateLandscapeSpot(nullptr, D3DXVECTOR2(hit_pos.x, hit_pos.y), FSRND((float)M_PI), FRND(3) + 6, SPOT_VORONKA);
