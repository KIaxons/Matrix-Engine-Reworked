...
ELIST_DECLARE_OUTCLASS(EFFECT_FIRE_ANIM);
...
void CEffectSpawner::Tact(float ms)
{
    if(m_Under == nullptr)
    {
        FOwner data;
...
void CMatrixEffect::InitEffects(CBlockPar& bp_in)
{
    ...
    for(int i = 0; i < m_DebrisCnt; ++i)
    {
        ...
        m_Debris[i].InitVOAnim(vo, nullptr);
    ...
    */
    ...
    bp_sprite_textures.ParAdd(L"CoolingFanAnim1",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_1);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim2",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_2);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim3",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_3);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim4",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_4);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim5",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_5);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim6",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_6);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim7",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_7);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim8",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_8);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim9",  (CWStr)SPR_COOLING_FAN_ANIM_FRAME_9);
    bp_sprite_textures.ParAdd(L"CoolingFanAnim10", (CWStr)SPR_COOLING_FAN_ANIM_FRAME_10);
    ...
    {
        int cnt = g_Config.m_RobotChassisConsts[i].jet_stream.size();
        for(int t = 0; t < cnt; ++t)
        {
            CWStr sprite_name;
            int sprite_num = -1;
        ...
        cnt = g_Config.m_RobotChassisConsts[i].linked_sprite_anim.size();
        for(int t = 0; t < cnt; ++t)
        {
            CWStr sprite_name;
            int sprite_num = -1;
            ...
            sprite_name = g_Config.m_RobotChassisConsts[i].linked_sprite_anim[t].sprites_name;
            sprites_count = g_Config.m_RobotChassisConsts[i].linked_sprite_anim[t].sprites_count;
            for(int j = 1; j <= sprites_count; ++j)
            {
                sprite_num = AddSpriteTexture(sprite_name, j, sprites_count, sprites_block, bp_sprite_textures);
                g_Config.m_RobotChassisConsts[i].linked_sprite_anim[t].sprites_num.push_back(sprite_num);
            }
        }
    }
    ...
    for(int i = 1; i < g_Config.m_WeaponsConsts.size(); ++i)
    {
        ...
        int cnt = 1 + g_Config.m_WeaponsConsts[i].sprite_set.size();
        for(int t = 0; t < cnt; ++t)
        {
            CWStr sprite_name;
            int sprite_num = -1;
...
void CMatrixEffect::CreateFireAnim(SEffectHandler* eh, const D3DXVECTOR3& pos, float anim_width, float anim_height, int time_to_live, const std::vector<int>& sprites_id)
{
    ...
    if(ELIST_CNT(EFFECT_FIRE_ANIM) >= MAX_EFFECTS_FIREANIM)
    {
        // remove oldest
        ELSIT_REMOVE_OLDEST(EFFECT_FIRE_ANIM);
    }
...
{
    CMatrixEffectRepair* effect = HNew(m_Heap) CMatrixEffectRepair(pos, dir, seek_radius, skip, sprite_spot);
    return effect;
}

CMatrixEffectFireStream* CMatrixEffect::CreateFireStream(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, float width, dword color, const std::vector<int>& sprites_num)
{
    CMatrixEffectFireStream* effect = HNew(m_Heap) CMatrixEffectFireStream(pos0, pos1, width, color, sprites_num);
    return effect;
}

CMatrixEffectLinkedSpriteAnim* CMatrixEffect::CreateLinkedSpriteAnim(const D3DXVECTOR3& pos0, const D3DXVECTOR3& pos1, float width, dword color, int frame_delay, bool camera_oriented, const std::vector<int>& sprites_num)
{
    CMatrixEffectLinkedSpriteAnim* effect = HNew(m_Heap) CMatrixEffectLinkedSpriteAnim(pos0, pos1, width, color, frame_delay, camera_oriented, sprites_num);
    return effect;
}

void CMatrixEffect::DrawBegin()
{
    float fBias = -1.0f;
...
/*
    ...
    {
        if(pri[i] == -1) break;
        g_MatrixMap->m_DI.ShowScreenText(L"ls:" + CWStr(pri[i]), CWStr(pric[i]), 1000);
    }
#endif
*/
}