...
CMatrixEffectFirePlasma::CMatrixEffectFirePlasma(const D3DXVECTOR3& start, const D3DXVECTOR3& end, float speed, dword hitmask, CMatrixMapStatic* skip, FIRE_END_HANDLER handler, dword user) :
CMatrixEffect(), m_Pos(start), m_End(end), m_Speed(speed), m_UserData(user), m_Handler(handler), m_HitMask(hitmask),
m_Skip(skip),
m_BBLine(TRACE_PARAM_CALL start, start, FIRE_PLASMA_W * 1.4f, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_PLASMA_RECT)),
m_BB1(TRACE_PARAM_CALL start, FIRE_PLASMA_W, 0, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_PLASMA_SPOT)),
m_BB2(TRACE_PARAM_CALL start, FIRE_PLASMA_W, 0, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_PLASMA_SPOT)),
m_BB3(TRACE_PARAM_CALL start, FIRE_PLASMA_W, 0, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_PLASMA_SPOT)),
m_BB4(TRACE_PARAM_CALL start, FIRE_PLASMA_W, 0, 0xFFFFFFFF, GetSingleBrightSpriteTex(SPR_PLASMA_SPOT))
#ifdef _DEBUG
...
CMatrixEffectFirePlasma::~CMatrixEffectFirePlasma()
{
    ...
    if(m_Handler) m_Handler(TRACE_STOP_NONE, m_Pos, m_UserData, FEHF_LASTHIT);
}
...
void CMatrixEffectFirePlasma::Tact(float step)
{
DTRACE();
    ...
    {
        // finish
        if(m_Handler) m_Handler(hito, hitpos,  m_UserData, FEHF_LASTHIT);
        m_Handler = nullptr;
        ...
        else
        {
            if(hito == TRACE_STOP_WATER)
            {
                ...
                CMatrixEffect::CreateSmoke(nullptr, hitpos, 100, 1400, 41, 0xFFFFFFFF);
            }
            else CMatrixEffect::CreateSmoke(nullptr, hitpos, 100, 1400, 41, 0xFF3F2F2F);
        }
#ifdef _DEBUG
