...
CFlamePuff::CFlamePuff(CMatrixEffectFlame* owner, const D3DXVECTOR3& pos, const D3DXVECTOR3& dir, const D3DXVECTOR3& speed) :
    m_Owner(owner), m_Pos(pos), m_Dir(dir), m_Speed(speed)
{
    CMatrixEffect::CreatePointLight(&m_Light, pos, 20, 0xFFFFFFFF, false);

    //Здесь задаётся интенсивность визуального эффекта
    //Фактически, сила альфа-канала спрайтов огня
    //Закоментированное присвоение вводит зависимость интенсивности альфа-канала от дальности струи в конфигах
    //И при этом не зависит от её фактической дальности, например, при разгоне робота
    //Что при небольшой дальности, по сути, делает огненные спрайты очень бледными и едва различимыми
...
CFlamePuff::~CFlamePuff()
{
    ...
    //if(m_Prev) m_Prev->m_Next = nullptr;
...
void CFlamePuff::Release()
{
    HDelete(CFlamePuff, this, m_Owner->m_Heap);
}

void CFlamePuff::Draw()
{
    for(int i = 0; i < FLAME_NUM_BILLS; ++i)
    {
...
static bool FlameEnum(const D3DXVECTOR3& center, CMatrixMapStatic* ms, dword user)
{
    ...
    if(fd->handler) fd->handler(ms, center + fd->norm * ms->GetRadius(), fd->uvalue, 0);
    return true;
}

void CFlamePuff::Tact(float step)
{
DTRACE();
    ...
    */
    ...
    if(m_Owner->m_hitmask & TRACE_WATER)
    {
        ...
        {
            if(m_Owner->m_Handler) m_Owner->m_Handler(TRACE_STOP_WATER, D3DXVECTOR3(m_Pos.x,m_Pos.y, WATER_LEVEL), m_Owner->m_User,0);
            data.norm.z += 1;
            hit = true;
        }
    ...
    for(int i = 0; i < FLAME_NUM_BILLS; ++i)
    {
        ...
        else
        {
            ...
            int b = byte(kk * 512.0f); if (b > 255) b = 255;
            ...
            if(x > scale * k1) m_Flames[i].m_Flame.m_Pos += delta * (x - scale * k1) / x;
        }
    }
    ...
    else
    {
        if(m_Prev && (!m_Break))
        {
            ...
            //Создаём и линкуем на указатель новый CSpriteSequence
            m_Shleif = (CSpriteSequence*)HAlloc(sizeof(CSpriteSequence), m_Owner->m_Heap);
            new(m_Shleif) CSpriteSequence(TRACE_PARAM_CALL m_Prev->m_Pos, m_Pos, m_Scale * 2.0f, (a << 24) | 0xFFFFFF, m_Owner->GetSingleBrightSpriteTex(SPR_FLAMETHROWER_BEAM));
        }
    }
}

////////////////
CMatrixEffectFlame::CMatrixEffectFlame(float ttl, dword hitmask, CMatrixMapStatic* skip, dword user, FIRE_END_HANDLER handler) :
    CMatrixEffect(), m_User(user), m_Handler(handler), m_hitmask(hitmask), m_skip(skip), m_TTL(ttl), _m_TTL(INVERT(ttl))
#ifdef _DEBUG
...
CMatrixEffectFlame::~CMatrixEffectFlame()
{
    ...
    if(m_Handler) m_Handler(TRACE_STOP_NONE, D3DXVECTOR3(0, 0, 0), m_User, FEHF_LASTHIT);
    //m_Owner->Release();
}
...
void CMatrixEffectFlame::AddPuff(const D3DXVECTOR3& start, const D3DXVECTOR3& dir, const D3DXVECTOR3& speed)
{
    ...
    p->m_Next = m_First;
    if(m_First) m_First->m_Prev = p; 
    else m_Last = p; 
    p->m_Prev = nullptr;
    m_First = p;

    if(m_Stream == nullptr && p->m_Next) p->m_Next->m_Break = 1;
...
void CMatrixEffectFlame::SubPuff(CFlamePuff *puf)
{
    ...
    if(m_Stream == puf) m_Stream = nullptr;
...
{
    for(CFlamePuff* p = m_First; p; p = p->m_Next) p->Draw();
}

void CMatrixEffectFlame::Tact(float ms)
{
...
{
    if(m_Smokes.effect == nullptr) CMatrixEffect::CreateShleif(&m_Smokes);
    if(m_Smokes.effect != nullptr) ((CMatrixEffectShleif *)m_Smokes.effect)->AddSmoke(pos, 300, 700, 400, 0x2F303030, false, 0);
}