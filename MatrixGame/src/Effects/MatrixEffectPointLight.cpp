...
SPL_VBIB *SPL_VBIB::GetCreate(int vbsize, int ibsize, dword fvf)
{
    ...
    if (m_FirstFree)
    {
        SPL_VBIB *candidate = nullptr;
        r = m_FirstFree;
        for (;r;r = r->m_NextFree)
        {
            if (r->m_FVF != fvf) continue;
            if (candidate == nullptr) candidate = r;
        ...
        if (candidate == nullptr) goto create;
...
CMatrixEffectPointLight::CMatrixEffectPointLight(const D3DXVECTOR3& pos, float radius, dword color, bool drawbill) :
    CMatrixEffect(), m_Pos(pos), m_Radius(radius), m_Color(color), m_PointLum(m_Heap)
{
    m_EffectType = EFFECT_POINT_LIGHT;
    ELIST_ADD(EFFECT_POINT_LIGHT);

    if(!g_Config.m_VertexLight) m_Tex = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, TEXTURE_PATH_POINTLIGHT);

    if(drawbill)
    {
        m_Sprite = (CSprite*)HAlloc(sizeof(CSprite), m_Heap);
        if(m_SpriteTextures[SPR_SPOT_LIGHT].IsSingleBrightTexture()) m_Sprite->CSprite::CSprite(TRACE_PARAM_CALL pos, radius * POINTLIGHT_BILLSCALE, 0, color, m_SpriteTextures[SPR_SPOT_LIGHT].tex);
        else m_Sprite->CSprite::CSprite(TRACE_PARAM_CALL pos, radius * POINTLIGHT_BILLSCALE, 0, color, &m_SpriteTextures[SPR_SPOT_LIGHT].spr_tex);
    }
...
void CMatrixEffectPointLight::MarkAllBuffersNoNeed()
{
    SPL_VBIB::ReleaseFree();
    SPL_VBIB::ReleaseBuffers();
}

void CMatrixEffectPointLight::Clear()
{
	//if(IS_VB(m_VB)) { DESTROY_VB(m_VB); }
    //if(IS_IB(m_IB)) { DESTROY_IB(m_IB); }
...
void CMatrixEffectPointLight::BuildLand()
{
    typedef struct
    {
        ...
        dword            outside;// = 0;
        int              index;// = 0;
        SMatrixMapPoint* mp;// = nullptr;
        float            lum;// = 0.0f;

    } STempVertex;

    CRect mr(Float2Int((m_Pos.x - m_Radius) * INVERT(GLOBAL_SCALE)) - 1, Float2Int((m_Pos.y - m_Radius) * INVERT(GLOBAL_SCALE)) - 1,
         1 + Float2Int((m_Pos.x + m_Radius) * INVERT(GLOBAL_SCALE)), 1 + Float2Int((m_Pos.y + m_Radius) * INVERT(GLOBAL_SCALE)));
    ...
    try
    {
        buff = (byte*)_alloca((sizeof(STempVertex) + sizeof(SPointLightVertex) + sizeof(word) * 3) * da_size);
    }
    catch (...)
    {
        ERROR_S(L"Not enough memory for buffer after _alloca() call in CMatrixEffectPointLight::BuildLand function!");
    }
...
#define ADDVERT(v) if(v->index < 0)                                       \
                   ...
                   {                                                      \
                       if(v->index < midx) midx = v->index;               \
                       if(v->index > (midx + madx)) madx = v->index-midx; \
                   }
    ...
    for(y = mr.top; y <= mr.bottom; ++y)
    {
        for(x = mr.left; x <= mr.right; ++x, ++tv, ++mp)
        {
            ...
            if(x >= 0 && x <= g_MatrixMap->m_Size.x && y >= 0 && y <= g_MatrixMap->m_Size.y)
            {
                ...
                if(tv->p.z < WATER_LEVEL) tv->p.z = WATER_LEVEL;
            }
            else
            {
                tv->p.z = WATER_LEVEL;
...
void CMatrixEffectPointLight::BuildLandV()
{
    typedef struct
    {
        //unnamed class used in typedef name cannot declare members other than non - static data members, member enumerations, or member classes
    ...
    try
    {
        buff = (byte*)_alloca((sizeof(STempVertex) + sizeof(SPointLightVertexV) + sizeof(word) * 3) * da_size);
    }
    catch(...)
    {
        ERROR_S(L"Not enough memory for buffer after _alloca() call in CMatrixEffectPointLight::BuildLandV function!");
    }
...
void CMatrixEffectPointLight::UpdateData()
{
    Clear();
    ...
    if(m_Sprite)
    {
        ...
        m_Sprite->SetScale(m_Radius * POINTLIGHT_BILLSCALE);
    }

    //RemoveColorData();
    if(g_Config.m_VertexLight) BuildLandV();
    else BuildLand();
...
void CMatrixEffectPointLight::RemoveColorData()
{
    if(m_PointLum.Len() == 0) return;
...
void CMatrixEffectPointLight::Draw(void)
{
    ...
    if (m_Sprite) m_Sprite->Sort(g_MatrixMap->m_Camera.GetViewMatrix());
    if (!m_DX) return;
...
void CMatrixEffectPointLight::Tact(float step)
{
    DTRACE();
