...
{
    m_EffectType = EFFECT_SMOKE_SHLEIF;
    m_SmokesCnt = 0;
    m_TTL = SHLEIF_TTL;
...
void CMatrixEffectShleif::Tact(float step)
{
    DTRACE();
    m_TTL -= step;
    if (m_TTL < 0)
    ...
    for(int i = 0; i < m_SmokesCnt; ++i)
    {
        ...
        else
        {
            while(i < (m_SmokesCnt - 1))
            {
                ...
                m_Smokes[m_SmokesCnt].effect = nullptr;
                m_Smokes[i].Rebase();

                if(m_Smokes[i].effect) break;
            }
        }
    }
}
...
DTRACE();
    ...
    for (int i = 0; i < m_SmokesCnt; ++i)
    {
        if(m_Smokes[i].effect) m_Smokes[i].effect->Draw();
    }
}
...
void CMatrixEffectShleif::AddSmoke(
...
void CMatrixEffectShleif::AddFire(
    const D3DXVECTOR3& pos,
    float ttl,
    float puffttl,
    float spawntime,
    ...
    bool is_bright,
    float speed,
    SFloatRGBColor close_color,
    SFloatRGBColor far_color
)
{
    if(m_SmokesCnt < SHLEIF_MAX_SMOKES)
    {
...
/*
void CMatrixEffectShleif::AddFlame(const D3DXVECTOR3& start, const D3DXVECTOR3& dir, const D3DXVECTOR3& speed, float ttl, dword hitmask, CMatrixMapStatic* skip, dword user, FIRE_END_HANDLER handler)
{
    if(m_SmokesCnt < SHLEIF_MAX_SMOKES)
    {
