...
{
    int idx = g_MatrixMap->IsPaused() ? 1 : (IRND(m_SpritesCount)); //¬ыбор случайного номера кадра
    ...
    CMatrixEffect(), m_Pos(pos), m_Width(anim_width), m_Height(anim_height), m_SpritesCount(sprites_count), m_Sprites(new CSpriteSequence[sprites_count])
{
    ...
    m_AnimFrame = IRND(m_SpritesCount); //ƒл€ старта анимации всегда выбираем случайный кадр
