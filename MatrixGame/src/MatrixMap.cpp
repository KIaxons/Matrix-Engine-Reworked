...
CMatrixMap::CMatrixMap() : CMain(), m_Console(), m_Camera(), m_AllObjects(Base::g_MatrixHeap, 1024), m_RoadNetwork(Base::g_MatrixHeap), m_DialogModeHints(Base::g_MatrixHeap)
{
    ...
    //Задаём модификаторы сложности игры
...
//Загружаем модели и текстуры роботов по путям из кэша, определяем все связанные с конкретными моделями параметры
...
void CMatrixMap::BeforeDraw()
{
    ...
    */
    ...
    //TAKT_BEGIN();
    //Здесь регулярно обновляем m_TraceStopPos и m_TraceStopObj
...
bool CMatrixMap::FindObjects(
    ...
    dword user //Просто иди нахуй за такое даб
)
{
    bool hit = false;
...
void CMatrixMap::AddEffectSpawner(float x, float y, float z, int ttl, const CWStr& par)
{
    ...
    //Создаются объекты эффектов (выделение памяти по данному указателю происходит выше)
...
//Простой выход из диалогового окна без дальнейших действий (например, окно со вступительным текстом)
void CMatrixMap::OkHandler()
{
    g_MatrixMap->LeaveDialogMode();
}
//Выход из диалогового окна и открытие статистики боя (без поражения, либо победы)
...
//Выход из победного диалогового и вызов окна со статистикой боя
...
//Выход из пораженческого диалогового и вызов окна со статистикой боя
...
//Выход из диалогового окна и завершение приложения
...
//Открывает окно с вступительным и победным/пораженческим текстом, либо подтверждение 
    ...
    for(int i = 0; i < cnt; ++i)
    {
        if(bp->ParGetName(i) == hint)
        {
            ...
            if(h->GetCopyPosCnt() > 0)
            {
                ...
                //Задаём действия кнопкам "Ок" в открытом окне
                if(!wcscmp(hint, TEMPLATE_DIALOG_MENU))
                {
