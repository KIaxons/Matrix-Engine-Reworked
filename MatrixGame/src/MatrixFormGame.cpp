...
void CFormMatrixGame::Draw(void)
{
    ...
    {
        g_MatrixMap->m_DI.ShowScreenText(L"Automatic mode", L"");
    }

    if(FLAG(g_Config.m_DIFlags, DI_DRAW_FPS)) g_MatrixMap->m_DI.ShowScreenText(L"FPS", CWStr(g_DrawFPS, Base::g_MatrixHeap).Get());
    if(FLAG(g_Config.m_DIFlags, DI_TMEM))
    {
        g_MatrixMap->m_DI.ShowScreenText(L"Free Texture Mem", CWStr(g_AvailableTexMem, Base::g_MatrixHeap).Get());
    }
    if(FLAG(g_Config.m_DIFlags, DI_TARGET_COORD))
    {
        ...
        //txt.Insert(txt.GetLen()-1,L".",1);
        g_MatrixMap->m_DI.ShowScreenText(L"Camera target", txt.Get());
    }
    if(FLAG(g_Config.m_DIFlags, DI_FRUSTUM_CENTER))
    {
        CWStr txt(Base::g_MatrixHeap);
        ...
        txt.Insert(txt.GetLen() - 1, L".", 1);
        g_MatrixMap->m_DI.ShowScreenText(L"Frustum Center", txt.Get());
        ...
        //txt.Insert(txt.GetLen() - 1, L".", 1);
        //g_MatrixMap->m_DI.ShowScreenText(L"Cam dist", txt.Get());

        //g_MatrixMap->m_DI.ShowScreenText(L"Z rel", CWStr(g_MatrixMap->m_Camera.GetZRel()));
    }
...
void CFormMatrixGame::Tact(int step)
{
    ...
    if(!g_MatrixMap->GetPlayerSide()->IsManualControlMode())
    {
        ...
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_LEFT]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_LEFT_ALT]) & 0x8000) == 0x8000))
        {
            if(!g_MatrixMap->GetPlayerSide()->m_ConstructPanel->IsActive()) g_MatrixMap->m_Camera.MoveLeft();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_RIGHT]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_RIGHT_ALT]) & 0x8000) == 0x8000))
        {
            if(!g_MatrixMap->GetPlayerSide()->m_ConstructPanel->IsActive()) g_MatrixMap->m_Camera.MoveRight();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_UP]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_UP_ALT]) & 0x8000) == 0x8000))
        {
            if(!g_MatrixMap->GetPlayerSide()->m_ConstructPanel->IsActive()) g_MatrixMap->m_Camera.MoveUp();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_DOWN]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SCROLL_DOWN_ALT]) & 0x8000) == 0x8000))
        {
            if(!g_MatrixMap->GetPlayerSide()->m_ConstructPanel->IsActive()) g_MatrixMap->m_Camera.MoveDown();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_ROTATE_LEFT]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_ROTATE_LEFT_ALT]) & 0x8000) == 0x8000))
        {
            g_MatrixMap->m_Camera.RotLeft();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_ROTATE_RIGHT]) & 0x8000) == 0x8000 || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_ROTATE_RIGHT_ALT]) & 0x8000) == 0x8000))
        {
            g_MatrixMap->m_Camera.RotRight();
        }
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_SPEED_UP]) & 0x8000))
        {
            g_MatrixMap->m_Camera.SpeedUp();
        }
    }

    if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_ROTATE_UP]) & 0x8000) == 0x8000)
    {
        g_MatrixMap->m_Camera.RotUp();
    }
    if((GetAsyncKeyState(g_Config.m_KeyActions[KA_CAM_ROTATE_DOWN]) & 0x8000) == 0x8000)
    {
                    ...
                    if(hr == D3D_OK)
                    {
                        ...
                        if(hr == D3D_OK)
                        {
                            ...
                            if(hr == D3D_OK)
                            {
                                ...
                                g_MatrixMap->m_DI.ShowScreenText(L"Screen shot has been saved", L"");
                            }
                            else
                            {
                                //LockRect fail
...
void CFormMatrixGame::MouseMove(int x, int y)
{
    ...
    if(g_MatrixMap->IsMouseCam())
    {
        ...
        if(p_side->GetUnitUnderManualControl())
        {
            int dx = x - g_MatrixMap->m_Cursor.GetPosX();
            //if(dx < 0) p_side->GetUnitUnderManualControl()->AsRobot()->RotateRobotLeft();
            //if(dx > 0) p_side->GetUnitUnderManualControl()->AsRobot()->RotateRobotRight();

            //Поворот шасси
            CMatrixRobotAI* robot = p_side->GetUnitUnderManualControl()->AsRobot();
            if(dx < 0)
            {
            ...
            if(true/*cbs.calls > 0*/)
            {
                ...
                if(robots_cnt > 1 || flyers_cnt > 1 || (robots_cnt + flyers_cnt) > 1)
                {
                    ...
                    if((GetAsyncKeyState(g_Config.m_KeyActions[KA_SHIFT]) & 0x8000) == 0x8000 && ps->GetCurGroup())
                    {
                        ...
                        while(go)
                        {
                            if(ps->GetCurGroup()->FindObject(go->ReturnObject()))
                            {
                                CMatrixGroupObject* go_tmp = go->m_NextObject;
                                ps->RemoveObjectFromSelectedGroup(go->ReturnObject());
                                go = go_tmp;
                                continue;
                            }
                            go = go->m_NextObject;
                    ...
                    else ps->SetCurGroup(ps->CreateGroupFromCurrent());
                ...
                else if(robots_cnt == 1 && !flyers_cnt)
                {
                    ...
                    if((GetAsyncKeyState(g_Config.m_KeyActions[KA_SHIFT]) & 0x8000) == 0x8000 && ps->GetCurGroup())
                    {
                        if(ps->GetCurGroup()->FindObject(ps->GetCurSelGroup()->m_FirstObject->ReturnObject()))
                        {
                            ps->RemoveObjectFromSelectedGroup(ps->GetCurSelGroup()->m_FirstObject->ReturnObject());
                        }
                        else
                        {
                            ps->AddToCurrentGroup();
...
//Отладочное завершение боя - победа, либо поражение, по нажатию Left/Right Shift + Esc соответственно
    ...
    //bool down - однократное нажатие кнопки (точнее, какая конкретно кнопка была утоплена)
    //int scan - запись последовательности нажатых за последнее время кнопок
    ...
    if(down)
    {
        ...
        else
        {
            ...
            if(!FLAG(g_IFaceList->m_IfListFlags, ORDERING_MODE)/*!IS_PREORDERING_NOSELECT*/)
            {
                ...
                if(ps->GetCurGroup() && (ps->m_CurrSel == ROBOT_SELECTED || ps->m_CurrSel == FLYER_SELECTED || ps->m_CurrSel == GROUP_SELECTED))
                {
                    ...
                    else if((GetAsyncKeyState(g_Config.m_KeyActions[KA_ORDER_STOP]) & 0x8000) == 0x8000)
                    {
                        //"S"top - Стоять, блять!
                        ps->PGOrderStop(ps->SelGroupToLogicGroup());
                        ps->SelectedGroupBreakMoveOrders();
                    }
            ...
            else if((GetAsyncKeyState(g_Config.m_KeyActions[KA_ORDER_ROBOT_SWITCH1]) & 0x8000) == 0x8000)
            {
                //","
                CMatrixMapStatic* obj = CMatrixMapStatic::GetFirstLogic();
                if(ps->GetCurGroup() && ps->GetCurGroup()->GetObjectsCnt() == 1 && ps->GetCurGroup()->m_FirstObject->ReturnObject()->IsRobotAlive())
                {
            ...
            else if(((GetAsyncKeyState(g_Config.m_KeyActions[KA_ORDER_ROBOT_SWITCH2]) & 0x8000) == 0x8000))
            {
                ...
                if(ps->GetCurGroup() && ps->GetCurGroup()->GetObjectsCnt() == 1 && ps->GetCurGroup()->m_FirstObject->ReturnObject()->IsRobotAlive())
                {
                    obj = ps->GetCurGroup()->m_FirstObject->ReturnObject()->GetNextLogic();
                }
            ...
            if(fCtrl)
            {
                ...
                if(ps->GetCurGroup())
                {
                    ...
                    while(go)
                    {
                        ...
                        {
                            ((CMatrixRobotAI*)go->ReturnObject())->SetCtrlGroup(scan);
                        }
                        ...
                        {
                            ((CMatrixFlyer*)go->ReturnObject())->SetCtrlGroup(scan);
                        }
                        go = go->m_NextObject;
                    }
                }
            ...
            else
            {
                ...
                if(ps->GetCurGroup())
                {
                    if(m_LastScans[MAX_SCANS - 1].scan == scan && m_LastScans[MAX_SCANS - 2].scan == scan && (m_LastScans[MAX_SCANS - 1].time - m_LastScans[MAX_SCANS - 2].time) < DOUBLESCAN_TIME_DELTA)
                    {
                        ...
                        if(ps->GetCurGroup()->m_FirstObject) object = ps->GetCurGroup()->m_FirstObject->ReturnObject();

                        if(object && object->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
                        {
...
//BUTTON PRESSED
        ...
        if(scan == KEY_Q)
        {
            ...
            g_MatrixMap->m_DI.ShowScreenText(L"LogicDump", L"LogicDump", 1000);
        }
        else if(/*scan == KEY_T*/false)
        {
            ...
            while(mps)
            {
                ...
                {
                    cnt += mps->AsBuilding()->m_BuildingQueue.GetItemsCnt();
                }
            ...
            if(cnt<side->GetMaxSideRobots())
            {
                ...
                while(mps)
                {
                    if(mps->GetSide() == side_id && mps->GetObjectType() == OBJECT_TYPE_BUILDING && mps->AsBuilding()->IsBase())
                    {
                        ...
                        if(mps->AsBuilding()->m_BuildingQueue.GetItemsCnt() < 6)
                        {
