...
void CMatrixMapGroup::MarkAllBuffersNoNeed()
{
...
CMatrixMapGroup::CMatrixMapGroup() : m_RemindCore(FreeResources, (dword)this)
{
    //Нахуй. Всё уже давно инициализируется на полях.
    ...
    if(m_BigIB_bottom == nullptr) m_BigIB_bottom = CBigIB::NewBigIB(g_MatrixHeap);
...
DTRACE();
    ...
    if(HasWater())
    {
        if(m_Inshorewaves)
        {
            while(m_InshorewavesCnt > 0) m_Inshorewaves[--m_InshorewavesCnt].Release();
            HFree(m_Inshorewaves, g_MatrixHeap);
            m_Inshorewaves = nullptr;
        }

        if(m_PreInshorewaves)
        {
            HFree(m_PreInshorewaves, g_MatrixHeap);
            m_PreInshorewaves = nullptr;
        }
    }
    ...
    if(m_RenderShadowObject)
    {
        ...
        m_RenderShadowObject = nullptr;
    }

    if(m_VertsTrace)
    {
        HFree(m_VertsTrace, g_MatrixHeap);
        m_VertsTrace = nullptr;
    }
...
void CMatrixMapGroup::AddShadow(CMatrixShadowProj* s)
{
    if(m_Shadows)
    {
        ...
        {
            if(m_Shadows[i] == s) return;
            ++i;
        }
    }

    m_Shadows = (CMatrixShadowProj**)HAllocEx(m_Shadows, (m_ShadowsCnt + 1) * sizeof(CMatrixShadowProj*), g_MatrixHeap);
...
DTRACE();
    ...
    for(int i = 0; i < m_ShadowsCnt; ++i)
    {
        ...
        if(m_Shadows[i]->GetTexture() == nullptr)
        {
            if(m_RenderShadowObject == nullptr) m_RenderShadowObject = m_Shadows[i]->GetOwner()->GetCore(DEBUG_CALL_INFO);
        }
        else m_Shadows[i]->Render();
    }
}


bool CMatrixMapGroup::IsInFrustum() const
{
...
DTRACE();
    ...
    m_IdxsSource_bottom.inds = nullptr;

    m_BottomGeometryCount = *(dword*)rawbottom; rawbottom += sizeof(dword);
    ...
    {
        m_BottomGeometry[i].texture = *(dword*)rawbottom; rawbottom += sizeof(dword);

        //if(m_BottomGeometry[i].texture < 0) downz = -0.1f;

        dword sz = *(dword*)rawbottom; rawbottom += sizeof(dword);

        m_IdxsSource_bottom.inds = (word*)HAllocEx(m_IdxsSource_bottom.inds, m_IdxsSource_bottom.size + sz, g_MatrixHeap);
    ...
    dword vsz = *(dword*)rawbottom; rawbottom += sizeof(dword);
    ...
    {
        SCompileBottomVert* v = (SCompileBottomVert*)rawbottom; rawbottom += sizeof(SCompileBottomVert);
        ...
        while(!down && v->x >= 1 && v->y >= 1 && v->x < g_MatrixMap->m_Size.x && v->y < g_MatrixMap->m_Size.y)
        {
            ...
            if(!mu->IsDown()) break;
            if(!(mu - 1)->IsDown()) break;
            if(!(mu - 1 - g_MatrixMap->m_Size.x)->IsDown()) break;
            if(!(mu - g_MatrixMap->m_Size.x)->IsDown()) break;
            down = true;
            mp->color |= 0x80000000;
            break;
        }
        ...
        if(down) m_VertsSource_bottom.verts[i].ivd.v -= mp->n * 0.5f;
    ...
    if(m_minz < 0) SETFLAG(m_Flags, GRPFLAG_HASWATER);
}
...
void CMatrixMapGroup::BuildWater(int x, int y)
{
    if(HasWater())
    {
        ...
        if (m_Inshorewaves == nullptr && !FLAG(g_MatrixMap->m_Flags, MMFLAG_DISABLEINSHORE_BUILD))
        {
            ...
            while (posc_cnt-- > 0)
            {
                pos = D3DXVECTOR2((posc[posc_cnt].x + x) * GLOBAL_SCALE + FRND(GLOBAL_SCALE), (posc[posc_cnt].y + y) * GLOBAL_SCALE + FRND(GLOBAL_SCALE));
                ...
                {
                    m_PreInshorewaves = (SPreInshorewave *)HAlloc(INSHORE_PRE_COUNT * sizeof(SPreInshorewave), g_MatrixHeap);
...
void CMatrixMapGroup::BeforeDraw(void)
{
    ...
    if (HasWater()) GetWaterAlpha()->Preload();

    if (m_RenderShadowObject)
    {
        if (m_RenderShadowObject->m_Object)
        {
            m_RenderShadowObject->m_Object->GetResources(MR_ShadowProjTex|MR_Matrix|MR_Graph);
        }
...
void CMatrixMapGroup::Draw(void)
{
    ...
    int type = (g_MatrixMap->m_Macrotexture == nullptr) ? 0 : 1;
    ...
    for (int i = 0; i<m_BottomGeometryCount; ++i)
    {
        ...
        if (bg->texture < 0)
        {
            ...
            ASSERT_DX(g_D3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,vbase,bg->minvidx, bg->vertcnt, ibase + bg->idxbase, bg->tricnt));
    ...
    //    static int i1 = -1;
    //    int prei = i1;
    //    if (g_MatrixMap->m_KeyDown && g_MatrixMap->m_KeyScan == KEY_F7) {g_MatrixMap->m_KeyDown = false; i1--; if (i1 < 0) i1 = CTerSurface::GetSufracesCount()-1;}
    //    if (g_MatrixMap->m_KeyDown && g_MatrixMap->m_KeyScan == KEY_F8) {g_MatrixMap->m_KeyDown = false; i1++; if (i1 >= CTerSurface::GetSufracesCount()) i1 = 0;}
    //    CTerSurface *su = CTerSurface::GetSufrace(i1);
    //    if (prei != i1 && prei >=0) CTerSurface::GetSufrace(prei)->unstable = false;
    //    su->unstable = true;
...
void  CMatrixMapGroup::RecalcMaxZ(void)
{
    ...
    while (--cnt >= 0)
    {
        ...
        if (ms->GetObjectType() == OBJECT_TYPE_FLYER) continue;
        ...
        {
            if (maxs.z > m_maxz_obj) m_maxz_obj = maxs.z;
        }

    }
    if (m_maxz_obj_robots < m_maxz_obj) m_maxz_obj_robots = m_maxz_obj;
...
void CMatrixMapGroup::AddObject(CMatrixMapStatic *obj)
{
	...
	if (m_Objects)
    {
        ...
        } else
        {
            ...
            m_Objects=(CMatrixMapStatic * *)HAllocEx(m_Objects,m_ObjectsAllocated*sizeof(CMatrixMapStatic * *),g_MatrixHeap);
            m_Objects[m_ObjectsContained++] = obj;
        }
    } else
    {
...
#ifdef _DEBUG
            // check that
            while (i<m_ObjectsContained) {if (m_Objects[i] == obj) ERROR_S(L"The same object is in the group with more than one times"); ++i;}
#endif
            return;
        }
        ++i;
...
bool CMatrixMapGroup::Pick(const D3DXVECTOR3& pos, const D3DXVECTOR3& _dir, float& outt)
{
    ...
    if(m_IdxsTraceCnt == 0)
    {
        ...
        m_VertsTrace = (D3DXVECTOR3*)HAlloc(sizeof(D3DXVECTOR3) * (MAP_GROUP_SIZE + 1) * (MAP_GROUP_SIZE + 1), g_MatrixHeap);
        ...
        int* set = (int*)_malloca(sz);
        if(!set) ERROR_S(L"Not enough memory for set after _malloca() call in CMatrixMapGroup::Pick function!");
        memset(set, -1, sz);
        ...
        m_IdxsTraceCnt = ci;
        m_VertsTraceCnt = cv;
        _freea(set); //Очистка после вызова _malloca
    }

    byte* vflags = (byte*)_malloca(m_VertsTraceCnt);
    if(!vflags) ERROR_S(L"Not enough memory for vflags after _malloca() call in CMatrixMapGroup::Pick function!");
    ...
    *(((dword*)&adir) + 2) = *(((dword*)&_dir) + 2) & 0x7FFFFFFF; // adir.z = abs(pd._dir.z);
    ...
    for(; trb < tre; trb += 3)
    {
        ...
        {
            if(IntersectTriangle(_orig, _dir, *(m_VertsTrace + *(trb + 0)), *(m_VertsTrace + *(trb + 1)), *(m_VertsTrace + *(trb + 2)), kt, ku, kv) && kt < okt && kt >= 0.0f)
            {
                okt = kt;
                rv = true;
            }
