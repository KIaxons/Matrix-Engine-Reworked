...
{
#ifdef _DEBUG
    if(m_Current == IDFVF_CNT) ERROR_S(L"Error #2 in instant draw");
#endif
...
void CInstDraw::ActualDraw()
{
    ...
    for(int i = 0; i < cvb->sets_cnt; ++i)
    {
        ...
        g_D3DD->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, cvb->disp + disp, 0, cvb->sets[i].accumcnt, 0, cvb->sets[i].accumcnt / 2);
