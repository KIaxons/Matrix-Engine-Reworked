...
SWeaponRepairData* SWeaponRepairData::Allocate(ESpriteTextureSort sprite_spot, ESpriteTextureSort sprite_rect, int dev_start_id, int dev_end_id)
{
    ...
    r->m_Sprites = CSpriteSequence(TRACE_PARAM_CALL D3DXVECTOR3(-1000, -1000, -1000), D3DXVECTOR3(-1000, -1000, -1000), 10, 0xFFFFFFFF, CMatrixEffect::GetSingleBrightSpriteTex(sprite_rect));
...
void CMatrixRobotAI::LogicTact(int ms)
{
    ...
    do_animation:;
        ...
        if(fabs(m_Speed) <= 0.01f) //Эта проверка постоянно сбрасывает всем роботам анимацию на STAY, если те не двигаются и не вращаются на месте
        {
            if(!FLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING))
            {
    ...
    //Маркер подрыва выставляется из DoAnimation(), что немного говнокодит ситуацию, но хуле нам :{
    if(GetBombStatus() == BS_TIME_TO_EXPLODE)
    {
        SetBombStatus(BS_EXPLODING); //Чтобы не срабатывало отсюда повторно
        DetonateTheBomb();
    }

    //Срабатывает мгновенное уничтожение робота, если он должен умереть, но не был помечен маркером Death in progress (видимо)
    if(IsMustDie() && m_CurrentState != ROBOT_DIP && GetBombStatus() != BS_COUNTDOWN_STARTED)
    {
    ...
    if(IsAblaze())
    {
        ...
        if(m_Ablaze.effect != nullptr)
        {
            ...
            while(g_MatrixMap->GetTime() > m_NextTimeAblaze)
            {
                ...
                do
                {
                    pos.x = m_Core->m_Matrix._41 + (float)FSRND(m_Core->m_Radius);
                    pos.y = m_Core->m_Matrix._42 + (float)FSRND(m_Core->m_Radius);
                    pos.z = m_Core->m_Matrix._43 + (float)FRND(m_Core->m_Radius * 2);
    ...
    if(m_CurrentState == ROBOT_FALLING)
    {
        ...
        if(m_Core->m_Matrix._43 < z)
        {
            ...
            m_CurrentState = ROBOT_SUCCESSFULLY_BUILD; //Маркер о готовности робота к получению дальнейших приказов
            m_KeelWaterCount = 0.0f;

            m_FallingSpeed = 0.0f;

            CMatrixEffect::CreateDust(nullptr, *(D3DXVECTOR2*)&GetGeoCenter(), D3DXVECTOR2(0.0f, 0.0f), 3000.0f);
            CMatrixEffect::CreateDust(nullptr, *(D3DXVECTOR2*)&GetGeoCenter(), D3DXVECTOR2(0.0f, 0.0f), 3000.0f);
            CMatrixEffect::CreateDust(nullptr, *(D3DXVECTOR2*)&GetGeoCenter(), D3DXVECTOR2(0.0f, 0.0f), 3000.0f);
            CMatrixEffect::CreateDust(nullptr, *(D3DXVECTOR2*)&GetGeoCenter(), D3DXVECTOR2(0.0f, 0.0f), 3000.0f);
            JoinToGroup();
    ...
    if(m_CurrentState == ROBOT_CARRYING)
    {
        ...
        float mul = 1.0f - float(pow(CARRYING_SPEED, double(ms)));
        ...
        *(D3DXVECTOR3*)&m_Core->m_Matrix._41 += move;
        ...
        float da = mul * mul * float(AngleDist(m_CargoFlyer->GetDirectionAngle(), m_CargoFlyer->GetCarryData()->m_RobotAngle));
        ...
        *((D3DXVECTOR2*)&m_ChassisForward) = RotatePoint(*(D3DXVECTOR2*)&m_CargoFlyer->GetCarryData()->m_RobotForward, m_CargoFlyer->GetCarryData()->m_RobotAngle);
        ...
        if(CalcBounds(vmin, vmax)) ERROR_S(L"CMatrixRobotAI::LogicTact Error!");
    ...
    float mul = 1.0f - float(pow(0.996, double(ms)));
    D3DXVECTOR3 up;
    ...
    *(D3DXVECTOR3*)&m_Core->m_Matrix._31 = LERPVECTOR(mul, *(D3DXVECTOR3*)&m_Core->m_Matrix._31, up);
    ...
    if(m_CurrentState != ROBOT_IN_SPAWN)
    {
    ...
    if(m_CurrentState == ROBOT_IN_SPAWN)
    {
        if(GetBase())
        {
            ...
            {
                m_CurrentState = ROBOT_BASE_MOVEOUT;
    ...
    else if(m_CurrentState == ROBOT_BASE_MOVEOUT)
    {
        ...
        if(GetBase())
        {
            ...
            if(D3DXVec2LengthSq(&dist) >= BASE_DIST * BASE_DIST)
            {
                ...
                m_CurrentState = ROBOT_SUCCESSFULLY_BUILD;
    ...
    if(m_CurrentState != ROBOT_SUCCESSFULLY_BUILD)
    {
        RChange(MR_Matrix | MR_ShadowProjGeom | MR_ShadowProjTex | MR_ShadowStencil);
        if(m_CurrentState != ROBOT_CAPTURING_BASE) goto do_animation;
    }
    ...
    if(m_Side == PLAYER_SIDE && IsInManualControl() && m_CurrentState == ROBOT_SUCCESSFULLY_BUILD)
    {
    ...
    //Выполняем наведение корпуса робота на заданную ему огневую цель
    if(!IsInManualControl())
    {
        CMatrixMapStatic* cur_target = m_Environment.m_Target;
        if(!cur_target) goto no_target;

        if(cur_target->IsRobotAlive()) //Если целью робота выбран другой робот
        {
            if(cur_target->AsRobot()->m_CurrentState != ROBOT_SUCCESSFULLY_BUILD) goto no_target;
        
            bool capturing = FindOrderLikeThat(ROT_CAPTURE_BUILDING);

            CMatrixRobotAI* Enemy = cur_target->AsRobot();
            D3DXVECTOR2 napr(0.0f, 0.0f);
            D3DXVECTOR3 enemy_pos(0.0f, 0.0f, 0.0f);
            enemy_pos = D3DXVECTOR3(Enemy->m_PosX, Enemy->m_PosY, 0.0f);
            ...
            //SetHullTargetDirection(D3DXVECTOR3(Enemy->m_PosX, Enemy->m_PosY, 0.0f));
            ...
            {
                RotateRobotChassis(D3DXVECTOR3(Enemy->m_PosX, Enemy->m_PosY, 0.0f));
                SetHullTargetDirection(D3DXVECTOR3(m_PosX + m_ChassisForward.x, m_PosY + m_ChassisForward.y, 0.0f));
            }
        ...
        else if(cur_target->IsBuildingAlive()) //Если целью робота выбрано здание
        {
            int order_capture = FindOrderLikeThat(ROT_CAPTURE_BUILDING);
            if(order_capture)
            {
            ...
            else if(!m_CollAvoid.x && !m_CollAvoid.y)
            {
                RotateRobotChassis(enemy_pos);
                SetHullTargetDirection(D3DXVECTOR3(m_PosX + m_ChassisForward.x, m_PosY + m_ChassisForward.y, 0.0f));
            }
        }
        //Если цели у робота нет, либо он ещё только выезжает с родной базы, сбрасываем положение корпуса к положению шасси
        else
        {
        no_target:;

            //Здесь сбрасываем положение корпуса к положению шасси, если у робота нет огневой цели, либо он не должен её атаковать
            if(!IsInManualControl())
            {
                StopFire();
    ...
    //ORDERS PROCESSING////////////////////////////////////////////////////
    if(IsInManualControl()/* && !(GetAsyncKeyState(VK_RBUTTON) & 0x8000) == 0x8000*/)
    {
        ...
        if(is_strafing >= 0 && (GetAsyncKeyState(g_Config.m_KeyActions[KA_ROBOT_STRAFE_LEFT]) & 0x8000) == 0x8000 || (GetAsyncKeyState(g_Config.m_KeyActions[KA_ROBOT_STRAFE_LEFT_ALT]) & 0x8000) == 0x8000)
        {
            ...
            else
            {
                ...
                SETFLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING); //Маркер будет сброшен в методе RotateRobotChassis() автоматически
        ...
        if(!is_strafing)
        {
            RESETFLAG(m_ObjectFlags, ROBOT_FLAG_STRAFING);

            if(m_ChassisCamForward != m_ChassisForward)
            {
                SETFLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING);
                if(RotateRobotChassis(m_ChassisCamForward + D3DXVECTOR3(m_PosX, m_PosY, 0.0f), ROTATION_TO_CAM_POSITION))
                {
                    //Я не ебу почему, но почему-то после полного цикла разворота после стрейфа, анимация разворота не выключается
                    //Выставление анимации STAY отсюда я расцениваю как костыль, который также не ебу почему, но работает
                    //В общем, если будете дальше ковырять, то желаю удачи - я всё.
                    SwitchAnimation(ANIMATION_STAY);
                }
                else is_rotating = true;
            }
        ...
        if((GetAsyncKeyState(g_Config.m_KeyActions[KA_ROBOT_ROTATE_LEFT]) & 0x8000) == 0x8000 || (GetAsyncKeyState(g_Config.m_KeyActions[KA_ROBOT_ROTATE_LEFT_ALT]) & 0x8000) == 0x8000)
        {
            ...
            if(!is_strafing) RotateRobotChassis(dest + D3DXVECTOR3(m_PosX, m_PosY, 0.0f), CONNECTED_CAM_ROTATION, nullptr, rotate_anim);
            else RotateRobotChassis(dest + D3DXVECTOR3(m_PosX, m_PosY, 0.0f), SIMULTANEOUS_CAM_ROTATION, nullptr, rotate_anim);
            SETFLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING);
            is_rotating = true;
        }
        ...
        //Поскольку до тех пор, пока зажата кнопка разворота, целевое направление шасси не будет достигнуто никогда, сбрасываем маркер разворота отсюда
        if(!is_rotating) RESETFLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING);
    ...
    */
    ...
    while(cnt < m_OrdersInPool)
    {
        ...
        switch(m_OrdersList[0].GetOrderType())
        {
            case ROT_FIRE:
            {
                ...
                if(IsInManualControl())
                {
                    ...
                    //else m_WeaponDir.z = g_MatrixMap->m_TraceStopPos.z;
                }
                else
                {
                    /*
                ...
                {
                    if(m_RobotWeapons[nC].m_On && m_RobotWeapons[nC].IsEffectPresent())
                    {
            ...
            case ROT_CAPTURE_BUILDING:
            {
                ...
                //Робот добрался до центра круга захвата завода, либо лифта базы, разворачиваем его
                ...
                else if(ord_phase == ROP_CAPTURING)
                {
                    ...
                    if(building->IsBase())
                    {
                        ...
                        {
                            m_CurrentState = ROBOT_CAPTURING_BASE;
                            building->CloseAssemblyChamber();
                        }
                        else if(building->m_State == BASE_CLOSED)
                        {
                            ...
                            building->m_BuildingQueue.ClearStack();
            ...
            case ROT_MOVE_TO_BACK:
            {
                if(IsInManualControl())
                {
                    byte move_direction = 0;
                    if(((GetAsyncKeyState(g_Config.m_KeyActions[KA_UNIT_FORWARD]) & 0x8000) == 0x8000) || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_UNIT_FORWARD_ALT]) & 0x8000) == 0x8000)) move_direction += 1;
                    if(((GetAsyncKeyState(g_Config.m_KeyActions[KA_UNIT_BACKWARD]) & 0x8000) == 0x8000) || ((GetAsyncKeyState(g_Config.m_KeyActions[KA_UNIT_BACKWARD_ALT]) & 0x8000) == 0x8000)) move_direction += 2;
            ...
            case ROT_MOVE_RETURN:
            {
                if(!FindOrderLikeThat(ROT_MOVE_TO))
                {
                    ...
                    if(g_MatrixMap->PlaceIsEmpty(m_Module[0].m_Kind - 1, 4, int(f1), int(f2), this))
                    {
                        RemoveOrderFromTop();
                        MoveTo(int(f1), int(f2));
                        continue;
                    }
                }
                break;
        ...
        //Process next order
        ProcessOrdersList(); //Первый приказ в списке удаляется, остальной список приказов сдвигается на один пункт вверх
        ++cnt;
    }

    //{
    //    if(IsInManualControl() && !m_Speed)
    //    {
...

    {
        for(int nC = 0; nC < m_WeaponsCount; ++nC)
        {
            if(m_RobotWeapons[nC].IsEffectPresent() && (m_RobotWeapons[nC].m_On || IsInManualControl()))
            {
                //m_WeaponDir.x = g_MatrixMap->m_TraceStopPos.x;
                //m_WeaponDir.y = g_MatrixMap->m_TraceStopPos.y;
                ...
                if(m_RobotWeapons[nC].m_Heat + m_RobotWeapons[nC].m_HeatingSpeed > WEAPON_MAX_HEAT)
                {
                    m_RobotWeapons[nC].FireEnd();
                    continue;
                }

                float fire_dist = m_RobotWeapons[nC].GetWeaponDist();
                D3DXVECTOR3 vPos(0, 0, 0), vDir(0, 0, 0);
                vDir = m_WeaponDir;

                D3DXMATRIX m = (*m_RobotWeapons[nC].m_Module->m_Graph->GetMatrixById(1)) * m_RobotWeapons[nC].m_Module->m_Matrix;
                ...
                if(prim_weap_type == EFFECT_MORTAR)
                {
                    ...
                    {
                        //FIREINDAHOLE:
                        m_RobotWeapons[nC].m_Heat = min(m_RobotWeapons[nC].m_Heat + m_RobotWeapons[nC].m_HeatingSpeed, WEAPON_MAX_HEAT);

                        if(m_RobotWeapons[nC].m_Module->m_Graph->SetAnimByNameNoBegin(ANIMATION_NAME_FIRELOOP))
                        {
                            // not looped
                ...
                //g_MatrixMap->m_DI.ShowScreenText(L"d", CWStr(fire_dist));
                //g_MatrixMap->m_DI.ShowScreenText(L"d1", CWStr(D3DXVec3Length(&vDir)));
                //g_MatrixMap->m_DI.ShowScreenText(L"d2", CWStr(D3DXVec3Length(&D3DXVECTOR3(vPosNorm.x, vPosNorm.y, vDir.z))));
                ...
                if(angle > BARREL_TO_SHOT_ANGLE && !g_Config.m_WeaponsConsts[m_RobotWeapons[nC].GetWeaponNum()].is_repairer)
                {
                    m_RobotWeapons[nC].FireEnd();
                    continue;
                }
    ...
    {
        if(m_RobotWeapons[cnt].IsEffectPresent())
        {
            //Пока оружие нагрето, постоянно накручиваем задержку до следующего охлаждения, иначе сбрасываем её
            if(m_RobotWeapons[cnt].m_Heat)
            {
                m_RobotWeapons[cnt].m_CoolingDelay += ms;

                int period = g_Config.m_RobotWeaponsConsts[m_RobotWeapons[cnt].m_Module->m_Kind].cooling_delay;
                ...
                while(m_RobotWeapons[cnt].m_Heat && m_RobotWeapons[cnt].m_CoolingDelay >= period)
                {
                    m_RobotWeapons[cnt].m_Heat = max(m_RobotWeapons[cnt].m_Heat - m_RobotWeapons[cnt].m_CoolingSpeed, 0);
                    m_RobotWeapons[cnt].m_CoolingDelay -= period;
                }
            }
            else m_RobotWeapons[cnt].m_CoolingDelay = 0;
        }
    }
...
void CMatrixRobotAI::MoveToRndBuilding()
{
    ...
    while(ms)
    {
        ...
        else if(ms != this && ms->GetObjectType() == OBJECT_TYPE_ROBOT_AI && (g_MatrixMap->Rnd(0, 10) <= 5))
        {
        float x = (ms->AsRobot()->m_PosX);
        float y = (ms->AsRobot()->m_PosY + 30);

        if(ms->AsRobot()->m_CurrentState == ROBOT_SUCCESSFULLY_BUILD && ((x - m_PosX) * (x - m_PosX) + (y - m_PosY) * (y - m_PosY)) > 100 * 100)
        {
            int mx = int(x / GLOBAL_SCALE);
            int my = int(y / GLOBAL_SCALE);
...
{
    ASSERT(this);
    if(m_CurrentState == ROBOT_DIP) return true;
    ...
    if(g_Config.m_WeaponsConsts[weap].extra_effects.size())
    {
        for(int i = 0; i < (int)g_Config.m_WeaponsConsts[weap].extra_effects.size(); ++i)
        {
            ...
            else if(effect_type == SECONDARY_EFFECT_SHORTED_OUT)
            {
                ...
                float dur_per_hit = float(effect->duration_per_hit);
                float max_dur = float(effect->max_duration);
    ...
    inst_death:
        ...
        if(IsInPosition()) g_MatrixMap->ShowPortraits();

        if(GetBombStatus() == BS_HAVE_BOMB)
        {
            //if(GetSide() == PLAYER_SIDE)
            //{
                //Для активации по правому клику мыши по значку подрыва
                //if(AutoBoomSet() && !IsShorted()) BeginBombCountdown();
            //}
            //else
            //{
            if(GetSide() != PLAYER_SIDE || AutoBoomSet())
            {
                float danger = 0.0f; //Автоподрыв бомбы ИИ бота происходит только в случае, если в зоне поражения вокруг находятся преимущественно враги
                CMatrixMapStatic* ms = CMatrixMapStatic::GetFirstLogic();
                for(; ms; ms = ms->GetNextLogic())
                {
                    if(ms == this) continue;
                    ...
                    if(ms->IsRobotAlive())
                    {
                        ...
                        if((POW2(robot->m_PosX - m_PosX) + POW2(robot->m_PosY - m_PosY)) < POW2(m_BombRange * 1.0f))
                        {
                ...
                if(danger > 0.0f) BeginBombCountdown();
            }
        }

        ResetMustDie(); //Чтобы исключить лишнюю проверку флага MUST_DIE, т.к. робот всё равно уже умирает

        //Ну, или не совсем... Ждём, пока завершится анимация активации бомбы
        if(GetBombStatus() == BS_COUNTDOWN_STARTED) return false;
        else if(GetBombStatus() == BS_TIME_TO_EXPLODE) //Дублирую здесь код подрыва, поскольку иначе робот не всегда успевает ещё раз перед смертью прокрутить логику
        {
            SetBombStatus(BS_EXPLODING); //Чтобы не срабатывало отсюда повторно
            DetonateTheBomb();
        }
        ...
        CMatrixEffect::CreateLandscapeSpot(nullptr, *(D3DXVECTOR2*)&GetGeoCenter(), float(FSRND(M_PI)), 4.0f + float(FRND(2)), SPOT_VORONKA);
        ...
        {
            for(dword i = 0; i < g_Config.m_RobotChassisConsts[m_Module[0].m_Kind].jet_stream.size(); ++i)
            {
        ...
        if(IsManualControlLocked())
        {
            if(GetBase())
            {
                if(m_CurrentState == ROBOT_BASE_MOVEOUT || m_CurrentState == ROBOT_IN_SPAWN)
                {
        ...
        else
        {
            ...
            else m_Module[0].m_Velocity = D3DXVECTOR3(0.0f, 0.0f, 0.1f);
        }

        m_Module[0].Smoke().effect = nullptr;
        CMatrixEffect::CreateSmoke(&m_Module[0].Smoke(), m_Module[0].m_Pos, m_Module[0].m_TTL + 100000, 1600, 80, 0xCF303030, false, 1.0f / 30.0f);
        ...
        for(int i = 1; i < m_ModulesCount; ++i)
        {
            ...
            m_Module[i].m_dp = (float)FSRND(0.005);
            m_Module[i].m_dy = (float)FSRND(0.005);
            m_Module[i].m_dr = (float)FSRND(0.005);

            if(onair) m_Module[i].m_Velocity = D3DXVECTOR3((float)FSRND(0.08), (float)FSRND(0.08), (float)FSRND(0.1));
            else m_Module[i].m_Velocity = D3DXVECTOR3((float)FSRND(0.08), (float)FSRND(0.08), 0.1f);

            m_Module[i].m_TTL = float(FRND(3000.0) + 2000.0);
            ...
            CMatrixEffect::CreateSmoke(&m_Module[i].Smoke(), m_Module[i].m_Pos, m_Module[i].m_TTL + 100000, 1600, 80, 0xCF303030, false, 1.0f / 30.0f);
        }
...
void CMatrixRobotAI::RobotSpawn(CMatrixBuilding* pBase)
{
    ...
    m_CurrentState = ROBOT_IN_SPAWN;
    pBase->OpenAssemblyChamber();
}
...
void CMatrixRobotAI::SetHullTargetDirection(const D3DXVECTOR3& dest)
{
    ...
    if(IsInManualControl() && D3DXVec3LengthSq(&destDir) < MIN_ROT_DIST * MIN_ROT_DIST)
    {
...
bool CMatrixRobotAI::RotateRobotChassis(const D3DXVECTOR3& dest, byte rotate_cam_link, float* rotate_angle, bool rotate_anim)
{
    RChange(MR_Matrix | MR_ShadowProjTex | MR_ShadowStencil);

    D3DXVECTOR3 dest_dir_norm = { 0.0f, 0.0f, 0.0f };
    ...
    D3DXVECTOR3 vec = { forward.y, -forward.x, 0.0f };
    ...
    if(angle1 <= rot_speed)
    {
        ...
        RESETFLAG(m_ObjectFlags, ROBOT_FLAG_ROTATING);
        //SwitchAnimation(ANIMATION_STAY); //Это здесь не к чему, т.к. в момент разворота робот может также двигаться вперёд или назад

        return true;
    }

    //Если отклонение от требуемого угла незначительно, то также считаем разворот завершённым
    if(fabs(angle1 - rot_speed) < 0.001)
    {
        forward = dest_dir_norm;
    }
    //Если скорости разворота шасси оказалось недостаточно для достижения нужного положения, вращение необходимо будет продолжить повторным вызовом
    else if(rot_dir > 0) //Поворот налево
    {
        D3DXMATRIX rotMat;
        D3DXMatrixRotationZ(&rotMat, -rot_speed);
        D3DXVec3TransformCoord(&forward, &forward, &rotMat);

        if(rotate_anim) SwitchAnimation(ANIMATION_ROTATE_LEFT);
    }
    else //Поворот направо
    {
        D3DXMATRIX rotMat;
        ...
        if(rotate_anim) SwitchAnimation(ANIMATION_ROTATE_RIGHT);
    }
...
)
{
    ...
    rotate = true;
    if(cam_rotation && m_CurrentState != ROBOT_BASE_MOVEOUT && moving_direction == UNIT_MOVING_FORWARD)
    {
...
//    if(m_CurrentState == ROBOT_BASE_MOVEOUT){
...
)
{
    ...
    m_Cols = 0;
    D3DXVECTOR3 r = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3 o = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3 result_coll = { 0.0f, 0.0f, 0.0f };
    D3DXVECTOR3 tmp_vel = { 0.0f, 0.0f, 0.0f };
    ...
    if(IsInManualControl())
    {
        g_MatrixMap->m_DI.ShowScreenText(L"pos_x", CWStr(m_PosX));
        g_MatrixMap->m_DI.ShowScreenText(L"pos_y", CWStr(m_PosY));
        g_MatrixMap->m_DI.ShowScreenText(L"geo_x", CWStr(m_Core->m_GeoCenter.x));
        g_MatrixMap->m_DI.ShowScreenText(L"geo_y", CWStr(m_Core->m_GeoCenter.y));
        g_MatrixMap->m_DI.ShowScreenText(L"m_Velocity", CWStr(D3DXVec3Length(&m_Velocity)));
        //g_MatrixMap->m_DI.ShowScreenText(L"genetic_mutated_velocity", CWStr(D3DXVec3Length(&genetic_mutated_velocity)));
        g_MatrixMap->m_DI.ShowScreenText(L"result_coll", CWStr(D3DXVec3Length(&result_coll)));
    }
...
void CMatrixRobotAI::LowLevelDecelerate(
    int ms,
    bool robot_coll,
    bool obst_coll
)
...
void CMatrixRobotAI::Decelerate()
{
    ...
    m_Speed = D3DXVec3Length(&m_Velocity);
    D3DXVECTOR3 vvv(0.0f, 0.0f, 0.0f);
    D3DXVec3Normalize(&vvv, &m_Velocity);
    if(!IS_ZERO_VECTOR(vvv))
    {
        m_ChassisForward = vvv;
...
D3DXVECTOR3 CMatrixRobotAI::SphereRobotToAABBObstacleCollision(D3DXVECTOR3& corr, const D3DXVECTOR3& vel)
{
    ...
    D3DXVECTOR3 vCorrTotal(0.0f, 0.0f, 0.0f);

    robot_pos.x = m_PosX + corr.x + vel.x;
    robot_pos.y = m_PosY + corr.y + vel.y;

    D3DXVECTOR3 oldpos = D3DXVECTOR3(robot_pos.x, robot_pos.y, 0.0f);
    ...
    //if(IsInManualControl())
    //{
    //    g_MatrixMap->m_DI.ShowScreenText(L"col_cnt", CWStr(col_cnt + m_Cols));
    //}
    //#endif
...
static bool CollisionCallback(const D3DXVECTOR3& fpos, CMatrixMapStatic* pObject, dword user)
{
    ...
    if (pObject->IsRobot() && !pObject->AsRobot()->IsAutomaticMode())
    {
        ...
        if (dist < (COLLIDE_BOT_R + COLLIDE_BOT_R)/**(COLLIDE_BOT_R + COLLIDE_BOT_R)*/)
        {
...
#ifdef _SUB_BUG
                g_MatrixMap->m_DI.ShowScreenText(L"ColsWeight", CWStr().Format(L"<i>    <i>", data->robot->GetColsWeight(), data->robot->GetColsWeight2()).Get(), 1000);
#endif
            ...
            /*            if(data->robot->m_CurrentState == ROBOT_BASE_MOVEOUT || data->robot->FindOrderLikeThat(ROT_MOVE_TO, ROT_GETING_LOST)){
    ...
    else if (pObject->GetObjectType() == OBJECT_TYPE_TURRET)
    {
        ...
        //                g_MatrixMap->m_DI.ShowScreenText(L"CAN_DIST", CWStr(dist));
        //#endif
        ...
        if (dist < (COLLIDE_BOT_R + CANNON_COLLIDE_R)/**(COLLIDE_BOT_R + CANNON_COLLIDE_R)*/)
        {
            ...
            //#ifdef _SUB_BUG
            //                    g_MatrixMap->m_DI.ShowScreenText(L"correction", CWStr(correction));
            //#endif
            D3DXVec2Normalize(&vDist, &vDist);
            vDist *= correction;
            data->result += D3DXVECTOR3(vDist.x, vDist.y, 0);
...
void robot_weapon_hit(CMatrixMapStatic* hit, const D3DXVECTOR3& pos, dword user, dword flags)
{
    ...
    while(obj)
    {
        if(user == dword(obj))
        {
            if(obj->AsRobot()->m_CurrentState != ROBOT_DIP) obj->AsRobot()->HitTo(hit, pos);
            return;
        }
        obj = obj->GetNextLogic();
    }
...
void CMatrixRobotAI::HitTo(CMatrixMapStatic* hit, const D3DXVECTOR3& pos)
{
    if(IS_TRACE_STOP_OBJECT(hit))
    {
        ...
        */
        ...
        if(hit->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
        {
...
void CMatrixRobotAI::RobotWeaponInit()
{
    ...
    for(int i = m_ModulesCount - 1; i >= 0; --i) //Расположение орудий в списке модулей обратное, так что делаем перебор с инверсией
    {
        if(m_Module[i].m_Type == MRT_WEAPON)
        {
            ...
            m_Module[i].m_Weapon = &m_RobotWeapons[Weap];
            m_RobotWeapons[Weap].m_Module = &m_Module[i];

            m_RobotWeapons[Weap].CreateEffect((dword)this, &robot_weapon_hit, g_Config.m_RobotWeaponsConsts[kind].weapon_type);

            m_RobotWeapons[Weap].m_HeatingSpeed = g_Config.m_RobotWeaponsConsts[kind].heating_speed;
            m_RobotWeapons[Weap].m_CoolingSpeed = g_Config.m_RobotWeaponsConsts[kind].cooling_speed;
            //m_RobotWeapons[Weap].m_CoolingDelay = 0; //Инициализируется в конструкторе
            //m_RobotWeapons[Weap].m_Heat = 0;         //Инициализируется в конструкторе

            m_RobotWeapons[Weap].SetOwner(this);
            m_RobotWeapons[Weap].m_On = true; //По умолчанию выключено
...
{
    m_HaveRepair = NOT_REPAIRER;
    bool have_normal_weapon = false;
    ...
    for(int nC = 0; nC <= m_ModulesCount; ++nC)
    {
        ...
        switch(type)
        {
            ...
            case MRT_WEAPON:
            {
                if(!HaveBomb())
                {
                    m_BombStatus = g_Config.m_RobotWeaponsConsts[kind].is_module_bomb ? BS_HAVE_BOMB : m_BombStatus;
                    if(HaveBomb()) m_BombRange = m_BombRange < m_Module[nC].m_Weapon->GetWeaponDist() ? m_Module[nC].m_Weapon->GetWeaponDist() : m_BombRange;
                }
                if(m_HaveRepair == NOT_REPAIRER && g_Config.m_RobotWeaponsConsts[kind].is_module_repairer) m_HaveRepair = PARTIAL_REPAIRER;
                if(!have_normal_weapon) have_normal_weapon = ( g_Config.m_RobotWeaponsConsts[kind].is_module_bomb     == false &&
                                                               g_Config.m_RobotWeaponsConsts[kind].is_module_repairer == false );
                break;
            }
        }
    }

    //Если все модули робота - ремонтные (не считая бомбы), то такой робот считается полным ремонтником, отвлекаться на боевые задачи ему будет незачем
    if(m_HaveRepair == PARTIAL_REPAIRER && !have_normal_weapon) m_HaveRepair = FULL_REPAIRER;
    ...
    {
        m_RobotWeapons[cnt].ModifyCoolDown(cool_down);
        ...
        if(g_Config.m_WeaponsConsts[m_RobotWeapons[cnt].GetWeaponNum()].is_repairer)
        {
            r_min = min(r_min, m_RobotWeapons[cnt].GetWeaponDist());
            ++cnt;
            continue;
        }

        if(m_RobotWeapons[cnt].GetWeaponDist() < tmp_min)
        {
            tmp_min = m_RobotWeapons[cnt].GetWeaponDist();
        }

        if(m_RobotWeapons[cnt].GetWeaponDist() > tmp_max)
        {
            tmp_max = m_RobotWeapons[cnt].GetWeaponDist();
        }
...
void CMatrixRobotAI::GatherInfo(int type)
{
    ...
    */
    ...
    if(!type)
    {
        ...
        while(obj)
        {
            if(obj->IsRobotAlive() && obj != this && obj->GetSide() != m_Side)
            {
                ...
                if(dist_enemy <= POW2(max(robot->m_MaxFireDist, m_MaxFireDist) * 1.1) /*(D3DXVec3LengthSq(&enemy_napr) <= m_MinFireDist*m_MinFireDist) || (D3DXVec3LengthSq(&enemy_napr) <= m_MaxFireDist*m_MaxFireDist && angle_rad <= ROBOT_FOV)*/ && robot->m_CurrentState != ROBOT_DIP)
                {
    ...
    else if(type == 1)
    {
        ...
        while(obj)
        {
            if(this != obj && obj->GetObjectType() == OBJECT_TYPE_ROBOT_AI && obj->GetSide() == GetSide())
            {
                //Если в одной группе
    ...
    /*    CMatrixGroupObject* gobj = my_group->m_FirstObject;
        ...
        while(gobj){
            if(gobj->ReturnObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI){
                CMatrixRobotAI* bot = (CMatrixRobotAI*)gobj->ReturnObject();
...
void CMatrixRobotAI::StopMoving()
{
    for(int cnt = 0; cnt < m_OrdersInPool; ++cnt)
    {
...
void CMatrixRobotAI::BeginBombCountdown()
{
    if(GetBombStatus() >= BS_COUNTDOWN_STARTED) return;
    if(IsShorted()) return;
    ...
    for(int i = 0; i < m_WeaponsCount; ++i)
    {
        ...
        //Если в модели бомбы имеется анимация детонации
        SetBombStatus(BS_COUNTDOWN_STARTED);
    }

    //Если анимации детонации у бомб на роботе не нашлось
    if(GetBombStatus() != BS_COUNTDOWN_STARTED)
    {
        SetBombStatus(BS_EXPLODING);
        DetonateTheBomb();
    }
}

void CMatrixRobotAI::DetonateTheBomb()
{
    if(IsShorted()) return;
...
//Хз для чего это когда-то использовалось
/*
struct SSeekCaptureMeB
{
    CMatrixBuilding* b = nullptr;
    CMatrixRobotAI* r = nullptr;
    float dist2 = 0.0f;
};
*/
...
void CMatrixRobotAI::BreakAllOrders()
{
    LowLevelStop();
    m_Environment.Reset();
    LowLevelStopFire();
    ...
    for(int cnt = m_OrdersInPool - 1; cnt >= 0; --cnt)
    {
        if(m_OrdersList[cnt].GetOrderType() == ROT_CAPTURE_BUILDING)
        {
            ...
            if(building && building->IsCaptured() && building->m_Capturer == this)
            {
                ...
                if(IsManualControlLocked())
                {
                    if(building->IsBase() && building->m_State != BASE_CLOSED) building->CloseAssemblyChamber();
                }
                /*
                else
                {
...
void CMatrixRobotAI::BreakAllMoveOrders()
{
    CMatrixMapStatic* cur_target = m_Environment.m_Target; //Пересохраняем текущую огневую цель робота, поскольку при отмене приказов она будет сброшена автоматически
    ...
    for(int cnt = m_OrdersInPool - 1; cnt >= 0; --cnt)
    {
        OrderType order = m_OrdersList[cnt].GetOrderType();
        if(order == ROT_CAPTURE_BUILDING)
        {
            CMatrixBuilding* building = (CMatrixBuilding*)m_OrdersList[cnt].GetStatic();
            if(building && building->IsCaptured() && building->m_Capturer == this)
            {
        ...
        else if(order != ROT_FIRE && order != ROT_STOP_FIRE) RemoveOrder(cnt);
    }

    m_Environment.m_Target = cur_target;
}
...
void CMatrixRobotAI::LowLevelStopFire()
{
    for(int nC = 0; nC < m_WeaponsCount; ++nC)
    {
        if(m_RobotWeapons[nC].IsEffectPresent())
        {
            m_RobotWeapons[nC].FireEnd();
        }
    }
...
void CMatrixRobotAI::LowLevelStop()
{
    m_Speed = 0.0f;
...
bool CMatrixRobotAI::FindWeapon(int type)
{
    for(int nC = 0; nC < m_WeaponsCount; ++nC)
    {
        if(m_RobotWeapons[nC].IsEffectPresent() && m_RobotWeapons[nC].GetWeaponNum() == type) return true;
    }
    return false;
}
bool CMatrixRobotAI::FindRepairWeapon()
{
    for(int nC = 0; nC < m_WeaponsCount; ++nC)
    {
        if(m_RobotWeapons[nC].IsEffectPresent() && g_Config.m_WeaponsConsts[m_RobotWeapons[nC].GetWeaponNum()].is_repairer) return true;
    }
    return false;
}
bool CMatrixRobotAI::FindBombWeapon()
{
    for(int nC = 0; nC < m_WeaponsCount; ++nC)
    {
        if(m_RobotWeapons[nC].IsEffectPresent() && g_Config.m_WeaponsConsts[m_RobotWeapons[nC].GetWeaponNum()].is_bomb) return true;
    }
    return false;
}

void CMatrixRobotAI::ReleaseMe()
{
    UnSelect();
    BreakAllOrders();
    ClearCaptureCandidates();
    ...
    if(m_ZonePath)
    {
        HFree(m_ZonePath, Base::g_MatrixHeap);
        m_ZonePath = nullptr;
    }
...
bool CMatrixRobotAI::IsHomingMissilesEquipped()
{
    for(int i = 0; i < m_WeaponsCount; ++i)
    {
        if(m_RobotWeapons[i].IsEffectPresent())
        {
...
{
    for(int i = 0; i < m_WeaponsCount; ++i) m_RobotWeapons[i].SetArcadeCoefficient();
}

void CMatrixRobotAI::SetWeaponToDefaultCoeff()
{
    for(int i = 0; i < m_WeaponsCount; ++i) m_RobotWeapons[i].SetDefaultCoefficient();
}
