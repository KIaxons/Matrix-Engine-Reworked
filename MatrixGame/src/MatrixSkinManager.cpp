...
SMatrixSkin** CSkinManager::m_Skins[GSP_COUNT];
...
const SSkin* CSkinManager::GetSkin(const wchar* textures, dword gsp)
{
    ...
    */
    ...
    sk.m_Tex = nullptr;
    sk.m_TexGloss = nullptr;
    sk.m_TexMask = nullptr;
    sk.m_TexBack = nullptr;
    sk.m_dtu = 0;
    sk.m_dtv = 0;
    sk.m_tu = 0;
    sk.m_tv = 0;
    ...
    for(int i = 0; i < n; ++i)
    {
        ...
        if(!i)
        {
            ...
            sk.m_Tex->Preload();
            if(!(sk.m_Tex->Flags() & TF_ALPHABLEND))
            {
                if(gsp == GSP_SIDE) gsp = GSP_SIDE_NOALPHA;
            }
        }
        else if(i == 1 && g_Config.m_ObjTexturesGloss)
        {
            if(temp.Equal(L".")) gloss_off = true;
            else sk.m_TexGloss = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, temp.Get());
        }
        else if(i == 2) sk.m_TexBack = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, temp.Get());
        else if(i == 3) sk.m_TexMask = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, temp.Get());
        else if(i == 4)
        {
            sk.m_dtu = temp.GetStrPar(0, L",").GetFloat();
            sk.m_dtv = temp.GetStrPar(1, L",").GetFloat();
        }
    }

    if(!gloss_off && g_Config.m_ObjTexturesGloss && sk.m_TexGloss == nullptr && gspp == GSP_SIDE)
    {
    ...
    {
        if((*(m_Skins[gsp][i])) == sk) return m_Skins[gsp][i];
    }

    ++m_SkinsCount[gsp];
    m_Skins[gsp] = (SMatrixSkin**)HAllocEx(m_Skins[gsp], sizeof(SMatrixSkin*) * m_SkinsCount[gsp], g_MatrixHeap);
    ...
    *ms = sk;
    ...
    if(ms->m_TexGloss)
    {
        if(ms->m_TexMask)
        {
            if(ms->m_TexBack) ms->m_Preload = SkinPreloadGlossMaskBack;
            else  ms->m_Preload = SkinPreloadGlossMask;
        }
        else ms->m_Preload = SkinPreloadGloss;
    }
    else
    {
        if(ms->m_TexMask)
        {
            if(ms->m_TexBack) ms->m_Preload = SkinPreloadMaskBack;
            else ms->m_Preload = SkinPreloadMask;
        }
        else ms->m_Preload = SkinPreload;
    }
...
static void obj_clear5stage(const SSkin *, dword user_param)
{
    ...
    ASSERT_DX(g_D3DD->SetTexture(1, nullptr));
    ASSERT_DX(g_D3DD->SetTexture(2, nullptr));
    ASSERT_DX(g_D3DD->SetTexture(3, nullptr));
    ASSERT_DX(g_D3DD->SetTexture(4, nullptr));
    ASSERT_DX(g_D3DD->SetTexture(5, nullptr));
...
static bool obj_side4(const SSkin *sk, dword user_param, int pass)
{
    ...
    if (pass == 0)
    {
        if (ms->m_TexGloss)
        {
            ...
            return ms->m_TexMask != nullptr; // if mask present, do 2nd pass
...
static bool obj_side4NA(const SSkin *sk, dword, int pass)
{
    ...
    if (pass == 0)
    {
        ...
        } else if (ms->m_TexMask)
        {
            ...
            //g_MatrixMap->m_DI.ShowScreenText(L"tutv", CWStr(ms->m_tu) + L"," + CWStr(ms->m_tv));
...
static bool obj_side3(const SSkin *sk, dword user_param, int pass)
{
    ...
    if (pass == 0)
    {
        ...
        return ms->m_TexGloss != nullptr || ms->m_TexMask != nullptr;

    } else if (pass == 1)
    {
        if (ms->m_TexGloss != nullptr)
        {
            ...
            return ms->m_TexMask != nullptr;
        } else
        if (ms->m_TexMask != nullptr)
        {
            goto mask;
        }
...
static bool obj_ord4(const SSkin *sk, dword user_param, int pass)
{
    ...
    {
        CMatrixMapObject *o = (CMatrixMapObject *)user_param;
        if (o->m_BurnSkinVis == 255) goto burned;
...
void SMatrixSkin::Prepare6Ordinal(void)
{
    ...
    if (m_TexGloss == nullptr && m_TexMask != nullptr)
    {
...
void CSkinManager::Clear(void)
{
    for(int gsp = 0; gsp < GSP_COUNT; ++gsp)
    {
        if(m_Skins[gsp])
        {
            for(int i = 0; i < m_SkinsCount[gsp]; ++i) HFree(m_Skins[gsp][i], g_MatrixHeap);
            HFree(m_Skins[gsp], g_MatrixHeap);
        }
    }
}

void CSkinManager::Tact(float cms)
{
    for(int gsp = 0; gsp < GSP_COUNT; ++gsp)
    {
        for(int i = 0; i < m_SkinsCount[gsp]; ++i)
        {
            ...
            if(ms->m_tu > 1) ms->m_tu -= 1;
            if(ms->m_tv > 1) ms->m_tv -= 1;
            if(ms->m_tu < -1) ms->m_tu += 1;
            if(ms->m_tv < -1) ms->m_tv += 1;
        }
    }
