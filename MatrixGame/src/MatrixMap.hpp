...
#define IS_TRACE_UNIT(o) ((o->GetObjectType() == OBJECT_TYPE_ROBOT_AI)||(o->GetObjectType() == OBJECT_TYPE_FLYER))
...
public:
    ...
    {
        for(dword i = 0; i < m_GatheringPointsList.BuildingsList.size(); ++i)
        {
    ...
    float m_LightMainAngleX = 0.0f;
    float m_LightMainAngleZ = 0.0f;
...
public:
	...
	void IdsClear();
    inline const CWStr& MapName() { return m_Ids[m_IdsCnt - 1]; }
    inline const CWStr& IdsGet(int no) { return m_Ids[no]; }
    inline int IdsGetCount() const { return m_IdsCnt; }
    ...
    inline SMatrixMapUnit* UnitGet(int x, int y) { return m_Module + y * m_Size.x + x; }
    inline SMatrixMapUnit* UnitGetTest(int x, int y) { return (x >= 0 && x < m_Size.x && y >= 0 && y < m_Size.y) ? (m_Module + y * m_Size.x + x) : nullptr; }

    inline SMatrixMapMove* MoveGet(int x, int y)
    {
    ...
    inline SMatrixMapPoint* PointGet(int x, int y)
    {
    ...
    inline CTexture* GetPlayerSideColorTexture() { return m_PlayerSide->m_ColorTexture; };
    void ClearSide();
    ...
    int  PrepareMap(CStorage& stor, const CWStr& map_name);
...
//zakker
    ...
    inline CMatrixMapGroup* GetGroupByIndexTest(int x, int y) { return (x < 0 || y < 0 || x >= m_GroupSize.x || y >= m_GroupSize.x) ? nullptr : m_Group[x + y * m_GroupSize.x]; }
    ...
    inline bool  IsPaused() { return FLAG(m_Flags, MMFLAG_PAUSE); }

    inline void  MouseCam(bool p) { INITFLAG(m_Flags, MMFLAG_MOUSECAM, p); }
    inline bool  IsMouseCam() { return FLAG(m_Flags, MMFLAG_MOUSECAM); }
...
inline float PortionInDiapason(
