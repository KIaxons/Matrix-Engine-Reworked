...
{
    D3DXVECTOR3 stop;
    float len = 10.0f + FRND(5.0f);
    
    /*
    if (TRACE_STOP_NONE == g_MatrixMap->Trace(&stop, D3DXVECTOR3(pos.x, pos.y, WATER_LEVEL), 
                                                    D3DXVECTOR3(pos.x, pos.y, WATER_LEVEL) + 100 * D3DXVECTOR3(dir.x, dir.y, 0), TRACE_LANDSCAPE))
    */
    {
    ...
    if(m_VB_ref == 0)
    {
        m_Tex = (CTextureManaged*)g_Cache->Get(cc_TextureManaged, g_CacheData->BlockPathGet(PAR_SOURCE_WATER)->BlockGet(g_MatrixMap->m_WaterName)->ParGet(PAR_SOURCE_WATER_INSHORE));
        g_Cache->Up(m_Tex);
    }
    ++m_VB_ref;
}
...
{
    CBlockPar* bp = g_CacheData->BlockPathGet(PAR_SOURCE_WATER)->BlockGet(g_MatrixMap->m_WaterName);
...
void CMatrixWater::Tact(int step)
{
    bool skip = true;
