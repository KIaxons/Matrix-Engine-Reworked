...
void CIFaceCounter::MulRes()
{
    ...
    //{
    //    ps->m_ConstructPanel->m_Configs[ps->m_ConstructPanel->m_CurrentConfig].m_TotalPrice.m_Resources[i] = price.m_Resources[i] * m_Counter;
    //}
}
...
void CIFaceCounter::CheckUp()
{
    ...
    {
        if(m_ButtonUp) m_ButtonUp->SetState(IFACE_DISABLED);
        if(m_ButtonDown) m_ButtonUp->SetState(IFACE_DISABLED);
        m_Counter = 0;
    }

    if(!ps->IsEnoughResources(res) || (ps->GetSideRobots() + ps->GetRobotsInQueue() + m_Counter >= ps->GetMaxSideRobots()))
    {
        if(m_ButtonUp) m_ButtonUp->SetState(IFACE_DISABLED);
    }
    ...
    if(ps->m_ActiveObject && ps->m_ActiveObject->IsBuilding() && ps->m_ActiveObject->IsBase())
    {
        ...
        if((base->m_BuildingQueue.GetItemsCnt() + m_Counter + 1) > BUILDING_QUEUE_LIMIT)
        {
            if(m_ButtonUp) m_ButtonUp->SetState(IFACE_DISABLED);
        }
    }
