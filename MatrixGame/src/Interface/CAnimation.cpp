...
void CAnimation::LogicTact(int ms)
{
DTRACE();
    ...
    if(m_TimePass >= m_Period)
    {
        ...
        ++m_CurrentFrame;
        if(m_CurrentFrame > m_Frames - 1) m_CurrentFrame = 0;
    }
}
...
DTRACE();
    ...
    if(m_FramesLoaded == m_Frames) return true;
    ...
    int nC = 0;
	while(nC < MAX_STATES && st->m_StateImages[nC].Set)
    {
		for(int i = 0; i < 4; ++i)
        {
    ...
    for(int i = 0; i < m_FramesLoaded; ++i)
    {
        ...
        for(nC = 0; nC < MAX_STATES; ++nC)
        {
            if(pElement->m_StateImages[nC].Set)
            {
                ...
                pElement->m_StateImages[nC].m_Geom[0].p = D3DXVECTOR4(0 - 0.5f, pElement->m_ySize - 0.5f, 0, 1);
                pElement->m_StateImages[nC].m_Geom[1].p = D3DXVECTOR4(0 - 0.5f, 0 - 0.5f, 0, 1);
                pElement->m_StateImages[nC].m_Geom[2].p = D3DXVECTOR4(pElement->m_xSize - 0.5f, pElement->m_ySize - 0.5f, 0, 1);
                pElement->m_StateImages[nC].m_Geom[3].p = D3DXVECTOR4(pElement->m_xSize - 0.5f, 0 - 0.5f, 0, 1);
            }
        }
