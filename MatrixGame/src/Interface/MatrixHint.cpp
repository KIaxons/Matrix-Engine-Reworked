...
CMatrixHint* CMatrixHint::Build(int border, const CWStr& soundin, const CWStr& soundout, SHintElement* elems, CRect* otstup)
{
    ...
    for(;; ++els)
    {
        ...
        else ERROR_S(L"CMatrixHint::Build Error!");
    }

    struct SBordPart
    {
        CPoint pos = { 0, 0 }; // in texture
        CPoint size = { 0, 0 };
        CPoint size_down = { 0, 0 };
        dword present = 0;
        const wchar* name = L"";
...
static EHintElementModificator Convert(CWStr& bmph, const CWStr& temp, int index)
{
    ...
    else if(bmph == L"CR") return (EHintElementModificator)(HEM_TAB_LARGEST + temp.GetIntPar(index+1, L":"));
    else if(bmph == L"CL") return (EHintElementModificator)(HEM_CENTER_RIGHT_LARGEST + temp.GetIntPar(index + 1, L":"));
    else if(bmph == L"T") return (EHintElementModificator)(temp.GetIntPar(index + 1, L":"));
    else if(bmph == L"COPY") return HEM_COPY;
    else return HEM_BITMAP;
}
...
CMatrixHint* CMatrixHint::Build(const CWStr& str, CBlockPar* repl, const wchar* baserepl)
{
    ...
    CMatrixHint* hint = Build(border, soundin, soundout, elems, otstup ? (&otstup_r) : nullptr);
