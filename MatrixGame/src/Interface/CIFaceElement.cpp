...
{ 
	...
	for(int nC = 0; nC < MAX_STATES; nC++)
    {
		if(pElement->m_StateImages[nC].Set)
        {
            ...
            (v+0)->p.x = 0-0.5f;                 (v+0)->p.y = pElement->m_ySize-0.5f; (v+0)->p.z = 0; (v+0)->p.w = 1; (v+0)->tu = u0; (v+0)->tv = v1;
            (v+1)->p.x = 0-0.5f;                 (v+1)->p.y = 0-0.5f;                 (v+1)->p.z = 0; (v+1)->p.w = 1; (v+1)->tu = u0; (v+1)->tv = v0;
            (v+2)->p.x = pElement->m_xSize-0.5f; (v+2)->p.y = pElement->m_ySize-0.5f; (v+2)->p.z = 0; (v+2)->p.w = 1; (v+2)->tu = u1; (v+2)->tv = v1;
            (v+3)->p.x = pElement->m_xSize-0.5f; (v+3)->p.y = 0-0.5f;                 (v+3)->p.z = 0; (v+3)->p.w = 1; (v+3)->tu = u1; (v+3)->tv = v0;
		}
	}
}
...
void CIFaceElement::Reset()
{
    ...
    {
        SetState(m_DefState);
        //if(m_strName == IF_BASE_HULL_PYLON || m_strName == IF_BASE_CHASSIS_PYLON || m_strName == IF_BASE_HEAD_PYLON || m_strName == IF_BASE_WEAPON_PYLON_1 || m_strName == IF_BASE_WEAPON_PYLON_2 || m_strName == IF_BASE_WEAPON_PYLON_3 || m_strName == IF_BASE_WEAPON_PYLON_4 || m_strName == IF_BASE_WEAPON_PYLON_5 || m_strName == IF_BASE_HEAD_PYLON_EMPTY || m_strName == IF_BASE_WEAPON_PYLON_EMPTY)
        //{
        Action(ON_UN_FOCUS);
        //}
    }
...
void CIFaceElement::Action(EActions action)
{
    ...
    else if(m_Actions[action].m_classfunction)
    {
        ...
        CMain* obj = (CMain*)m_Actions[action].m_object;
        (obj->*function)(this);
    }
}
...
)
{
    ...
    if(!ichanged)
    {
        //Дополнительное смещение в полпикселя здесь необходимо для корректного округления координат. Было добавлено уже в HD.
    ...
    nC = 0;
	while(nC < MAX_STATES && m_StateImages[nC].Set)
    {
		for(int i = 0; i < 4; ++i)
        {
