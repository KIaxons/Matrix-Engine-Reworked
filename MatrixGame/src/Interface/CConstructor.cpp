...
SNewBorn* CConstructor::ProduceRobot(void*)
{
	...
	if(m_nModuleCnt)
    {
        ...
        m_Build->m_ShadowType = g_Config.m_UnitShadows;
...
)
{
    ...
    if(!m_Base || m_Base->m_BuildingQueue.GetItemsCnt() >= BUILDING_QUEUE_LIMIT) return;
	if(m_nModuleCnt)
    {
        ...
        m_Base->m_BuildingQueue.AddItem(m_Build);
	}
}
...
std::vector<SRobotTemplate> SRobotTemplate::m_AIRobotTypesList[TOTAL_SIDES + 2]; //Два дополнительных места резервируем для загрузки шаблонов спавнера и подкреплений
STemplateRanges SRobotTemplate::m_AIRobotTypesCategories[TOTAL_SIDES][5];
...
)
{
    ...
    robot->m_ShadowType = g_Config.m_UnitShadows;
...
//Считывает и заносит в массив список шаблонов роботов для использования ИИ (на этапе загрузки карты)
...
)
{
    ...
    {
        SRobotTemplate bot;
        m_AIRobotTypesList[side_num].push_back(bot);
        if(!m_AIRobotTypesList[side_num][i].CreateRobotTemplateFromPar(bp->ParGetName(i), bp->ParGet(i)))
        {
            ERROR_S4(L"LoadAIRobotType function, loading robot #", CWStr(i).Get(), L" for side_num ", CWStr(side_num));
        }

        //Определяем относительную силу шаблона робота, исходя из его набора пушек - необходима для дальнейшей работы боевой логики роботов
        m_AIRobotTypesList[side_num][i].CalcStrength();
    }

    //Разбивка на диапазоны имеется только у списков шаблонов для постройки
    if(side_num < TOTAL_SIDES)
    {
        //Разброс шаблонов роботов по диапазонам, число здесь означает приоритет шаблона
        bp = bp->BlockGetNE(L"PriorityRanges");
        int lowest_first = bp->ParGet(L"FirstRangeLowest").GetInt();
        int highest_first = bp->ParGet(L"FirstRangeHighest").GetInt();
        int lowest_second = bp->ParGet(L"SecondRangeLowest").GetInt();
        int highest_second = bp->ParGet(L"SecondRangeHighest").GetInt();
        int lowest_third = bp->ParGet(L"ThirdRangeLowest").GetInt();
        int highest_third = bp->ParGet(L"ThirdRangeHighest").GetInt();
        int lowest_fourth = bp->ParGet(L"FourthRangeLowest").GetInt();
        int highest_fourth = bp->ParGet(L"FourthRangeHighest").GetInt();
        int lowest_fifth = bp->ParGet(L"FifthRangeLowest").GetInt();
        int highest_fifth = bp->ParGet(L"FifthRangeHighest").GetInt();

        //Сортировка шаблонов роботов для ИИ по их приоритету (параметр приоритета также заранее определён в шаблоне)
        if(m_AIRobotTypesList[side_num].size() > 1) AIRobotTypeVectorSort(m_AIRobotTypesList[side_num], 0, m_AIRobotTypesList[side_num].size() - 1);
        ...
        for(int i = 0; i < m_AIRobotTypesList[side_num].size() - 1; ++i)
        {
            ...
            if(!m_AIRobotTypesCategories[side_num][1].Begin && priority > lowest_second - 1) m_AIRobotTypesCategories[side_num][1].Begin = i;
            if(!m_AIRobotTypesCategories[side_num][1].End && priority > highest_second)      m_AIRobotTypesCategories[side_num][1].End = i - 1;
            if(!m_AIRobotTypesCategories[side_num][2].Begin && priority > lowest_third - 1)  m_AIRobotTypesCategories[side_num][2].Begin = i;
            if(!m_AIRobotTypesCategories[side_num][2].End && priority > highest_third)       m_AIRobotTypesCategories[side_num][2].End = i - 1;
            if(!m_AIRobotTypesCategories[side_num][3].Begin && priority > lowest_fourth - 1) m_AIRobotTypesCategories[side_num][3].Begin = i;
            if(!m_AIRobotTypesCategories[side_num][3].End && priority > highest_fourth)      m_AIRobotTypesCategories[side_num][3].End = i - 1;
            if(!m_AIRobotTypesCategories[side_num][4].Begin && priority > lowest_fifth - 1)  m_AIRobotTypesCategories[side_num][4].Begin = i;
        }

        //На случай, если какие-то диапазоны также упёрлись в потолок имеющихся в списке приоритетов
        if(!m_AIRobotTypesCategories[side_num][1].End) m_AIRobotTypesCategories[side_num][2].End = m_AIRobotTypesList[side_num].size() - 1;
        if(!m_AIRobotTypesCategories[side_num][2].End) m_AIRobotTypesCategories[side_num][2].End = m_AIRobotTypesList[side_num].size() - 1;
        if(!m_AIRobotTypesCategories[side_num][3].End) m_AIRobotTypesCategories[side_num][3].End = m_AIRobotTypesList[side_num].size() - 1;
        m_AIRobotTypesCategories[side_num][4].End = m_AIRobotTypesList[side_num].size() - 1;
    }
}
...
{
    for(int i = 0; i < TOTAL_SIDES + 2; ++i) m_AIRobotTypesList[i].clear();
}
...
int SRobotTemplate::AIRobotTypeVectorPartition(std::vector<SRobotTemplate>& vector, int low, int high)
{
    ...
    while(true)
    {
        while(vector[++i].m_TemplatePriority < vector[low].m_TemplatePriority)
        {
            if(i == high) break;
        }

        while(vector[--j].m_TemplatePriority > vector[low].m_TemplatePriority)
        {
            if(j == low) break;
        }
...
bool SRobotTemplate::CreateRobotTemplateFromPar(const CWStr& par_name, const CWStr& par_val)
{
    ...
    //Отладочная запись оригинального строчного шаблона в игровой шаблон, чтобы была возможность прологировать его постройку позже
    //m_StringTemplate = par_name;
    ...
    //Отсутствие оружия во всех слотах по умолчанию
    for(int i = 0; i < RUK_WEAPON_PYLONS_COUNT; ++i)
    {
    ...
    //for(int k = 0; k < MAX_RESOURCES; ++k) m_Resources[k] = 0; //И так всё по нулям при инициализации
    ...
    m_Chassis.m_Price.SetPrice(MRT_CHASSIS, m_Chassis.m_nKind);
    ...
    m_Hull.m_Module.m_Price.SetPrice(MRT_HULL, m_Hull.m_Module.m_nKind);
    ...
    if(par_name.GetCountPar(L",") >= MIN_PAR_CNT + 1)
    {
        ...
        for(int i = 0; i < m_Hull.m_MaxWeaponsCnt; ++i)
        {
            ...
            if(gun_des == L"-") continue; //Если слот оружия в шаблоне пуст
            ...
            for(int j = 1; j <= ROBOT_WEAPONS_COUNT; ++j)
            {
                if(gun_des == g_Config.m_RobotWeaponsConsts[j].short_designation)
                {
                    ...
                    //Если оружие может быть установлено в данный пилон
                    if(g_Config.m_RobotHullsConsts[cur_hull_kind].weapon_pylon_data[i].fit_weapon.test(j))
                    {
                        ...
                        //Проверяем наличие ремонтных модулей и бомб, поскольку они тактически меняют статус шаблона робота
                        if(m_HaveRepair == NOT_REPAIRER && g_Config.m_RobotWeaponsConsts[j].is_module_repairer) m_HaveRepair = PARTIAL_REPAIRER;
                        if(!have_normal_weapon) have_normal_weapon = ( g_Config.m_RobotWeaponsConsts[j].is_module_bomb     == false &&
                                                                       g_Config.m_RobotWeaponsConsts[j].is_module_repairer == false );
                        if(!m_HaveBomb && g_Config.m_RobotWeaponsConsts[j].is_module_bomb) m_HaveBomb = true;

                        m_Weapon[constr_pylon_num].m_Module.m_Price.SetPrice(MRT_WEAPON, m_Weapon[constr_pylon_num].m_Module.m_nKind);
            ...
            //Если оружие в шаблоне оказалось невалидным (отсутствовало в загруженном ранее списке пушек), либо не подходило под указанный пилон корпуса
            if(!is_valid)
            {
                WARNING_S5("Robot weapon with designation \"", gun_des, "\" from template \"", par_name, "\" was not found in loaded weapons list!");
            }
            else if(m_Weapon[constr_pylon_num].m_Module.m_nKind == RUK_EMPTY)
            {
                WARNING_S5("Robot weapon with designation \"", gun_des, "\" from template \"", par_name, "\" does not fit to specified robot pylon!");
            }
        }
    ...
    if(par_name.GetCountPar(L",") >= MIN_PAR_CNT + 2)
    {
        ...
        //Только если голова в шаблоне оказалась валидна (имелась в загруженном ранее списке голов)
        if(m_Head.m_nKind)
        {
            ...
            if(effects_count)
            {
                for(int i = 0; i < effects_count; ++i)
                {
                    if(bp->ParGetName(i) == L"CostModifyPercent")
                    {
                        float cost_mod = max(bp->ParGet(i).GetFloat() * 0.01f, -1.0f) + 1.0f;
                        for(int k = 0; k < MAX_RESOURCES; ++k) m_Resources[k] = int(m_Resources[k] * cost_mod);
                        break;
                    }
                }
            }

            m_Head.m_Price.SetPrice(MRT_HEAD, m_Head.m_nKind); //Цена самого "скидочного модуля" в скидку шаблона, разумеется, не попадает
            for(int k = 0; k < MAX_RESOURCES; ++k) m_Resources[k] += m_Head.m_Price.m_Resources[k];
        }
        else
        {
            WARNING_S5("Robot head with designation \"", str, "\" from template \"", par_name, "\" was not found in loaded heads list!");
        }
    }
