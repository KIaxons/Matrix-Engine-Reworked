...
bool CInterface::Load(CBlockPar& bp, const wchar* name)
{
	...
	int nElementNum = 0;
	CWStr tmpStr;
    CBlockPar* pbp1 = nullptr;
    CBlockPar* pbp2 = nullptr;
    ...
    for(int nC = 0; nC < m_nTotalElements; ++nC)
    {
		...
		if(tmpStr == L"Button")
        {
			...
			if(Const)
            {
                ...
                {
					FSET(ON_UN_PRESS, pButton, g_MatrixMap->GetPlayerSide()->m_Constructor, CConstructor::RemoteOperateModule);
                    FSET(ON_FOCUS, pButton, g_MatrixMap->GetPlayerSide()->m_ConstructPanel, CConstructorPanel::RemoteFocusElement);
                    FSET(ON_UN_FOCUS, pButton, g_MatrixMap->GetPlayerSide()->m_ConstructPanel, CConstructorPanel::RemoteUnFocusElement);
                    pButton->m_nId = POPUP_REACTION_ELEMENT_ID;
				}
                else if(pButton->m_strName == IF_BASE_CONST_BUILD)
                {
					FSET(ON_UN_PRESS, pButton, g_MatrixMap->GetPlayerSide()->m_Constructor, CConstructor::RemoteBuild);
                    g_MatrixMap->GetPlayerSide()->m_Constructor->m_BaseConstBuildBut = pButton;
                }
                else if(pButton->m_strName == IF_BASE_CONST_CANCEL)
                {
                    FSET(ON_UN_PRESS, pButton, g_MatrixMap->GetPlayerSide()->m_Constructor, CIFaceList::PlayerAction);
                }
			}
            ...
            {
                FSET(ON_UN_PRESS, pButton, g_IFaceList, CIFaceList::PlayerAction);
                FSET(ON_UN_PRESS_RIGHT, pButton, g_IFaceList, CIFaceList::PlayerAltAction);
            }
            ...
            {
                FSET(ON_UN_PRESS, pButton, g_IFaceList, CMinimap::ShowPlayerBots);
            }
                ...
                if(pObjectsList->ElementCatch(g_MatrixMap->m_Cursor.GetPos()))
                {
                    ...
                    else if(IS_GROUP_ICON(pObjectsList->m_nId))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if(ps->GetCurSelObject() && ps->GetCurSelObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
                                {
                    ...
                    else if(IS_QUEUE_ICON(pObjectsList->m_nId))
                    {
                        ...
                        CMatrixSideUnit* ps = g_MatrixMap->GetPlayerSide();
                        ((CMatrixBuilding*)ps->m_ActiveObject)->m_BuildingQueue.DeleteItem((pObjectsList->m_nId - QUEUE_ICON) + 1);
                        pObjectsList = ne;
                        return true;
                    }
        ...
        else if(m_strName == IF_MAIN)
        {
            ...
            if(work_group)
            {
                ...
                if(gsel)
                {
                    ...
                    while(objs)
                    {
                        CMatrixRobotAI* robot = objs->ReturnObject()->AsRobot();
                        if(objs->ReturnObject()->IsRobot())
                        {
...
//Always visible elements////////////////////////////////////////////////////////////////////////////////////
                ...
                else if(pElement->m_strName == IF_TURRETS_MAX)
                {
                    ...
                    {
                        if(player_side->m_ActiveObject->AsBuilding()->m_TurretsLimit != turmax)
                        {
                            turmax = player_side->m_ActiveObject->AsBuilding()->m_TurretsLimit;
                            new_turmax = true;
                        }
                    }
                }
...
//Invisible by default elements///////////////////////////////////////////////////////////////////////////////
                ...
                else if(player_side->m_CurrSel == BUILDING_SELECTED || player_side->m_CurrSel == BASE_SELECTED)
                {
                    ...
                    else if(bld->m_BuildingQueue.GetItemsCnt() && (pElement->m_strName == IF_QUEUE_ICON || pElement->m_strName == IF_QUEUE_OTHER || (IS_QUEUE_ICON(pElement->m_nId) && pElement->m_iParam == int(bld))))
                    {
                        pElement->SetVisibility(true);
                    }
                    ...
                    else if(pElement->m_strName == IF_BUILD_TURRET && !bld_tu && !bld_he)
                    {
                        ...
                        if(!bld->GetPlacesForTurrets(pl) || cant_build_turret || bld->m_BuildingQueue.IsMaxItems())
                        {
                            pElement->SetState(IFACE_DISABLED);
                        }
                        else if(bld->GetPlacesForTurrets(pl) && !cant_build_turret && !bld->m_BuildingQueue.IsMaxItems() && pElement->GetState() == IFACE_DISABLED)
                        {
                            pElement->SetState(IFACE_NORMAL);
                        }
                    }
                    ...
                    */
                    ...
                    else if(bld->m_TurretsLimit == 1 && pElement->m_strName == IF_PODL1)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_TurretsLimit == 2 && pElement->m_strName == IF_PODL2)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_TurretsLimit == 3 && pElement->m_strName == IF_PODL3)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_TurretsLimit == 4 && pElement->m_strName == IF_PODL4)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->IsBase() && !bld->m_BuildingQueue.GetItemsCnt() && pElement->m_strName == IF_MB_RES)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_Kind == BUILDING_TITAN && !bld->m_BuildingQueue.GetItemsCnt() && pElement->m_strName == IF_TF_RES)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_Kind == BUILDING_ELECTRONIC && !bld->m_BuildingQueue.GetItemsCnt() && pElement->m_strName == IF_ELF_RES)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_Kind == BUILDING_ENERGY && !bld->m_BuildingQueue.GetItemsCnt() && pElement->m_strName == IF_ENF_RES)
                    {
                        pElement->SetVisibility(true);
                    }
                    else if(bld->m_Kind == BUILDING_PLASMA && !bld->m_BuildingQueue.GetItemsCnt() && pElement->m_strName == IF_PF_RES)
                    {
                        pElement->SetVisibility(true);
                    }
...
///////////////////EOF MAIN////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if((m_strName == IF_BASE) && (player_side->m_CurrSel == BUILDING_SELECTED || player_side->m_CurrSel == BASE_SELECTED) && player_side->m_ActiveObject && player_side->m_ActiveObject->GetObjectType() == OBJECT_TYPE_BUILDING)
    {
        ...
        bool build_flag = (building->m_TurretsHave < building->m_TurretsLimit);
        m_VisibleAlpha = IS_VISIBLE;
        ...
        while(pElement)
        {
            ...
            if(building->IsBase())
            {
                bool bld = building->m_BuildingQueue.GetItemsCnt() < BUILDING_QUEUE_LIMIT;
...
bool CIFaceList::OnMouseLBDown()
{
    ...
    while(pIFace)
    {
        if(pIFace->m_VisibleAlpha)
        {
            //Запуск обработчика кнопки (если надо) и проставление маркера валидности произведённого клика
            if(pIFace->OnMouseLBDown()) bCatch = true;
        }
        pIFace = pIFace->m_NextInterface;
    }
...
bool CIFaceList::OnMouseRBDown()
{
	...
	while(pIFace)
    {
        ...
        {
            //Запуск обработчика кнопки (если надо) и проставление маркера валидности произведённого клика
            if(pIFace->OnMouseRBDown()) bCatch = true;
		}
		pIFace = pIFace->m_NextInterface;
	}
...
void CIFaceList::LogicTact(int ms)
{
    ...
    //Аркадный режим
    else if(ps->GetUnitUnderManualControl()->IsRobotAlive())
    {
        ...
        else
        {
            ...
            if(g_MatrixMap->IsTraceNonPlayerObj())
            {
                ...
                if(((CMatrixRobotAI*)ps->GetUnitUnderManualControl())->CheckFireDist(g_MatrixMap->m_TraceStopPos))
                {
                    //Устанавливаем курсор CROSS_RED
                    g_MatrixMap->m_Cursor.Select(CURSOR_CROSS_RED);
                }
...
void CIFaceList::CreateWeaponDynamicStatics()
{
    ...
    else bot = (CMatrixRobotAI*)player_side->GetCurGroup()->m_FirstObject->ReturnObject();
...
void CIFaceList::EnterManualControlMode(bool pos)
{
    ...
    if(ps->GetCurGroup() && ps->GetCurGroup()->m_FirstObject)
    {
        ...
        CMatrixMapStatic* o = ps->GetCurGroup()->m_FirstObject->ReturnObject();
        if(pos)
        {
...
void CIFaceList::ExitManualControlMode()
{
    ...
    if(ps->IsManualControlMode())
    {
        ...
        if(obj)
        {
            ...
            if(obj_type == OBJECT_TYPE_ROBOT_AI)
            {
                type = ROBOT;
                ...
                //На случай, если игрок дропнул управление, пока робот находился на развороте после стрейфа
                robot->m_ChassisCamForward = robot->m_ChassisForward;
                robot->SetRobotRotating(false);
                ...
                {
                    if(robot->m_RobotWeapons[i].IsEffectPresent()) robot->m_RobotWeapons[i].m_On = true;
                }
            }
        ...
        ps->SelectedGroupBreakAllOrders();
        ps->Select(type, obj);
...
void __stdcall CIFaceList::PlayerAltAction(void* object)
{
    ...
    {
        g_MatrixMap->GetPlayerSide()->GetUnitUnderManualControl()->AsRobot()->AutoBoomSet(true);
    }
    ...
    {
        g_MatrixMap->GetPlayerSide()->GetUnitUnderManualControl()->AsRobot()->AutoBoomSet(false);
    }
...
void CIFaceList::DeleteProgressBars(CMatrixMapStatic* from)
{
    ...
    else
    {
        while(gos)
        {
            if(gos->ReturnObject() == from) break;
    ...
    while(interfaces)
    {
        if(interfaces->m_strName == IF_MAIN)
        {
            ...
            for(int i = 0; i < sel_objs; ++i)
            {
                ...
                if(so)
                {
                    if(so->ReturnObject()->IsRobotAlive())
                    {
                        tex = so->ReturnObject()->AsRobot()->GetMedTexture();
                        robot = true;
                    }
                }
...
DTRACE();
    ...
    while(ifs)
    {
        if(ifs->m_strName == IF_MAIN)
        {
            ...
            if(num == 1 && base->m_BuildingQueue.GetItemsCnt() > 1 && base->m_BuildingQueue.GetTopItem()->m_NextQueueItem)
            {
                CreateQueueIcon(1, base, base->m_BuildingQueue.GetTopItem()->m_NextQueueItem);
            }
            return;
        }
        ifs = ifs->m_NextInterface;
...
void CIFaceList::CreateDynamicTurrets(CMatrixBuilding* building)
{
    ...
    int tur_sheme = building->m_TurretsLimit;
