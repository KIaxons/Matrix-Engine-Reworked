...
void CMatrixCursor::SetVisible(bool flag)
{
    if(!g_Config.m_SoftwareCursor)
    {
        ...
        {
            SetCursor(m_CursorIcons[m_AnimFrame]);
            int cntTry = 0;
...
void CMatrixCursor::Select(const wchar* name)
{
    ...
    for(idx = 0; idx < g_Config.m_CursorsCnt; ++idx)
    {
        if(g_Config.m_Cursors[idx].key == name)
        {
            ...
            m_FramesCnt = par.GetIntPar(2, L",");
            m_CursorSize = par.GetIntPar(3, L",");
            m_CursorTimePeriod = par.GetIntPar(4, L",");
            ...
            else RESETFLAG(m_CursorFlags, CURSOR_REVERSEANIM);
...
{
    int y = TruncFloat(float(m_AnimFrame * m_CursorSize) * m_TexSizeXInversed);
    int x = m_AnimFrame - y * m_CursorInTexLine;
...
void CMatrixCursor::Tact(int ms)
{
    DTRACE();
    ...
    while (m_Time > m_NextCursorTime)
    {
        ...
        {
            ++m_AnimFrame;
            if (m_AnimFrame >= m_FramesCnt) m_AnimFrame = 0;
        } else
        {
            m_AnimFrame += m_FrameInc;
            if (m_FrameInc > 0)
            {
                if (m_AnimFrame >= m_FramesCnt) { m_AnimFrame -= 2; m_FrameInc = -m_FrameInc; } 
            } else
            {
                if (m_AnimFrame < 0) { m_AnimFrame += 2; m_FrameInc = -m_FrameInc; } 
            }

        }
    }
    if (oldframe != m_AnimFrame && FLAG(m_CursorFlags, CURSOR_VISIBLE))
    {
        if (!g_Config.m_SoftwareCursor)
        {
