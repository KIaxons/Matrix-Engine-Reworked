...
CMatrixMap::EScanResult CMatrixMap::ScanLandscapeGroup(void* d, int gx, int gy, const D3DXVECTOR3& start, const D3DXVECTOR3& end)
{
    ...
    if (mg->GetMaxZObjRobots() >= minz)
    {
        ...
        if ((data->mask & (TRACE_OBJECT | TRACE_ROBOT | TRACE_FLYER | TRACE_BUILDING | TRACE_TURRET)) != 0)
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (o->m_IntersectFlagTracer == m_IntersectFlagTracer) continue;
                if (o == data->skip) continue;
                if (!o->FitToMask(data->mask)) continue;
    ...
    if (data->usex)
    {
        ...
        else
        {
            ...
            if (ex * GLOBAL_SCALE < end.x) ex = TruncFloat(end.x * INVERT(GLOBAL_SCALE));
            --ex;
        }
...
//»щем указанные в сете объекты и координаты точки пересечени€ по указанному в запросе вектору (в том числе используетс€ дл€ определени€ точки наведени€ курсора)
CMatrixMapStatic* CMatrixMap::Trace(
    D3DXVECTOR3* result, //„ерез этот аргумент записываетс€ новое значение дл€ найденной координаты (оставл€ть пустым, если поиск новой координаты не требуетс€)
    const D3DXVECTOR3& start, //¬ектор, по которому будем ловить координату его пересечени€ с поверхностью/объектом
    const D3DXVECTOR3& end,
    dword mask, //—ет типов объектов (их коллизий)/ландшафта, пересечение вектора с которыми мы должны ловить во врем€ запроса
    CMatrixMapStatic* skip // акой объект мы пропускаем при поиске (например, при обновлении m_TraceStopObj это будет юнит, что в данный момент находитс€ под ручным контролем игрока)
)
{
        ...
        for(int od = 0; od < m_AD_Obj_cnt; ++od)
        {
            ...
            if(m_AD_Obj[od]->IsFlyer())
            {
                if(mask & TRACE_FLYER)
                {
                    ...
                    else hit = m_AD_Obj[od]->Pick(start,data.dir, &ttt);
                ...
                if(fl->IsCarryingRobot() && (mask & TRACE_ROBOT))
                {
                    ...
                    else hit = ms->Pick(start, data.dir, &ttt);
            ...
            if(data.obj) data.out = start + data.dir * data.last_t;
        }
    }
    
    if( ((*((dword*)(&data.dx)) | *((dword*)(&data.dy))) & 0x7FFFFFFF) == 0 )
    {
        if(!data.dz) return TRACE_STOP_NONE;
                ...
                if(mg != nullptr)
                {
                    if(land || ((data.minz < mg->GetMaxZObjRobots()) && (data.maxz > mg->GetMinZ())))
                    {
                        ...
                        for(int i = 0; i < n; ++i)
                        {
                            ...
                            else hit = o->Pick(start, data.dir, &t);
        ...
        if(land && (mask & (TRACE_LANDSCAPE | TRACE_WATER)))
        {
            ...
            if(z < WATER_LEVEL) return TRACE_STOP_WATER;
            else return TRACE_STOP_LANDSCAPE;
        }
...
//trace landscape
    ...
    {
        int gx0 = TruncDouble(double(start.x * (1.0f / (GLOBAL_SCALE * MAP_GROUP_SIZE))));
        int gy0 = TruncDouble(double(start.y * (1.0f / (GLOBAL_SCALE * MAP_GROUP_SIZE))));
        int gx1 = TruncDouble(double(end.x * (1.0f / (GLOBAL_SCALE * MAP_GROUP_SIZE))));
        int gy1 = TruncDouble(double(end.y * (1.0f / (GLOBAL_SCALE * MAP_GROUP_SIZE))));
        int gxc;
        int gyc;
        ...
        CMatrixMap::EScanResult scan_hit = SR_NONE;
        ...
        if(data.usex)
        {
            ...
            scan_hit = ScanLandscapeGroup(&data, gx0, gy0, start, end);

            if(gxc == 0)
            {
                if((scan_hit == SR_NONE) && (gyc != 0))
                {
                    scan_hit = ScanLandscapeGroup(&data, gx1, gy1, start, end);
                }
            }
            else if((scan_hit == SR_NONE))
            {
                int gcx;
                if(data.gdx >= 0) gcx = 1;
                else gcx = 0;
                ...
                while(gxc-- > 0)
                {
                    int gy = TruncDouble((data.k * ((gx0 + gcx) * (GLOBAL_SCALE * MAP_GROUP_SIZE) - start.x) + start.y) * INVERT((GLOBAL_SCALE * MAP_GROUP_SIZE)));
                    
                    if(prev_gy != gy)
                    {
                        scan_hit = ScanLandscapeGroup(&data, gx0, gy, start, end);
                        prev_gy = gy;
                    }
                    else goto second_scan_x; // to avoid unnecessary condition checking

                    if((scan_hit == SR_NONE))
                    {
    second_scan_x:
                        scan_hit = ScanLandscapeGroup(&data, gx0 + data.gdx, gy, start, end);
                    }

                    if((scan_hit != SR_NONE)) goto scan_x_complete;
                ...
                if(prev_gy != gy1) scan_hit = ScanLandscapeGroup(&data, gx1, gy1, start, end);
    ...
    scan_x_complete:;
        ...
        else
        {
            ...
            if(!gyc)
            {
                if((scan_hit == SR_NONE) && (gxc != 0)) scan_hit = ScanLandscapeGroup(&data, gx1, gy1, start, end);
            }
            else if((scan_hit == SR_NONE))
            {
                int gcy;
                ...
                while(gyc-- > 0)
                {
                    int gx = TruncDouble((data.k * ((gy0 + gcy) * (GLOBAL_SCALE * MAP_GROUP_SIZE) - start.y) + start.x) * INVERT((GLOBAL_SCALE * MAP_GROUP_SIZE)));
                    
                    if(prevgx != gx)
                    {
                        scan_hit = ScanLandscapeGroup(&data, gx, gy0, start, end);
                        prevgx = gx;
                    }
                    else goto second_scan_y; // to avoid unnecessary condition checking

                    if((scan_hit == SR_NONE))
                    {
    second_scan_y:
                        scan_hit = ScanLandscapeGroup(&data, gx, gy0 + data.gdy, start, end);
                    }

                    if((scan_hit != SR_NONE)) goto scan_y_complete;
                    ...
                    {
                        if(gy0 < (m_GroupSize.y - 1)) ++gy0; else
                        {
                            scan_hit = SR_BREAK;
                            goto scan_y_complete;
                        }
                ...
                if(prevgx != gx1) scan_hit = ScanLandscapeGroup(&data, gx1, gy1, start, end);

    scan_y_complete:;
            }
        }

        if(scan_hit == SR_BREAK)
        {
            if(data.obj != nullptr) goto obj_found;
            //ƒобавил здесь этот возврат, чтобы на ручнике была возможность целить оруди€ в небо и за поверхностные границы карты
            //»спользование одного метода "на все случаи жизни, однако, дало свои плоды,
            //ведь если оставить такой же возврат дл€ "стратегического режима", то при установке турелей вектор курсора будет иногда "улетать" за границы карты,
            //что приведЄт к крашу при попытке определени€ позиции проектной турели, поскольку регион карты вне границ карты PointGetTest() в таком случае, разумеетс€, не найдЄт
            //¬ общем, идите-ка вы нахуй со своими авральными кодовыми решени€ми, € их рот ебал.
            if(result && GetPlayerSide()->IsManualControlMode()) *result = end;
            return TRACE_STOP_NONE;
        }

        if(scan_hit == SR_FOUND)
        {
            if(data.obj != nullptr)
            {
obj_found:
    ...
    if(mask & TRACE_WATER)
    {
        ...
        {
            float k = (WATER_LEVEL - start.z) / data.dir.z;
            data.out.x = start.x + data.dir.x * k;
            data.out.y = start.y + data.dir.y * k;
...
CMatrixMap::EScanResult CMatrixMap::ScanLandscapeGroupForLand(void* d, int gx, int gy, const D3DXVECTOR3& start, const D3DXVECTOR3& end)
{
    internal_trace_data* data = (internal_trace_data*)d;

    if(gx < 0  || gx >= m_GroupSize.x || gy < 0 || gy >= m_GroupSize.y) return SR_NONE;
    ...
    if(mg->GetMaxZLand() < minz) return SR_NONE;

    if(start.y < 0 && data->gdy < 0) return SR_BREAK;
    if(start.y > (m_Size.y * GLOBAL_SCALE) && data->gdy > 0) return SR_BREAK;
    if(start.x < 0 && data->gdx < 0) return SR_BREAK;
    if(start.x > (m_Size.x * GLOBAL_SCALE) && data->gdx > 0) return SR_BREAK;
    ...
    if(data->usex)
    {
        ...
        while(x != ex)
        {
            if((x >= px0) && (x < px1) && (y >= py0) && (y < py1))
            {
                ...
                if((mz < mpo->z_land) || (mz < (mpo + 1)->z_land) || (mz < (mpo + m_Size.x+1)->z_land) || (mz < (mpo + 1 + g_MatrixMap->m_Size.x + 1)->z_land))
                {
                    ...
                    D3DXVECTOR3 p2(GLOBAL_SCALE * (x + 1), GLOBAL_SCALE * (y + 1), (mpo + 1 + g_MatrixMap->m_Size.x + 1)->z_land);
...
//  trace landscape
    {
        ...
        if (data.usex)
        {
            ...
            {
                int gcx;
                if (data.gdx >= 0) { gcx = 1; } else { gcx = 0; }
                while (gxc-- > 0)
                {
                    int gy = TruncDouble((data.k * ((gx0 + gcx)*(GLOBAL_SCALE * MAP_GROUP_SIZE) - start.x) + start.y) * INVERT((GLOBAL_SCALE * MAP_GROUP_SIZE)));
    ...
    second_scan_x:
                    ...
                    if ((scanhit!=SR_NONE)) goto scan_x_complete;
                    ...
                    {
                        if (gx0 < (m_GroupSize.x-1)) ++gx0; else
                        {
                            scanhit = SR_BREAK;
                            goto scan_x_complete;
                        }
    ...
    scan_x_complete: ;
        ...
        } else
        {
            ...
            {
                int gcy;
                if (data.gdy >= 0) { gcy = 1; } else { gcy = 0; }
    ...
    second_scan_y:
                    ...
                    if ((scanhit!=SR_NONE)) goto scan_y_complete;
    ...
    EScanResult scanhit = SR_NONE;
    {
        ...
        if (data.usex)
        {
            ...
            if ((scanhit==SR_NONE))
            {
                ...
                while (gxc-- > 0)
                {
                    int gy = TruncDouble((data.k * ((gx0 + gcx)*(GLOBAL_SCALE * MAP_GROUP_SIZE) - from.x) + from.y) * INVERT((GLOBAL_SCALE * MAP_GROUP_SIZE)));
    ...
    scan_x_complete: ;
        ...
        } else
        {
            ...
            if ((scanhit==SR_NONE))
            {
                ...
                while (gyc-- > 0)
                {
                    int gx = TruncDouble((data.k * ((gy0 + gcy)*(GLOBAL_SCALE * MAP_GROUP_SIZE) - from.y) + from.x) * INVERT((GLOBAL_SCALE * MAP_GROUP_SIZE)));
