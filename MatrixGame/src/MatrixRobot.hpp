...
//Статус наличия ремонтных пушек у робота
#define NOT_REPAIRER            0
#define PARTIAL_REPAIRER        1
#define FULL_REPAIRER           2
...
#define ROTATION_TO_CAM_POSITION  4 //После завершения поворота шасси, вектор направления камера будет синхронизирован с вектором шасси (маркер используется для возвращения направления шасси в текущее направление камеры)
...
enum OrderType
{
	...
	ROT_MOVE_TO,
	ROT_MOVE_TO_BACK,
    ROT_MOVE_RETURN,  //Робот пропускает другого робота, а затем возвращается обратно на свою позицию
    ROT_STOP_MOVE,
	ROT_FIRE,
	ROT_STOP_FIRE,
...
//Используется для маркера состояния бомбы (или бомб)
enum EBombStatus
{
    BS_NO_BOMB,
    BS_HAVE_BOMB,
    BS_COUNTDOWN_STARTED,
    BS_TIME_TO_EXPLODE,
    BS_EXPLODING
};
...
{
private:
    CMatrixEffectWeapon* m_WeaponEffect = nullptr;
...
public:
    ...
    void ModifyCoolDown(float addk) { m_WeaponEffect->ModifyCoolDown(addk); }
    void ModifyDist(float addk) { m_WeaponEffect->ModifyDist(addk); }
    bool GetWeaponPos(D3DXVECTOR3& pos) { if(m_WeaponEffect) { pos = m_WeaponEffect->GetPos(); return true; } return false; }
    void SetArcadeCoefficient() { if(m_WeaponEffect) m_WeaponEffect->SetArcadeCoefficient(); }
    void SetDefaultCoefficient() { if(m_WeaponEffect) m_WeaponEffect->SetDefaultCoefficient(); }
    bool IsEffectPresent() const { return m_WeaponEffect != nullptr; }
    int  GetWeaponNum() const { return m_WeaponEffect->GetWeaponNum(); }
    float GetWeaponDist() const { return m_WeaponEffect->GetWeaponDist(); }
    void Modify(const D3DXVECTOR3& pos, const D3DXVECTOR3& dir, const D3DXVECTOR3& speed) { m_WeaponEffect->Modify(pos, dir, speed); }
    void UpdateRepair() { if(m_Module->m_WeaponRepairData) m_Module->m_WeaponRepairData->Update(m_Module); }

    void Tact(float t)
    {
        m_WeaponEffect->Tact(t);
        if(m_Module->m_WeaponRepairData) m_Module->m_WeaponRepairData->Update(m_Module);
    }
    bool IsFireWas() const { return m_WeaponEffect->IsFireWas(); }

    void SetOwner(CMatrixMapStatic* ms) { m_WeaponEffect->SetOwner(ms); }

    void CreateEffect(dword user, FIRE_END_HANDLER handler, int type, int cooldown = 0)
    {
        m_WeaponEffect = (CMatrixEffectWeapon*)CMatrixEffect::CreateWeapon(D3DXVECTOR3(0, 0, 0), D3DXVECTOR3(0, 0, 1), user, handler, type, cooldown);
        if(g_Config.m_WeaponsConsts[type].is_repairer) PrepareRepair();
    }

    void FireBegin(const D3DXVECTOR3& speed, CMatrixMapStatic* skip) { m_WeaponEffect->FireBegin(speed, skip); }

    void FireEnd()
    {
        m_WeaponEffect->FireEnd();
...
public:
    SRobotWeapon            m_RobotWeapons[RUK_WEAPON_PYLONS_COUNT]; //Установленное оружие (заполняется всегда от начала без пропусков, так что чем меньше пушек у робота, тем быстрее переборы)
    ...
    const SRobotWeapon& GetWeapon(int i) const { return m_RobotWeapons[i]; }
    ...
    SRobotWeapon* GetWeaponByPylonNum(byte pylon) //Ищем и возвращаем соответствующее указанному слоту оружие, либо nullptr
    {
        ...
        {
            if(m_RobotWeapons[i].IsEffectPresent() && m_RobotWeapons[i].m_Module->m_ConstructorPylonNum == pylon) return &m_RobotWeapons[i];
        }
    ...
    float GetStrength() { return m_Strength; } //Относительная сила робота, считаемая по суммарной силе его орудий (прописанной в конфиге) и количеству HP
    void CalcStrength();                       //Считаем силу робота
    ...
    bool IsInManualControl() { return (g_MatrixMap->GetPlayerSide()->GetUnitUnderManualControl() == this); }
    ...
    bool CanBreakOrder()
    {
        ...
        return !IsAutomaticMode() && ((m_Side != PLAYER_SIDE) || !IsInManualControl());
    }
    ...
    bool RotateRobotChassis(const D3DXVECTOR3& dest, byte rotate_cam_link = CONNECTED_CAM_ROTATION, float* rotate_angle = nullptr, bool rotate_anim = true);      // поворот робота
...
//Orders queue processing
    ...
    void StopFire();
    void BeginBombCountdown(); //Помечает все найденные бомбы и, тем самым, запускает их анимацию подрыва (если такая есть)
    void DetonateTheBomb();    //Подрывает все помеченные выше бомбы после того, как доиграет их анимация подрыва
...
inline void SRobotWeapon::Draw(CMatrixRobotAI* robot)
{
...
inline bool CMatrixRobotAI::PLIsInPlace() const
{
    CPoint ptp;
