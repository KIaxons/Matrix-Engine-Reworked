...
void CTransition::BuildTexture()
{
    //Проверяем, включено ли раскрытие створок в data.txt
    bool loadingScreenAnim = g_MatrixData->BlockGet(BLOCK_PATH_MAIN_CONFIG)->ParGet(CFG_LOADING_SCREEN_ANIM).GetBool();
    ...
    HDC hdc = GetDC(g_Wnd);
    //HDC hdc = GetDC(nullptr); //Этот вариант решает проблему с "чёрным кадром" после альттабов, однако создаёт разрыв между сменой рендера из основной игры и из ПБ
    ...
    for(int i = 0; i < m_GeomCnt; ++i)
    {
        ...
        else m_Geom[i].verts[0].p.x = da.GetStrPar(0, L"|").GetFloatPar(0, L",") * coord_mod + x_offset;
        m_Geom[i].verts[0].p.y = da.GetStrPar(0, L"|").GetFloatPar(1, L",") * coord_mod;
        ...
        else m_Geom[i].verts[1].p.x = da.GetStrPar(1, L"|").GetFloatPar(0, L",") * coord_mod + x_offset;
        m_Geom[i].verts[1].p.y = da.GetStrPar(1, L"|").GetFloatPar(1, L",") * coord_mod;
        ...
        else m_Geom[i].verts[2].p.x = da.GetStrPar(3, L"|").GetFloatPar(0, L",") * coord_mod + x_offset;
        m_Geom[i].verts[2].p.y = da.GetStrPar(3, L"|").GetFloatPar(1, L",") * coord_mod;
        ...
        else m_Geom[i].verts[3].p.x = da.GetStrPar(2, L"|").GetFloatPar(0, L",") * coord_mod + x_offset;
        m_Geom[i].verts[3].p.y = da.GetStrPar(2, L"|").GetFloatPar(1, L",") * coord_mod;
        ...
        if(tra->ParGetName(i).CompareFirst(L"Up,"))
        {
            ...
            m_Geom[i].dir *= tra->ParGetName(i).GetFloatPar(1, L",");
        }
        ...
        else if(tra->ParGetName(i).CompareFirst(L"Left,"))
        {
            ...
            m_Geom[i].dir *= tra->ParGetName(i).GetFloatPar(1, L",") * (float(g_ScreenX) * 1.08f / native_aspect_res);
        }
...
void CTransition::RenderToPrimaryScreen()
{
    ...
    Render();
    g_D3DD->EndScene();
    g_D3DD->Present(nullptr, nullptr, nullptr, nullptr);

    SETFLAG(g_MatrixMap->m_Flags, MMFLAG_TRANSITION);
}

void CTransition::Tact(int ms)
{
