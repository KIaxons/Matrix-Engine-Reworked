...
void CMatrixBuilding::GetResources(dword need)
{
	...
	if(need & m_RChange & MR_Graph)
    {
        ...
        if(!m_GGraph->IsAlreadyLoaded())
        {
            ...
            m_GGraph->Load(path, CMatrixEffect::GetSingleBrightSpriteTex(SPR_SPOT_LIGHT), CSkinManager::GetSkin, GSP_SIDE);
        }
	}
...
{
    if(m_Side == PLAYER_SIDE || FLAG(g_Config.m_DIFlags, DI_SIDE_INFO))
    {
        //Отрисовываем иконки поступающих ресурсов над зданиями игрока
        if(FLAG(m_ObjectFlags, BUILDING_NEW_INCOME))
        {
            ...
            switch(m_Kind)
            {
                case BUILDING_BASE:
                {
                    ...
                    int income;
                    if(m_Side == PLAYER_SIDE) income = RESOURCES_INCOME_FROM_BASE * fu / 100;
                    else income = RESOURCES_INCOME_FROM_BASE * g_MatrixMap->GetSideById(m_Side)->GetResourceForceUp() / 100;
                    CMatrixEffect::CreateBillboardScore(L"a" + CWStr(income, g_CacheHeap), m_TopPoint, 0xFFFFFFFF);

                    //Вывод картинок ресурсов поштучно
                    /*
                    if(m_BaseRCycle == 0)
                    {
                        CMatrixEffect::CreateBillboardScore(L"t" + CWStr(income, g_CacheHeap), m_TopPoint, 0xFFFFFF00);
                    }
                    else if(m_BaseRCycle == 1)
                    {
                        CMatrixEffect::CreateBillboardScore(L"e" + CWStr(income, g_CacheHeap), m_TopPoint, 0xFFFFFF00);
                    }
                    else if(m_BaseRCycle == 2)
                    {
                        CMatrixEffect::CreateBillboardScore(L"b" + CWStr(income, g_CacheHeap), m_TopPoint, 0xFFFFFF00);
                    }
                    else if(m_BaseRCycle == 3)
                    {
                        CMatrixEffect::CreateBillboardScore(L"p" + CWStr(income, g_CacheHeap), m_TopPoint, 0xFFFFFF00);
                    }
                    */
	...
	if(m_GGraph)
    {
        ...
        else
        {
            ...
            if(m_OccasionalAnimRun)
            {
                ...
                byte frame_state = m_GGraph->OccasionalTact(cms);
                if(frame_state == FRAME_CHANGED)
                {
                    if(m_ShadowType == SHADOW_STENCIL)
                    {
                        RChange(MR_ShadowStencil);
                ...
                else if(frame_state == FRAMES_LOOP_DONE) m_OccasionalAnimRun = false;
            }
        }
	}
}
...
void CMatrixBuilding::LogicTact(int cms)
{
    ...
    if(m_State != BUILDING_DIP && m_State != BUILDING_DIP_EXPLODED)
    {
        ...
        if(m_Kind != BUILDING_BASE)
        {
            ...
            {
                if(m_Side == NEUTRAL_SIDE && m_TrueColor.m_ColoredCnt > 0)
                {
        ...
        if(su)
        {
            ...
            switch(m_Kind)
            {
                case BUILDING_TITAN:
                {
                    ...
                    {
                        //m_ResourceAmount += RESOURCES_INCOME_FROM_FACTORY;
                        SETFLAG(m_ObjectFlags, BUILDING_NEW_INCOME);
                        m_ResourcePeriod = 0;
                        su->AddResourceAmount(TITAN, RESOURCES_INCOME_FROM_FACTORY);
                    }
                    break;
                }
                case BUILDING_PLASMA:
                {
                    if(m_ResourcePeriod >= g_Config.m_Timings[RESOURCE_PLASMA])
                    {
                        ...
                        SETFLAG(m_ObjectFlags, BUILDING_NEW_INCOME);
                        m_ResourcePeriod = 0;
                        su->AddResourceAmount(PLASMA, RESOURCES_INCOME_FROM_FACTORY);
                    }
                    break;
                }
                case BUILDING_ELECTRONIC:
                {
                    if(m_ResourcePeriod >= g_Config.m_Timings[RESOURCE_ELECTRONICS])
                    {
                        ...
                        SETFLAG(m_ObjectFlags, BUILDING_NEW_INCOME);
                        m_ResourcePeriod = 0;
                        su->AddResourceAmount(ELECTRONICS, RESOURCES_INCOME_FROM_FACTORY);
                    }
                    break;
                }
                case BUILDING_ENERGY:
                {
                    if(m_ResourcePeriod >= g_Config.m_Timings[RESOURCE_ENERGY])
                    {
                        ...
                        SETFLAG(m_ObjectFlags, BUILDING_NEW_INCOME);
                        m_ResourcePeriod = 0;
                        su->AddResourceAmount(ENERGY, RESOURCES_INCOME_FROM_FACTORY);
                    }
                    break;
                }
                case BUILDING_REPAIR:
                {
                    ...
                    while(obj)
                    {
                        if(obj->IsRobotAlive() && obj->GetSide() == m_Side && obj->NeedRepair())
                        {
                ...
                case BUILDING_BASE:
                {
                    if(m_ResourcePeriod >= g_Config.m_Timings[RESOURCE_BASE])
                    {
                        ...
                        int ra = RESOURCES_INCOME_FROM_BASE * fu / 100;
                        SETFLAG(m_ObjectFlags, BUILDING_NEW_INCOME);
                        m_ResourcePeriod = 0;
                        ...
                        //if(m_BaseRCycle == 0)
                        //{
                        //    su->AddTitan(RESOURCES_INCOME_FROM_FACTORY * fu / 100);
                        //}
                        //else if(m_BaseRCycle == 1)
                        //{
                        //    su->AddElectronics(RESOURCES_INCOME_FROM_FACTORY * fu / 100);
                        //}
                        //else if(m_BaseRCycle == 2)
                        //{
                        //    su->AddEnergy(RESOURCES_INCOME_FROM_FACTORY * fu / 100);
                        //}
                        //else if(m_BaseRCycle == 3)
                        //{
                        //    su->AddPlasma(RESOURCES_INCOME_FROM_FACTORY * fu / 100);
                        //}
                    }
                    break;
                }
...
void CMatrixBuilding::FreeDynamicResources()
{
...
void CMatrixBuilding::BeforeDraw()
{
...
void CMatrixBuilding::Draw()
{
    ...
    {
        m_GGraph->DrawLightSprites();
    }
}

void CMatrixBuilding::DrawShadowStencil()
{
...
float CMatrixBuilding::GetFloorZ()
{
...
void CMatrixBuilding::OnLoad()
{
    ...
    switch(m_Kind)
    {
        case BUILDING_BASE:
        {
            m_TurretsLimit = CONSTRUCTION_BASE_TURRETS_LIMIT;
            m_Name = L"BASE";
        ...
        case BUILDING_TITAN:
        {
            m_TurretsLimit = TITAN_FACTORY_TURRETS_LIMIT;
            m_Name = L"TITAN";
            break;
        }
        case BUILDING_PLASMA:
        {
            m_TurretsLimit = PLASMA_FACTORY_TURRETS_LIMIT;
            m_Name = L"PLASMA";
            break;
        }
        case BUILDING_ELECTRONIC:
        {
            m_TurretsLimit = ELECTRONIC_FACTORY_TURRETS_LIMIT;
            m_Name = L"ELECTRONICS";
            break;
        }
        case BUILDING_ENERGY:
        {
            m_TurretsLimit = ENERGY_FACTORY_TURRETS_LIMIT;
            m_Name = L"ENERGY";
            break;
        }
        case BUILDING_REPAIR:
        {
            m_TurretsLimit = REPAIR_FACTORY_TURRETS_LIMIT;
            m_Name = L"REPAIR";
            m_ConstantAnim = false; //Анимация ремонтного завода не должна проигрываться постоянно
            break;
        }
    }
...
void CMatrixBuilding::SetNeutral()
{
    m_TrueColor.m_ColoredCnt = 0;
    m_TrueColor.m_Color = 0;
    m_Side = NEUTRAL_SIDE;
...
void CMatrixBuilding::Reinforcements()
{
    ...
    while(cur_scores < max_scores)
    {
        ...
        for(; cnt > 0; --cnt)
        {
            template_num = g_MatrixMap->Rnd(0, SRobotTemplate::m_AIRobotTypesList[REINFORCEMENTS_TEMPLATES].size() - 1); //Выбираем случайный номер шаблона робота из списка
            template_scores = SRobotTemplate::m_AIRobotTypesList[REINFORCEMENTS_TEMPLATES][template_num].m_TemplatePriority; //В случае вызова подкреплений, за ограничитель их суммарной силы отвечает приоритет шаблона
...
void CMatrixBuilding::ReleaseMe()
{
    DeletePlacesShow();
...
void CMatrixBuilding::CreatePlacesShow(void)
{
    ...
    for(int i = 0; i < MAX_PLACES; ++i) m_PlacesShow[i] = SEffectHandler();
...
void CMatrixBuilding::DeletePlacesShow()
{
    ...
    {
        for(int i = 0; i < MAX_PLACES; ++i) m_PlacesShow[i].Release();
...
void CBuildingQueue::TickTimer(int ms)
{
...
int CBuildingQueue::GetRobotsCnt() const
{
    CMatrixMapStatic* objects = m_Top;
    int robots = 0;
    while(objects)
    {
        if(objects->GetObjectType() == OBJECT_TYPE_ROBOT_AI) ++robots;
...
void CBuildingQueue::ReturnRobotResources(CMatrixRobotAI* robot)
{
