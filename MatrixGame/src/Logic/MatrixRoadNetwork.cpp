...
CMatrixRoad::~CMatrixRoad()
{
    DeleteZone();
    DeletePath();
    if(m_Start != nullptr) m_Start->DeleteRoad(this);
    if(m_End != nullptr) m_End->DeleteRoad(this);
    m_Start = nullptr;
    m_End = nullptr;
    m_Parent = nullptr;
}
...
DTRACE();
    ...
    if(m_Start == crotch) m_Start = nullptr;
    if(m_End == crotch) m_End = nullptr;
}
...
void CMatrixRoad::AddPathFromRoad(CMatrixRoad* road)
{
    ...
    else
    {
        ...
        //Before
        else if(m_Path[0] == road->m_Path[road->m_PathCnt - 1])
        {
        ...
        {
            ERROR_S(L"CMatrixRoad::AddPathFromRoad Error!");
        }
...
void CMatrixRoad::CorrectStartEndByZone()
{
    ...
    {
        ERROR_S(L"CMatrixRoad::CorrectStartEndByZone Error!");
    }
}
...
void CMatrixRoad::AddZoneFromRoad(CMatrixRoad* road)
{
    ...
    else
    {
        ...
        else if(m_Zone[0] == road->m_Zone[road->m_ZoneCnt - 1])
        {
        ...
        {
            ERROR_S(L"CMatrixRoad::AddZoneFromRoad Error!");
        }
...
{
    if(m_ZoneCnt!=road->m_ZoneCnt) return false;

    int i;

    for(i=0;i<m_ZoneCnt;i++) {
        if(m_Zone[i]!=road->m_Zone[i]) break;
    }
    if(i<m_ZoneCnt) {
        for(i=0;i<m_ZoneCnt;i++) {
            if(m_Zone[i]!=road->m_Zone[m_ZoneCnt-i-1]) return false;
        }
    }
...
void CMatrixRoad::CalcDist()
{
    ...
    for(int i=1;i<m_ZoneCnt;i++) {
        m_Dist+=(int)sqrt(double(m_Parent->m_Zone[m_Zone[i-1]].m_Center.Dist2(m_Parent->m_Zone[m_Zone[i]].m_Center)));
    }
}
...
CMatrixCrotch::~CMatrixCrotch()
{
    ...
    for(int i=m_RoadCnt-1;i>=0;i--) {
        if(m_Road[i]!=nullptr) m_Road[i]->DeleteCrotch(this);
        m_Road[i]=nullptr;
    }
    m_RoadCnt=0;
    m_Parent=nullptr;
...
int CMatrixCrotch::CalcCntRoadEqualMoveTypeAndNoDeadlock()
{
    ...
    for(int i=0;i<m_RoadCnt;i++) {
        if(m_Road[i]->GetOtherCrotch(this)->m_RoadCnt<=1) continue;
        if(m_Road[i]->m_Move==mt) r++;
    }
    return r;
}
...
void CMatrixRoadRoute::Clear()
{
    ...
    m_ListCnt=0;
    m_ListCntMax=0;
    if(m_Header!=nullptr) { HFree(m_Header,m_Parent->m_Heap); m_Header=nullptr; }
    if(m_Modules!=nullptr) { HFree(m_Modules,m_Parent->m_Heap); m_Modules=nullptr; }
}
...
{
    if(m_ListCnt+cnt<=m_ListCntMax) return;
    m_ListCntMax+=m_ListCnt+cnt;
    if(strong) {
        if(m_ListCntMax<=1);
        else if(m_ListCntMax<=4) m_ListCntMax=4;
        else m_ListCntMax+=8;
    }

    m_Header=(SMatrixRoadRouteHeader *)HAllocClearEx(m_Header,m_ListCntMax*sizeof(SMatrixRoadRouteHeader),m_Parent->m_Heap);
    m_Modules=(SMatrixRoadRouteUnit *)HAllocClearEx(m_Modules,m_ListCntMax*m_Parent->m_CrotchCnt*sizeof(SMatrixRoadRouteUnit),m_Parent->m_Heap);
}
...
void CMatrixRoadRoute::CopyUnit(int listfrom,int listto)
{
    ...
    CopyMemory(m_Modules+listto*m_Parent->m_CrotchCnt,m_Modules+listfrom*m_Parent->m_CrotchCnt,m_Header[listto].m_Cnt*sizeof(SMatrixRoadRouteUnit));
}
...
//}
    ...
    m_Modules[listno*m_Parent->m_CrotchCnt+m_Header[listno].m_Cnt].m_Road=road;
    m_Modules[listno*m_Parent->m_CrotchCnt+m_Header[listno].m_Cnt].m_Crotch=crotch;
...
CMatrixRoadNetwork::CMatrixRoadNetwork(CHeap * he):CMain()
{
    ...
    m_CrotchCnt=0;
    m_CrotchFirst=nullptr;
    m_CrotchLast=nullptr;

    m_Zone=nullptr;
    m_ZoneCnt=0;
    m_ZoneCntMax=0;

    m_RouteFirst=nullptr;
    m_RouteLast=nullptr;
    ...
    m_RoadFindIndex=nullptr;
    m_CrotchFindIndex=nullptr;
    m_RegionFindIndex=nullptr;
...
void CMatrixRoadNetwork::Clear()
{
    ...
    if(m_RoadFindIndex) { HFree(m_RoadFindIndex,m_Heap); m_RoadFindIndex=nullptr; }
    if(m_CrotchFindIndex) { HFree(m_CrotchFindIndex,m_Heap); m_CrotchFindIndex=nullptr; }
    if(m_RegionFindIndex) { HFree(m_RegionFindIndex,m_Heap); m_RegionFindIndex=nullptr; }
    ...
    while(m_RoadFirst) DeleteRoad(m_RoadLast);
    while(m_CrotchFirst) DeleteCrotch(m_CrotchLast);
}
...
CMatrixRoad * CMatrixRoadNetwork::FindRoadByZone(int zone)
{
    ...
    while(road!=nullptr) {
        for(int i=0;i<road->m_ZoneCnt;i++) {
            if(road->m_Zone[i]==zone) return road;
        }
        road=road->m_Next;
    }
    return nullptr;
...
bool CMatrixRoadNetwork::IsCrotchExist(CMatrixCrotch * un)
{
    ...
    while(crotch!=nullptr) {
        if(un==crotch) return true;
...
CMatrixCrotch * CMatrixRoadNetwork::FindCrotchByZone(int zone)
{
    ...
    CMatrixCrotch * crotch=m_CrotchFirst;
    while(crotch!=nullptr) {
        if(crotch->m_Zone==zone) return crotch;
        crotch=crotch->m_Next;
    }
    return nullptr;
}
...
bool CMatrixRoadNetwork::UnionRoadStep(dword flags)
{
    ...
    while(crotch) {
        ...
        if(crotch2->m_RoadCnt==2 && (!crotch2->m_Critical)) {
            ...
            CMatrixCrotch * c1,* c2;
            if(r1->m_Start!=crotch2) c1=r1->m_Start; else c1=r1->m_End;
            if(r2->m_Start!=crotch2) c2=r2->m_Start; else c2=r2->m_End;
            ...
            if(flags&2) r1->CorrectStartEndByZone();
            if(flags&2) r2->CorrectStartEndByZone();
            ...
            if(flags&1) r1->AddPathFromRoad(r2);
            if(flags&2) r1->AddZoneFromRoad(r2);
            ...
            if(flags&1) r1->CorrectStartEndByPath();
            if(flags&2) r1->CorrectStartEndByZone();
...
int CMatrixRoadNetwork::SelectCrotchByRoadCnt(int amin,bool skipcritical)
{
    ...
    CMatrixCrotch * crotch=m_CrotchFirst;
    while(crotch) {
        if(!skipcritical) crotch->m_Select=crotch->m_RoadCnt<=amin;
        else crotch->m_Select=!crotch->m_Critical && crotch->m_RoadCnt<=amin;

        if(crotch->m_Select) cnt++;
...
void CMatrixRoadNetwork::SelectCrotchSingleAndRadius(int radius)
{
    ...
    while(crotch) {
        ...
        if(crotch->m_RoadCnt==1) {
            ...
            if((tp.x*tp.x+tp.y*tp.y)<radius*radius) crotch->m_Select=true;
        }
        crotch=crotch->m_Next;
    }
}
            ...
            while(road2) {
                ...
                if(iz==0 || iz==road2->m_ZoneCnt-1) {
                    ...
                    if(!crotch) {
                        ...
                        if(crotch==nullptr) crotch=AddCrotch();
                    }
...
void CMatrixRoadNetwork::CalcZoneColor()
{
    ...
    if(m_ZoneCnt>0) {
            ...
            else
            {
                for(t=0;t<colorscnt; ++t)
                {
                    for(u=0;u<m_Zone[i].m_NearZoneCnt; ++u)
                    {
                        ...
                        if(m_Zone[cz].m_Color==t) break;
                    }

                    if(u>=m_Zone[i].m_NearZoneCnt) break;
                }
        ...
        for(i=0;i<m_ZoneCnt; ++i)
        {
            if(m_Zone[i].m_Color<0 || m_Zone[i].m_Color>colorscnt) ERROR_S(L"CMatrixRoadNetwork::CalcZoneColor Error!");
...
{
    for(int i = 0; i < m_ZoneCnt; ++i) m_Zone[i].m_Road = false;
    CMatrixRoad* road = m_RoadFirst;
    while(road)
    {
        road->MarkZoneRoad();
...
{
    if(!m_Zone[zone].m_Road) ERROR_S(L"CMatrixRoadNetwork::InsertCrotch Error!");
...
bool CMatrixRoadNetwork::CreateCrotchAndRoadFromZone(int zone)
{
    ...
    {
        return InsertCrotch(zone)!=nullptr;
    }

    for(i=0;i<m_ZoneCnt;i++)
    {
        m_Zone[i].m_FPLevel=-1;
        if(m_Zone[i].m_Bottleneck) m_Zone[i].m_FPWt=4;
        else if(m_Zone[i].m_Move==0)
        {
            crotch=FindCrotchByZone(i);
            if(crotch && crotch->m_RoadCnt>=3) m_Zone[i].m_FPWt=1;
            else if(crotch && crotch->m_RoadCnt>=1) m_Zone[i].m_FPWt=2;
            else m_Zone[i].m_FPWt=3;
        }
        else m_Zone[i].m_FPWt=10;
        m_Zone[i].m_FPWtp=0;
    }
    ...
    while(zb1cnt>0)
    {
		...
		zb1cnt=zb2cnt;
		zb2cnt=0;
		int * zbt=zb1; zb1=zb2; zb2=zbt;
    }

    HFree(zb,m_Heap);

    if(zonefound<0) return false;
	...
	while(true)
    {
		...
		for(i=0;i<m_Zone[curzone].m_NearZoneCnt;i++)
        {
			...
			if(nz==zone) { nz_=nz; wtp_=wtp; break; }
			else if(wtp<=wtp_) { nz_=nz; wtp_=wtp; }
		}

		if(nz_<0)
        {
            ERROR_S(L"CMatrixRoadNetwork::CreateCrotchAndRoadFromZone Error!");
		}
...
bool CMatrixRoadNetwork::CreateCrotchAndRoadByPath(int * path,int cnt)
{
    ...
    while(ibegin<cnt-1) {
        while(ibegin<cnt && m_Zone[path[ibegin]].m_Road) ibegin++;
        ibegin--;

        int iend=ibegin+1;
        while(iend<cnt && !m_Zone[path[iend]].m_Road) iend++;
        if(iend>=cnt) iend--;
...
void CMatrixRoadNetwork::SelectUnneededIsland()
{
    ...
    for(int i=0;i<cnt;i++) {
        crotch=m_CrotchFirst;
        while(crotch) {
            if(crotch->m_Island==i && crotch->m_Critical) break;
            crotch=crotch->m_Next;
        }
        if(!crotch) {
            crotch=m_CrotchFirst;
            while(crotch) {
                if(crotch->m_Island==i) crotch->m_Select=true;
                crotch=crotch->m_Next;
            }
        }
    }
...
//CHelper::Create(0,2)->Cone(D3DXVECTOR3(10.0f*m_Zone[nz].m_Center.x,10.0f*m_Zone[nz].m_Center.y,0),D3DXVECTOR3(10.0f*m_Zone[nz].m_Center.x,10.0f*m_Zone[nz].m_Center.y,10.0f+wtp*2.0f),0.5f,0.5f,0xffffffff,0xffff0000,6);
	...
	while(true)
    {
		...
		if(nz_<0)
        {
            ERROR_S(L"CMatrixRoadNetwork::CalcPathZoneByRoad Error!");
		}
...
int CMatrixRoadNetwork::CalcDistByRoad(int zstart,int zend,int * path)
{
    ...
    {
        for(int i=1;i<cnt;i++) rv+=(int)sqrt((double)m_Zone[path[i-1]].m_Center.Dist2(m_Zone[path[i]].m_Center));
    }

    if(delpath) HFree(path,m_Heap);
...
int MRN_CAR_BuildPath(CMatrixRoadNetwork* rnw, SZT_CAR* zt, CMatrixRoad* road, int* path, int zstart)
{
    ...
    if(zt[curzone].road != road) ++curlevel;
    ...
    while(curlevel > 0)
    {
		...
		for(i = 0; i < ccnt; ++i)
        {
            ...
            if(zt[nz].road == road)
            {
                if((zt[nz].level >= 0 && zt[nz].level < curlevel) || (zt[nz].level2 >= 0 && zt[nz].level2 < curlevel))
                {
                    path[cnt] = nz;
                    ++cnt;
                    curzone = nz;
                    if(zt[nz].level >= 0) curlevel = zt[nz].level;
                    else curlevel = zt[nz].level2;
                    break;
                }
            }
        }
    ...
    if(curlevel != 0) ERROR_S(L"MRN_CAR_BuildPath Error!");
...
//CHelper::Create(0,2)->Cone(D3DXVECTOR3(10.0f*m_Zone[i].m_Center.x,10.0f*m_Zone[i].m_Center.y,0),D3DXVECTOR3(10.0f*m_Zone[i].m_Center.x,10.0f*m_Zone[i].m_Center.y,10.0f),1.5f,1.5f,0xffffffff,colors[(zt[i].road->m_Start->m_Zone) % colorscnt],6);
    ...
    while(findok) {
        ...
        for(int i=0;i<m_ZoneCnt;i++) zt[i].level2=-1;
...
//}
            ...
            if(u<m_Zone[i].m_NearZoneCnt) {
                ...
                for(;u<m_Zone[i].m_NearZoneCnt;u++) {
                    if((zt[m_Zone[i].m_NearZone[u]].level>=0 || zt[m_Zone[i].m_NearZone[u]].level2>=0) && zt[m_Zone[i].m_NearZone[u]].road!=zt[i].road) {
...
//CHelper::Create(0,2)->Cone(D3DXVECTOR3(10.0f*m_Zone[i].m_Center.x,10.0f*m_Zone[i].m_Center.y,0),D3DXVECTOR3(10.0f*m_Zone[i].m_Center.x,10.0f*m_Zone[i].m_Center.y,10.0f+5.0f*zt[i].level),1.5f,1.5f,0xffffffff,colors[(zt[i].road->m_Start->m_Zone) % colorscnt],6);
            }
        }
    ...
    for(i=0;i<ap_cnt;i++) {
        ...
        for(u=i+1;u<ap_cnt;u++) {
            if(ap[u].len<2) continue;

            int zs2=ap[u].path[0];
            int ze2=ap[u].path[ap[u].len-1];

            if(m_Zone[zs1].m_Center.Dist2(m_Zone[zs2].m_Center)<distdel2 && m_Zone[ze1].m_Center.Dist2(m_Zone[ze2].m_Center)<distdel2) {
                ap[u].len=0;
            } else if(m_Zone[zs1].m_Center.Dist2(m_Zone[ze2].m_Center)<distdel2 && m_Zone[ze1].m_Center.Dist2(m_Zone[zs2].m_Center)<distdel2) {
                ap[u].len=0;
            }
        }
    }
...
bool CMatrixRoadNetwork::IsValidate(bool exception)
{
    ...
    while(crotch) {
        for(int i=0;i<crotch->m_RoadCnt;i++)
        {
            if(!IsRoadExist(crotch->m_Road[i]))
            {
                if(exception)
                {
                    ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 1!");
                }
                else return false;
            }
        }
        if(crotch->m_Zone<0)
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 2!");
            }
            else return false;
        }
    ...
    for(int i=0;i<m_ZoneCnt; ++i)
    {
        if(m_Zone[i].m_FPLevel>1)
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 3!");
            }
            else return false; 
        }
    ...
    while(road)
    {
        if(!IsCrotchExist(road->m_Start))
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 4!");
            }
            else return false;
        }
        if(!IsCrotchExist(road->m_End))
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 5!");
            }
            else return false; 
        }
        if(road->m_ZoneCnt<2)
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 6!");
            }
            else return false; 
        }
    ...
    for(int i = 0; i < m_ZoneCnt; ++i)
    {
        if(!FindCrotchByZone(i) && m_Zone[i].m_FPLevel > 1)
        {
            if(exception)
            {
                ERROR_S(L"CMatrixRoadNetwork::IsValidate Error 7!");
            }
            else return false;
        }
...
void CMatrixRoadNetwork::Test()
{
    ...
    while(road)
    {
        if(!road->m_Start)
        {
            ERROR_S(L"CMatrixRoadNetwork::Test Error 1!");
        }

        if(!road->m_End)
        {
            ERROR_S(L"CMatrixRoadNetwork::Test Error 2!");
        }
...
{
    while (m_RouteFirst) DeleteRoute(m_RouteLast);
}
...
void CalcRoute_r(CMatrixCrotch * cstart,CMatrixCrotch * cend,CMatrixRoadRoute * route,CMatrixRoad * road,CMatrixCrotch * crotch,int maxdist)
{
    route->AddUnit(route->m_ListCnt-1,road,crotch);
    if(road) route->m_Header[route->m_ListCnt-1].m_Dist+=road->m_Dist;
    if(crotch==cend) {
    ...
    for(int i=0;i<crotch->m_RoadCnt;i++) {
        ...
        if(cin->m_Select) continue;
        if(route->m_Header[route->m_ListCnt-1].m_Dist+crotch->m_Road[i]->m_Dist>maxdist) continue;
...
CMatrixRoadRoute * CMatrixRoadNetwork::CalcRoute(CMatrixCrotch * cstart,CMatrixCrotch * cend)
{
    ...
    route->AddList();
    CalcRoute_r(cstart,cend,route,nullptr,cstart,maxdist);
    route->m_ListCnt--;
...
CMatrixRoadRoute* CMatrixRoadNetwork::GetRoute(CMatrixCrotch* cstart, CMatrixCrotch* cend)
{
    ...
    if(!route) route = CalcRoute(cstart, cend);
    return route;
}
...
void CMatrixRoadNetwork::FindPathFromCrotchToRegion(byte mm, CMatrixCrotch* cstart, int region, CMatrixRoadRoute* rr, bool test)
{
    ...
    if(!m_RoadFindIndex) m_RoadFindIndex = (CMatrixRoad**)HAlloc(m_RoadCnt * sizeof(CMatrixRoad*), m_Heap);
    if(!m_CrotchFindIndex) m_CrotchFindIndex = (CMatrixCrotch**)HAlloc(m_CrotchCnt * sizeof(CMatrixCrotch*), m_Heap);

    CMatrixCrotch* crotchcur = nullptr;

    for(int type = 0; type <= 2; ++type)
    {
        CMatrixRoad* road2 = nullptr;
        CMatrixCrotch* crotch2 = nullptr;
        CMatrixCrotch* crotch = m_CrotchFirst;
        ...
        while(crotchcur != cstart)
        {
            ...
            if(!crotch2) ERROR_S(L"CMatrixRoadNetwork::FindPathFromCrotchToRegion Error!");
...
void CMatrixRoadNetwork::FindPathFromRegionPath(byte mm, int rcnt, int* rlist, CMatrixRoadRoute* rr, bool test)
{
    ...
    int ln = rr->AddList();
    rr->AddUnit(ln, nullptr, m_Region[rlist[0]].m_Crotch[0]);

    for(int i = 1; i < rcnt; ++i)
    {
        FindPathFromCrotchToRegion(mm, rr->m_Modules[ln * m_CrotchCnt + rr->m_Header[ln].m_Cnt - 1].m_Crotch, rlist[i], rr, test);
        if(rr->m_Header == nullptr) return;
    }
}
...
{
    if(m_PlaceEmpty < 0) SetPlaceCnt(m_PlaceCnt + 256);
    int no = m_PlaceEmpty;
...
void CMatrixRoadNetwork::DeleteRegion(int no)
{
    ...
    --m_RegionCnt;
    if(m_RegionCnt <= 0) ClearRegion();
}

void CMatrixRoadNetwork::CalcNearRegion(int no)
{
    if(m_RegionCnt <= 2) return;
    ...
    while(clsme < clcnt)
    {
        ...
        for(i = 0; i < crotch->m_RoadCnt; ++i)
        {
            ...
            else if(crotch2->m_Data != no) // Принодлежит другому региону (мы ни будим рабоми рускай акодимичискай славестнасти, верно Даб?)
            {
                for(u = 0; u < rl_cnt; ++u) if(rl[u] == crotch2->m_Data) break;
                if(u >= rl_cnt)
                {
                    rl[rl_cnt] = crotch2->m_Data;
                    ++rl_cnt;
                }
            }
...
bool CMatrixRoadNetwork::IsNearestRegion(int reg1, int reg2)
{
    if(reg1 < 0 || reg2 < 0) return false;

    SMatrixRegion* region = GetRegion(reg1);

    for(int i = 0; i < region->m_NearCnt; ++i)
    {
        if(region->m_Near[i] == reg2) return true;
    }
...
int CMatrixRoadNetwork::FindNearestRegion(const CPoint& tp)
{
    ...
    int min_dist = tp.Dist2(m_Region[0].m_Center);
    int r = 0;
...
int CMatrixRoadNetwork::FindNearestRegionByRadius(const CPoint& tp, int cur_region)
{
    int i, r;
    ...
    if(cur_region >= 0)
    {
        ...
        if(POW2(m_Region[cur_region].m_RadiusPlace) >= m_Region[cur_region].m_Center.Dist2(tp)) return cur_region;

        //Проверяем ближайшие регионы
        for(i = 0; i < m_Region[cur_region].m_NearCnt; ++i)
        {
            r = m_Region[cur_region].m_Near[i];
...
{
    if(m_RegionCnt <= 1) return 0;
    if(rstart == rend) return 0;

    if(!m_RegionFindIndex) m_RegionFindIndex = (int*)HAlloc(m_RegionCnt * sizeof(int), m_Heap);
    ...
    while(sme < cnt)
    {
        ...
        if(sme >= next)
        {
            ...
            if(!ok && level > maxpath)
            {
                if(err)
                {
                    ERROR_S(L"CMatrixRoadNetwork::FindPathInRegionRun Error 1!");
                }
                else return 0;
            }
        }
    ...
    while(true)
    {
        ...
        if(nr_ < 0)
        {
            ERROR_S(L"CMatrixRoadNetwork::FindPathInRegionRun Error 2!");
        }
        ...
        if(!(cnt + 1 <= maxpath))
        {
            if(err)
            {
                ERROR_S(L"CMatrixRoadNetwork::FindPathInRegionRun Error 3!");
            }
            else return 0;
        }
...
void CMatrixRoadNetwork::InitPL(int map_size_x, int map_size_y)
{
    ClearPL();

    m_PLSizeX = (map_size_x >> m_PLShift) + ((map_size_x & m_PLMask) ? 1 : 0);
    m_PLSizeY = (map_size_y >> m_PLShift) + ((map_size_y & m_PLMask) ? 1 : 0);

    if(m_PlaceCnt <= 0 || m_PLSizeX <= 0 || m_PLSizeY <= 0) return;
    ...
    {
        for(int u = 0;u < region->m_PlaceCnt; ++u) region->m_Place[u] = changei[region->m_Place[u]];
        for(int u = 0;u < region->m_PlaceAllCnt; ++u) region->m_PlaceAll[u] = changei[region->m_PlaceAll[u]];
    }
    ...
    for(int i = 0; i < m_ZoneCnt; ++i)
    {
        for(int u = 0; u < m_Zone[i].m_PlaceCnt; ++u) m_Zone[i].m_Place[u] = changei[m_Zone[i].m_Place[u]];
    }
    for(int i = 0;i < m_PlaceCnt; ++i)
    {
        for(int u = 0; u < m_Place[i].m_NearCnt; ++u) m_Place[i].m_Near[u] = changei[m_Place[i].m_Near[u]];
    }

    HFree(changei, m_Heap);
}

CRect CMatrixRoadNetwork::CorrectRectPL(const CRect& map_coords)
{
    return CRect( max(0, map_coords.left >> m_PLShift),
                  max(0, map_coords.top >> m_PLShift),
                  min(m_PLSizeX, (map_coords.right >> m_PLShift) + 1),
                  min(m_PLSizeY, (map_coords.bottom >> m_PLShift) + 1) );
}

void CMatrixRoadNetwork::ActionDataPL(const CRect& map_coords, dword mask_and, dword mask_or)
{
    int sx = max(0, map_coords.left >> m_PLShift);
    int sy = max(0, map_coords.top >> m_PLShift);
    int ex = min(m_PLSizeX, (map_coords.right >> m_PLShift) + 1);
    int ey = min(m_PLSizeY, (map_coords.bottom >> m_PLShift) + 1);
    ...
    for(int y = sy; y < ey; y++, pl += m_PLSizeX - (ex - sx))
    {
        for(int x = sx; x < ex; ++x, ++pl)
        {
            ...
            for(int i = 0; i < pl->m_Cnt; ++i, ++mp) mp->m_Data = (mp->m_Data & mask_and) | mask_or;
        }
    }
}

int CMatrixRoadNetwork::FindInPL(const CPoint& map_pos)
{
    CRect plr = CorrectRectPL(CRect(map_pos.x - 4, map_pos.y - 4, map_pos.x + 4, map_pos.y + 4));
    ...
    for(int y = plr.top; y < plr.bottom; ++y, plist += m_PLSizeX - (plr.right - plr.left))
    {
        for(int x = plr.left; x < plr.right; ++x, ++plist)
        {
            ...
            {
                if(map_pos.x >= place->m_Pos.x && map_pos.y >= place->m_Pos.y && map_pos.x < (place->m_Pos.x + 4) && map_pos.y < (place->m_Pos.y + 4))
                {
...
void CMatrixRoadNetwork::Save(CBuf & b)
{
    ...
    for(int i=0;i<m_ZoneCnt;i++) {
        ...
        for(u=0;u<m_Zone[i].m_PlaceCnt;u++) b.Int(m_Zone[i].m_Place[u]);
    }
    ...
    for(i=0;i<m_PlaceCnt;i++) {
        ...
        for(u=0;u<9;u++) b.Bool(place->m_Blockade[u]);
    ...
    for(i=0;i<m_RegionCnt;i++) {
        ...
        for(u=0;u<region->m_CrotchCnt;u++) b.Int(region->m_Crotch[u]->m_Data);
        
        b.Byte(region->m_PlaceCnt);
        for(u=0;u<region->m_PlaceCnt;u++) b.Int(region->m_Place[u]);
        
		b.Byte(region->m_PlaceAllCnt);
        for(u=0;u<region->m_PlaceAllCnt;u++) b.Int(region->m_PlaceAll[u]);
...
void CMatrixRoadNetwork::Load(CBuf & b,int ver)
{
    ...
    for(int i=0;i<cnt;i++) {
        ...
        for(u=0;u<m_Zone[i].m_NearZoneCnt;u++) {
            ...
            if(ver>=25) m_Zone[i].m_NearZoneMove[u]=b.Byte();
        }
	    if(ver>=24) {
        ...
        if(ver>=27) {
            ...
            for(u=0;u<m_Zone[i].m_PlaceCnt;u++) m_Zone[i].m_Place[u]=b.Int();
        }
    ...
    if(ver>=21) {
        ...
        for(int i=0;i<cnt;i++) {
            ...
            for(u=0;u<9;u++) place->m_Blockade[u]=b.Bool();
            if(ver>=24) place->m_Cannon=b.Byte();
            else place->m_Cannon=0;
            if(place->m_Cannon) {
        ...
        for(int i=0;i<cnt;i++) {
            ...
            if(ver>=24) region->m_RadiusPlace=b.Int();
...
//                region->m_Crotch[u]->m_Region=i;
		    ...
		    if(ver>=24) {
				...
				for(u=0;u<region->m_PlaceAllCnt;u++) region->m_PlaceAll[u]=b.Int();
			}

            region->m_NearCnt=b.Byte();
            for(u=0;u<region->m_NearCnt;u++) {
                region->m_Near[u]=b.Int();
                if(ver>=26) region->m_NearMove[u]=b.Byte();
            }
