...
void CMatrixGroup::AddObject(CMatrixMapStatic* object, int team)
{
    ...
    while(objects)
    {
        if(objects->ReturnObject() == object) return;
...
void CMatrixGroup::RemoveObject(CMatrixMapStatic* object)
{
    ...
    while(g_object)
    {
        if(g_object->ReturnObject() == object)
        {
...
void CMatrixGroup::RemoveAll()
{
    ...
    {
        if(Objects->m_NextObject) Objects = Objects->m_NextObject;
		else
        {
		...
		if(Objects) HDelete(CMatrixGroupObject, Objects->m_PrevObject, g_MatrixHeap);
	}
...
void CMatrixGroup::FindNearObjects(CMatrixGroupObject* fn_object)
{
    ...
    while(gr_objects)
    {
        CMatrixMapStatic* ai_object = gr_objects->ReturnObject();

        if(ai_object->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
        {
        ...
        D3DXVECTOR2 fr(0, 0);
        D3DXVECTOR2 ar(0, 0);
        int* ai_gr = nullptr, *fn_gr = nullptr;
        
        if(fn_object->ReturnObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI)
        {
            fn_gr = ((CMatrixRobotAI*)fn_object->ReturnObject())->GetGroupP();
            fr = D3DXVECTOR2(((CMatrixRobotAI*)fn_object->ReturnObject())->m_PosX, ((CMatrixRobotAI*)fn_object->ReturnObject())->m_PosY);
        }
        else if(fn_object->ReturnObject()->GetObjectType() == OBJECT_TYPE_FLYER)
        {
            fn_gr = &((CMatrixFlyer*)fn_object->ReturnObject())->m_Group;
            fr = D3DXVECTOR2(((CMatrixFlyer*)fn_object->ReturnObject())->GetPos().x, ((CMatrixFlyer*)fn_object->ReturnObject())->GetPos().y);
        }
        ...
        {
            *ai_gr = *fn_gr;
            //ai_object->m_Group = fn_object->ReturnObject()->m_Group;
...
void CMatrixGroup::InstallTactics(TacticsType type, CBlockPar *par)
{
    ...
    if(m_Tactics != nullptr) DeInstallTactics();
...
void CMatrixGroup::LogicTact(CMatrixSideUnit *side)
{
...
void CMatrixGroup::RemoveBuildings()
{
    if(m_BuildingsCnt)
    {
        ...
        while(go)
        {
            if(go->ReturnObject()->GetObjectType() == OBJECT_TYPE_BUILDING)
            {
                CMatrixGroupObject *go2 = go->m_NextObject; 
                LIST_DEL(go, m_FirstObject, m_LastObject, m_PrevObject, m_NextObject);
                HDelete(CMatrixGroupObject, go, g_MatrixHeap);
...
void CMatrixGroup::SortFlyers()
{
    ...
    while(go->ReturnObject()->GetObjectType() == OBJECT_TYPE_FLYER)
    {
        LIST_DEL(go, m_FirstObject, m_LastObject, m_PrevObject, m_NextObject);
...
bool CMatrixGroup::FindObject(CMatrixMapStatic *object)
{
    ...
    while(objects)
    {
        if(objects->ReturnObject() == object) return true;
        objects = objects->m_NextObject;
    }
    return false;
}
...
int CMatrixGroup::GetBombersCnt()
{
    ...
    while(objects)
    {
        if(objects->ReturnObject()->IsRobotAlive())
        {
            if(objects->ReturnObject()->AsRobot()->FindBombWeapon()) ++cnt;
        }
        objects = objects->m_NextObject;
    }
...
int CMatrixGroup::GetRepairsCnt()
{
    ...
    while(objects)
    {
        if(objects->ReturnObject()->IsRobotAlive())
        {
            if(objects->ReturnObject()->AsRobot()->FindRepairWeapon()) ++cnt;
        }
        objects = objects->m_NextObject;
    }
...
CMatrixGroupList::~CMatrixGroupList()
{
    ...
    while(groups != nullptr)
    {
        ...
        if(groups) HDelete(CMatrixGroup, groups->m_PrevGroup, g_MatrixHeap);
    }
}
...
{
    if(m_GroupsCnt > 0 && m_ReGroupPeriod < REGROUP_PERIOD) return;
    ...
    while (objects) {
        if (objects->GetObjectType() == OBJECT_TYPE_ROBOT_AI && ((CMatrixRobotAI*)objects)->m_Side == side->m_Id) {
    ...
    for (int cnt = 0; cnt < 3; cnt++) {
        ...
        while (gr_objects) {
            if (gr_objects->GetTeam() == team) {
                CMatrixMapStatic* ai_object = gr_objects->ReturnObject();
                int ai_group = 0;
                if (ai_object->GetObjectType() == OBJECT_TYPE_ROBOT_AI && ((CMatrixRobotAI*)ai_object)->GetGroup() == 0) {
    ...
    while (groups) {
        ...
        while (objects) {
            int ob_group = 0;
            if (objects->ReturnObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI) {
                ob_group = ((CMatrixRobotAI*)objects->ReturnObject())->GetGroup();
            }
            else if (objects->ReturnObject()->GetObjectType() == OBJECT_TYPE_FLYER) {
                ob_group = ((CMatrixFlyer*)objects->ReturnObject())->m_Group;
            }
            ...
            if (ob_group != groups->GetGroupId()) {
                ...
                if (groups->GetTactics()) {
                    groups->GetTactics()->RemoveObjectFromT(rb->ReturnObject());
                }

                groups->RemoveObject(rb->ReturnObject());
                continue;
            }
            objects = objects->m_NextObject;
        }
...
void CMatrixGroupList::LogicTact(CMatrixSideUnit* side)
{
    m_ReGroupPeriod++;
    if (m_GroupsCnt == 0)
        return;
...
{
    CMatrixGroup* grouppies = m_FirstGroup, * group = nullptr;
    while (grouppies) {
    ...
    if (group == nullptr) group = AddNewGroup(team, id);
