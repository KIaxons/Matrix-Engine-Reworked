...
DTRACE();
    ...
    if(group->GetRobotsCnt() < min_cells || group->GetRobotsCnt() > max_cells) return false;
...
DTRACE();
    ...
    while(tmpsl && Robots > 0)
    {
        ...
        {
            if(group_objects->GetObject()->GetObjectType() != OBJECT_TYPE_ROBOT_AI)
            {
                group_objects = group_objects->m_NextObject;
                continue;
            }

            if(!FindRobot((CMatrixRobotAI*)group_objects->GetObject()))//!tmpsl->FindRobot(group_robots->CreateRobotByTemplate()))
            {
...
void CMatrixTactics::LogicTact(CMatrixSideUnit* side, CMatrixGroup* group)
{
    DTRACE();
    ...
    if (m_Type == CAPTURE_TACTICS && m_Target && m_Target->GetObjectType() == OBJECT_TYPE_BUILDING && ((CMatrixBuilding*)m_Target)->m_Side == side->m_Id && ((CMatrixBuilding*)m_Target)->IsBase()) {
        ...
        while (gr_objects) {
            if (gr_objects->GetObject() && gr_objects->GetObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI) {
    ...
    while (tmpsl) {
        ...
        while (logic_bot) {
            ...
            while (rule && !(g_MatrixMap->GetPlayerSide()->GetUnitUnderManualControl() == (CMatrixMapStatic*)(logic_bot->GetLogicRobot()))) {
                rule->LogicTact();
...
void CMatrixTactics::CalcNearestToTargetBot()
{
    ...
    CMatrixLogicSlot* slots = m_FirstSlot;
    CLogicSlotRobot* nearestBot = nullptr;
    float               nearestDist = 0;
    ...
    while (slots) {
        ...
        while (robots) {
            ...
            if (a < nearestDist || nearestBot == nullptr) {
                nearestDist = a;
                nearestBot = robots;
            }
