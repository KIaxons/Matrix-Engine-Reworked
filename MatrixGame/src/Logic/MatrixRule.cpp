...
//    if(m_Condition.m_DestroyedSlot != nullptr){
//        return false;
//    }
//
...
//                while(gr_objects){
//                    if(gr_objects->GetObject()->GetObjectType() == OBJECT_TYPE_ROBOT_AI){
...
//    }
//    if(m_Action.m_GoTo != nullptr){
//    }
//    if(m_Action.m_MoveTo != MOVETO_UNDEF){
//        if((m_Action.m_MoveTo & MOVETO_TARGETZONE) == MOVETO_TARGETZONE && parent_tactics->GetTarget() != nullptr){
...
//
//                g_MatrixMap->PlaceFindNear(robot->m_Module[0].m_Kind, 4, mx, my, robot);
//                robot->MoveTo(mx, my);
//            }
...
//            if((m_AroundPeriod >= AROUNDENEMY_PERIOD || m_AroundPeriod == 0) && robot->GetEnv()->m_Target && robot->GetEnv()->m_Target->GetObjectType() == OBJECT_TYPE_ROBOT_AI && !robot->FindOrderLikeThat(ROT_MOVE_TO)){
//                m_AroundPeriod = 0;
//
//                float ex = 0, ey = 0;
...
//
//            CPoint tp;
//            if(!robot->GetMoveToCoords(tp) || g_MatrixMap->m_RoadNetwork.FindNearestRegion(tp)!=parent_tactics->GetRegion()) {
//                tp=g_MatrixMap->m_RoadNetwork.m_Region[parent_tactics->GetRegion()].m_Center;
...
//            if(parent_tactics->GetTarget() != nullptr) robot->CaptureBuilding((CMatrixBuilding*)parent_tactics->GetTarget());
//        }
//    }
//    if(m_Action.m_Exit == EXIT){
//    }
