...
void CMatrixConfig::SetDefaults()
{
...
void CMatrixConfig::Clear()
{
    if(m_Cursors)
    {
        for(int i = 0; i < m_CursorsCnt; ++i) m_Cursors[i].~SStringPair();
...
void CMatrixConfig::ReadParams()
{
    ...
    for(int i = 0; i < m_CursorsCnt; ++i)
    {
        ...
        //m_Cursors[i].SStringPair::SStringPair();
    ...
    */
    ...
    {
        m_DrawAllObjectsToMinimap = (byte)(cfg_par->Par(CFG_OBJECTS_TO_MINIMAP).GetInt() & 0xFF);
    }
    ...
    //Маркеры для активации теней
    //Роботы и вертолёты
    if(cfg_par->ParCount(CFG_UNIT_SHADOWS))
    {
        if(cfg_par->Par(CFG_UNIT_SHADOWS).GetBool()) m_UnitShadows = SHADOW_STENCIL;
        else m_UnitShadows = SHADOW_OFF;
    }
    //Здания и турели
    if(cfg_par->ParCount(CFG_BUILDING_SHADOWS))
    {
        if(cfg_par->Par(CFG_BUILDING_SHADOWS).GetBool()) m_BuildingShadows = SHADOW_STENCIL;
        else m_BuildingShadows = SHADOW_OFF;
    }
    //Всё прочее
    if(cfg_par->ParCount(CFG_OBJECT_SHADOWS))
    {
        if(cfg_par->Par(CFG_OBJECT_SHADOWS).GetBool()) m_ObjectShadows = SHADOW_PROJ_STATIC;
        else m_ObjectShadows = SHADOW_OFF;
    }
    ...
    for(int i = 0; i <= m_RobotChassisCount; ++i)
    {
        ...
        m_RobotChassisConsts[i].anim_move_speed = chars->ParGetNE(L"MovingAnimationSpeed").GetFloat();
        ...
        const CWStr* moving_anim = &chars->ParGetNE(L"MovingAnimationType");
        if(*moving_anim == L"SpeedDependent") m_RobotChassisConsts[i].moving_anim_type = SPEED_DEPENDENT_MOVING_ANIM;
        else if(*moving_anim == L"Off") m_RobotChassisConsts[i].moving_anim_type = NO_MOVING_ANIM;
        else /*if(*moving_anim == L"Normal")*/ m_RobotChassisConsts[i].moving_anim_type = NORMAL_MOVING_ANIM;
        ...
        if(block_jet_streams)
        {
            ...
            byte jet_matrix = block_jet_streams->ParGetNE(buf).GetInt();
            while(jet_matrix)
            {
                SRobotSpriteAnims jet_stream;
                ...
                swprintf(buf, 32, L"Stream%i_Width", j);
                jet_stream.width = block_jet_streams->ParGet(buf).GetFloat();

                swprintf(buf, 32, L"Stream%i_Color", j);
                const CWStr* par = &block_jet_streams->ParGetNE(buf);
                if(!par->IsEmpty()) jet_stream.color = RGBAStringToABGRColorHEX(par);
        ...
        //Для шасси, которым нужно добавить дополнительные спрайтовые анимации
        CBlockPar* block_sprite_anim = chars->BlockGetNE(L"SpriteAnimations");
        if(block_sprite_anim)
        {
            int j = 1;
            wchar_t buf[32];
            swprintf(buf, 32, L"Anim%i_MatrixId", j);

            byte sprite_matrix = block_sprite_anim->ParGetNE(buf).GetInt();
            while(sprite_matrix)
            {
                SRobotSpriteAnims sprite_anim;
                sprite_anim.matrix_id = sprite_matrix;
                swprintf(buf, 32, L"Anim%i_Sprites", j);
                const CWStr* sprites = &block_sprite_anim->ParGet(buf);
                sprite_anim.sprites_name = sprites->GetStrPar(0, L":");
                if(sprites->GetCountPar(L":") > 1) sprite_anim.sprites_count = min(sprites->GetIntPar(1, L":"), 255);
                else sprite_anim.sprites_count = 1;

                swprintf(buf, 32, L"Anim%i_Length", j);
                sprite_anim.length = block_sprite_anim->ParGet(buf).GetFloat();

                swprintf(buf, 32, L"Anim%i_Width", j);
                sprite_anim.width = block_sprite_anim->ParGet(buf).GetFloat();

                swprintf(buf, 32, L"Anim%i_FramesPerSecond", j);
                sprite_anim.frame_delay = max(1000 / block_sprite_anim->ParGet(buf).GetInt(), 1);

                swprintf(buf, 32, L"Anim%i_Color", j);
                const CWStr* par = &block_sprite_anim->ParGetNE(buf);
                if(!par->IsEmpty()) sprite_anim.color = RGBAStringToABGRColorHEX(par);

                swprintf(buf, 32, L"Anim%i_CameraOriented", j);
                par = &block_sprite_anim->ParGetNE(buf);
                if(!par->IsEmpty()) sprite_anim.camera_oriented = par->GetBool();

                m_RobotChassisConsts[i].linked_sprite_anim.push_back(sprite_anim);
                swprintf(buf, 32, L"Anim%i_MatrixId", ++j);
                sprite_matrix = block_sprite_anim->ParGetNE(buf).GetInt();
            }
        }
    ...
    if(bp_tmp)
    {
        ...
        for(int i = 0; i < cnt; ++i)
        {
            const CWStr& name = bp_tmp->ParGetName(i);
            if(name == PAR_SOURCE_CAMERA_BASEANGLEZ) m_CamBaseAngleZ = GRAD2RAD(bp_tmp->ParGet(i).GetFloat());
            else if(name == PAR_SOURCE_CAMERA_ARCADE_FWD0) m_CamInRobotForward0 = bp_tmp->ParGet(i).GetFloat();
            else if(name == PAR_SOURCE_CAMERA_ARCADE_FWD1) m_CamInRobotForward1 = bp_tmp->ParGet(i).GetFloat();
            else if(name == PAR_SOURCE_CAMERA_MOVE_SPEED) m_CamMoveSpeed = bp_tmp->ParGet(i).GetFloat();
            else if(name == PAR_SOURCE_CAMERA_MOVE_SPEED_UP) m_CamMoveSpeedUp = bp_tmp->ParGet(i).GetFloat();
            else if(name == PAR_SOURCE_CAMERA_FOV) g_CamFieldOfView = bp_tmp->ParGet(i).GetFloat();
        }
        ...
        for(int i = 0; i < cnt; ++i)
        {
            ...
            if(index >= 0)
            {
                ...
                {
                    const CWStr& name = bp_c->ParGetName(j);
                    if(name == PAR_SOURCE_CAMERA_ROTSPEEDX) m_CamParams[index].m_CamRotSpeedX = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_ROTSPEEDZ) m_CamParams[index].m_CamRotSpeedZ = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_WHEELSTEP) m_CamParams[index].m_CamMouseWheelStep = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_ROTANGLEMIN) m_CamParams[index].m_CamRotAngleMin = GRAD2RAD(min(94.0f, bp_c->ParGet(j).GetFloat()));
                    else if(name == PAR_SOURCE_CAMERA_ROTANGLEMAX) m_CamParams[index].m_CamRotAngleMax = GRAD2RAD(bp_c->ParGet(j).GetFloat());
                    else if(name == PAR_SOURCE_CAMERA_DISTMIN) m_CamParams[index].m_CamDistMin = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_DISTMAX) m_CamParams[index].m_CamDistMax = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_DISTPARAM) m_CamParams[index].m_CamDistParam = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_ANGLEPARAM) m_CamParams[index].m_CamAngleParam = bp_c->ParGet(j).GetFloat();
                    else if(name == PAR_SOURCE_CAMERA_HEIGHT) m_CamParams[index].m_CamHeight = bp_c->ParGet(j).GetFloat();
                }
            }
        }
    }
    ...
    for(int i = 0; i < KA_LAST; ++i)
    {
        ...
        if(par != nullptr)
        {
            ...
            if(kk == -1)
            {
                WARNING_S3(L"Key binded to \"", CWStr(key_action_codes[i].name), L"\" was not found! Reassign is unavailable.");
                continue;
            }
