...
bool CMatrixShadowProj::AlreadyDraw()
{
    if(m_DrawFrame == g_MatrixMap->GetCurrentFrame()) return true;
...
byte* CMatrixShadowProj::ReadGeometry(byte* raw)
{
    ...
    m_VertCnt = *(int*)raw; raw += sizeof(int);
...
static void ShadowProjBuildGeomInt(CVOShadowProj& sp, const SProjData& pd, const D3DXMATRIX& objma, int mapradius, bool join_to_group)
{
    ...
    try
    {
        buff = (byte*)_alloca((sizeof(STempVertex) + sizeof(SVOShadowProjVertex) + sizeof(word) * 3) * da_size);
    }
    catch (...)
    {
        ERROR_S(L"Not enough memory for buffer after _alloca() call in ShadowProjBuildGeomInt function!");
    }
...
void ShadowProjBuildGeom(CVOShadowProj& sp, CVectorObjectAnim& obj, int noframe, const D3DXMATRIX& objma, const D3DXMATRIX& iobjma, D3DXVECTOR3& light, int mapradius, bool join_to_group)
{
    SProjData   pd;
    D3DXVECTOR3 lv;
...
void ShadowProjBuildGeomList(CVOShadowProj& sp, int cnt, CVectorObjectAnim** obj, const int* noframe, const D3DXMATRIX* wm, const D3DXMATRIX& objma, const D3DXMATRIX& iobjma, D3DXVECTOR3& light, int mapradius, bool join_to_group)
{
    SProjData   pd;
    D3DXVECTOR3 lv;
...
void ShadowProjBuildFull(CVOShadowProj& sp, CVectorObjectAnim& obj, int noframe, const D3DXMATRIX& objma, const D3DXMATRIX& iobjma, D3DXVECTOR3& light, int mapradius, int texsize, bool doclip, bool render_texture, bool join_to_group)
{
    SProjData   pd;
    D3DXVECTOR3 lv;
    ...
    CTextureManaged* tex;
    CMatrixShadowCliper* clipper = nullptr;
    ...
    if(obj.VO()->GetFramesCnt() > 1)
    {
        ...
        CVectorObject::CalcShadowTexture(1, &pobj, &noframe, nullptr, pd, texsize, clipper, tex);
        tex->RefInc();
        sp.SetTexture(tex);
    }
    else
    {
        tex = CVectorObject::CalcShadowTexture(1, &pobj, &noframe, nullptr, pd, texsize, clipper);
        tex->RefInc();
        sp.SetTexture(tex);
    }
}

void ShadowProjBuildTexture(CVOShadowProj& sp, CVectorObjectAnim& obj, int noframe, const D3DXMATRIX& iobjma, D3DXVECTOR3& light, int texsize, bool doclip)
{
    SProjData   pd;
	D3DXVECTOR3 lv;
    ...
    if (obj.VO()->GetFramesCnt() > 1)
    {
        ...
        {
            //CTexture *tex = CACHE_CREATE_TEXTURE();
            CVectorObject::CalcShadowTexture(1,&pobj,&noframe,nullptr,pd,texsize,clipper,sp.GetTexture());
            //sp.SetTexture(tex);
        } else
        {
            CTexture *tex = CACHE_CREATE_TEXTURE();
    ...
    {
        CTextureManaged *tex = CVectorObject::CalcShadowTexture(1,&pobj,&noframe,nullptr,pd,texsize,clipper);
        tex->RefInc();
        sp.SetTexture(tex);
    }
...
void ShadowProjBuildTexture(CMatrixMapObject *mo, CVOShadowProj & sp,CVectorObjectAnim & obj,int noframe,const D3DXMATRIX & iobjma,int texsize, bool doclip)
{
    DTRACE();
    ...
    if (obj.VO()->GetFramesCnt() > 1)
    {
        ...
        {
            //CTexture *tex = CACHE_CREATE_TEXTURE();
            CVectorObject::CalcShadowTextureWOMat(1,&pobj,&noframe,nullptr,texsize,clipper,sp.GetTexture());
            //sp.SetTexture(tex);
        } else
        {
            CTexture *tex = CACHE_CREATE_TEXTURE();
            CVectorObject::CalcShadowTextureWOMat(1,&pobj,&noframe,nullptr,texsize,clipper,tex);
            if (tex) tex->RefInc();
            sp.SetTexture(tex);
        }
    ...
    {
        CTextureManaged *tex = CVectorObject::CalcShadowTextureWOMat(1,&pobj,&noframe,nullptr,texsize,clipper);
        if (tex) tex->RefInc();
        sp.SetTexture(tex);
    }
...
void ShadowProjBuildTextureList(CVOShadowProj & sp,int cnt,CVectorObjectAnim * * obj,CTexture * tex,int * noframe,D3DXMATRIX * wm,const D3DXMATRIX & objma,const D3DXMATRIX & iobjma,D3DXVECTOR3 & light,int texsize)
{
    DTRACE();
    ...
    CVectorObject::CalcShadowTexture(cnt,obj,noframe,wm,pd,texsize,nullptr,tex);
	sp.SetTexture(tex);
...
void ShadowProjBuildFullList(CVOShadowProj & sp,int cnt,CVectorObjectAnim * * obj,CTexture * tex,const int * noframe,const D3DXMATRIX * wm,const D3DXMATRIX & objma,const D3DXMATRIX & iobjma,D3DXVECTOR3 & light,int mapradius,int texsize, bool join_to_group)
{
    DTRACE();
