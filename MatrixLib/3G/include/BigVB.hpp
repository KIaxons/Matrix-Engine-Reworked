...
template <class V> class CBigVB;
template <class V> struct SBigVBOne;

template <class V> struct SBigVBSource
{
private:
     // do not modify these!!!
    int one = -1;   // index of SBigVBOne in SBigVB
    int part = 0;   // index of part in SBigVBOne
    int base = 0;   // index of vert in SBigVBOne

public:
    friend class CBigVB<V>;
    friend struct SBigVBOne<V>;

    SBigVBSource() = default;
...
template <class V> struct SBigVBPart
{
    SBigVBSource<V>* source = nullptr;
    dword           flags = 0;
};

template <class V> struct SBigVBOne
{
    SBigVBPart<V>* parts = nullptr;
    int         count = 0;
    int         noneedcount = 0;
    int         vbsize = 0; // in bytes
    D3D_VB      vb;

    void PrepareVB(void);
    void SetOneIndex(int one) { for(int i = 0; i < count; ++i) parts[i].source->one = one; }
};

template <class V> class CBigVB : public Base::CMain
{
    CHeap          *m_Heap; 
    ...
    CBigVB(CHeap *heap):CMain(),m_VB_array(nullptr), m_VB_count(0), m_LastOne(-1), m_Heap(heap) {};
    ~CBigVB() {};
public:

    friend struct SBigVBSource<V>;

    static CBigVB<V> *NewBigVB(CHeap *heap)
    {
        return HNew(heap) CBigVB<V>(heap);
    }
    static void       DestroyBigVB(CBigVB<V> *bvb)
    {
        HDelete(CBigVB<V>,bvb, bvb->m_Heap);
    }
...
template <class V> inline void  SBigVBOne<V>::PrepareVB(void)
{
    if (IS_VB(vb)) return;
...
template <class V> inline int  SBigVBSource<V>::Select(CBigVB<V> *bvb)
{
...
template <class V> inline bool  SBigVBSource<V>::IsDirty(CBigVB<V> *bvb)
{
    return FLAG(bvb->m_VB_array[one].parts[part].flags, BVB_DIRTY_PART);
}

template <class V> inline bool  SBigVBSource<V>::IsMarkedNoNeed(CBigVB<V> *bvb)
{
    return FLAG(bvb->m_VB_array[one].parts[part].flags, BVB_NONEED_PART);
}

template <class V> inline bool  SBigVBSource<V>::Prepare(CBigVB<V> *bvb, int stl)
{
    DTRACE();
#ifdef _DEBUG
    ...
    if (IsDirty(bvb))
    {
        ...
        bvbo->PrepareVB();
        if (bvbo->vb == nullptr) return true;
        V * v;
...
template <class V> inline bool  SBigVBSource<V>::Prepare(CBigVB<V> *bvb)
{
    DTRACE();
#ifdef _DEBUG
    bvb->m_Preparewas = true;
...
template <class V> inline void  SBigVBSource<V>::Invalidate(CBigVB<V> *bvb)
{
    MarkNoNeed(bvb);
    if (!IsDirty(bvb))
    {
...
template <class V> inline void  SBigVBSource<V>::MarkNoNeed(CBigVB<V> *bvb)
{
...
template <class V> inline void CBigVB<V>::ReleaseBuffers(void)
{
    ...
    for (int i=0;i<m_VB_count;++i)
    {
        ...
        if (bvbo->vb) DESTROY_VB(bvbo->vb);
    }

}

template <class V> inline void CBigVB<V>::AddSource(SBigVBSource<V> *src)
{
    int index;
    ...
    } else
    {
        ...
        for (i=0; i<m_VB_count; ++i)
        {
            if (m_VB_array[i].vbsize <= (BVB_MAX_SIZE-src->size))
            {
                ...
                m_VB_array[i].parts = (SBigVBPart<V> *)HAllocEx(m_VB_array[i].parts, sizeof(SBigVBPart<V>) * (cc+1), m_Heap);
...
template <class V> __inline bool CBigVB<V>::DelSource(SBigVBSource<V> *src) // return true if BigVB has destroyed
{
    ...
    if (nn > 0)
    {
        ...
        if ((nn + m_VB_array[src->one].noneedcount) == m_VB_array[src->one].count)
        {
            ...
            ASSERT(m_VB_array[src->one].vb != nullptr); 
    ...
    if (m_VB_array[src->one].count == 0)
    {
        ...
        if (m_VB_array[src->one].vb) {DESTROY_VB(m_VB_array[src->one].vb);}
...
template <class V> inline void CBigVB<V>::PrepareAll(void)
{
    for (int i=0; i<m_VB_count; ++i)
    {
        for (int j=0; j<m_VB_array[i].count; ++j)
