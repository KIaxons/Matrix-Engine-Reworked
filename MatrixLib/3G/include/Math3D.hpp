...
#define RND(from, to) ((rand() * (1.0f / (RAND_MAX)) * fabs(to - from)) + from)
#define FRND(x)       (RND(0.0f, x))
#define FSRND(x)      (FRND(2.0f * x) - x)
#define IRND(n)       int(RND(0.0f, (float)n - 0.55f))
...
struct SPlane
{
    ...
    static inline void BuildFromPoints(SPlane& out, const D3DXVECTOR3& p0, const D3DXVECTOR3& p1, const D3DXVECTOR3& p2)
    {
        D3DXPlaneFromPoints(&out.dxplane, &p0, &p1, &p2);
        out.UpdateSignBits();
    }

    inline void  UpdateSignBits(void) { signbits = ((norm.x < 0) ? 1 : 0) | ((norm.y < 0) ? 2 : 0) | ((norm.z < 0) ? 4 : 0); }
...
inline double AngleNorm(double a) // Нормировать угол. Возвращает от -pi до +pi
{
...
inline double AngleDist(double a, double b) // Дистанция между углами. Возвращает от -pi до +pi
{
    /*
    ...
    double ang = b - a;
    double mod_div1 = ang + GRAD2RAD(180);
    double mod_div2 = GRAD2RAD(360);
    ang = (mod_div1 - std::floor(mod_div1 / mod_div2) * mod_div2) - GRAD2RAD(180);

    return ang;

    */

    //Возвращаем угол в интервале от -pi * 2 до +pi * 2
    double dist = fmod(b - a, 2.0 * M_PI);
    return AngleNorm(dist);
}
...
inline float GetColorA(dword c) { return float((c >> 24) & 255) / 255.0f; }
inline float GetColorR(dword c) { return float((c >> 16) & 255) / 255.0f; }
inline float GetColorG(dword c) { return float((c >> 8) & 255) / 255.0f; }
inline float GetColorB(dword c) { return float((c >> 0) & 255) / 255.0f; }
...
public:
    ...
    inline bool NeedContinue(void) const
    {
        return m_CurSeg >= (m_SegCnt - 2);
    }

    inline void GetCurPos(D3DXVECTOR3& pos)
    {
