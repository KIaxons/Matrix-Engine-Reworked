...
public:
	...
	int m_Len = 0;	       // Кол-во данных
	int m_Max = 0;	       // Размер буфера
	int m_Add = 32;        // Насколько увеличивается буфер
	int m_Pointer = 0;     // Указатель
	byte* m_Buf = nullptr; // Буфер

	CBuf(CHeap* heap = nullptr, int add = 32) : CMain(), m_Heap(heap), m_Add(add) {}
	~CBuf() { ClearFull(); }
	...
	template <class D> D* Buff(void)    { return (D*)m_Buf; }
	template <class D> D* BuffEnd(void) { return (D*)(m_Buf + m_Len); }
    int Len(void) const					{ return m_Len; }
	void Len(int zn);
    void SetLenNoShrink(int len)        { ASSERT(len <= m_Max); m_Len = len;}

	inline void TestGet(int len)		{ if((m_Pointer+len)>m_Len) ERROR_S(L"TestGet Error!"); }
	...
	void Pointer(int zn) { if((zn < 0) || (zn > m_Len)) { ERROR_S(L"Pointer Error!"); } m_Pointer = zn; }
	...
	char Char(void)				   { TestGet(sizeof(char)); m_Pointer += sizeof(char); return *(char*)(m_Buf + m_Pointer - sizeof(char)); }
	word Word(void)				   { TestGet(sizeof(word)); m_Pointer += sizeof(word); return *(word*)(m_Buf + m_Pointer - sizeof(word)); }
	short Short(void)			   { TestGet(sizeof(short)); m_Pointer += sizeof(short); return *(short*)(m_Buf + m_Pointer - sizeof(short)); }
	dword Dword(void)			   { TestGet(sizeof(dword)); m_Pointer += sizeof(dword); return *(dword*)(m_Buf + m_Pointer - sizeof(dword)); }
	long Long(void)				   { TestGet(sizeof(long)); m_Pointer += sizeof(long); return *(long*)(m_Buf + m_Pointer - sizeof(long)); }
	int Int(void)				   { TestGet(sizeof(int)); m_Pointer += sizeof(int); return *(int*)(m_Buf + m_Pointer - sizeof(int)); }
	int64 Int64(void)			   { TestGet(sizeof(int64)); m_Pointer += sizeof(int64); return *(int64*)(m_Buf + m_Pointer - sizeof(int64)); }
	float Float(void)			   { TestGet(sizeof(float)); m_Pointer += sizeof(float); return *(float*)(m_Buf + m_Pointer - sizeof(float)); }
	double Double(void)			   { TestGet(sizeof(double)); m_Pointer += sizeof(double); return *(double*)(m_Buf + m_Pointer - sizeof(double)); }

    template <class D> D Any(void) { DTRACE(); TestGet(sizeof(D)); m_Pointer += sizeof(D); return *(D*)(m_Buf + m_Pointer-sizeof(D)); }
    template <class D> const D& AnyStruct(void) { DTRACE(); TestGet(sizeof(D)); m_Pointer += sizeof(D); return *(D*)(m_Buf + m_Pointer-sizeof(D)); }

	void Bool(bool zn)			   { TestAdd(sizeof(bool)); *(bool*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(bool); }
	void Byte(byte zn)			   { TestAdd(sizeof(byte)); *(byte*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(byte); }
	void Char(char zn)		       { TestAdd(sizeof(char)); *(char*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(char); }
	void Word(word zn)			   { TestAdd(sizeof(word)); *(word*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(word); }
	void Short(short zn)		   { TestAdd(sizeof(short)); *(short*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(short); }
	void Dword(dword zn)		   { TestAdd(sizeof(dword)); *(dword*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(dword); }
	void Long(long zn)			   { TestAdd(sizeof(long)); *(long*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(long); }
	void Int(int zn)			   { TestAdd(sizeof(int)); *(int*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(int); }
	void Int64(int64 zn)		   { TestAdd(sizeof(int64)); *(int64*)(m_Buf + m_Pointer) = zn; m_Pointer += sizeof(int64); }
