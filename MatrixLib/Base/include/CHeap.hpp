...
//#define FORCE_ENABLE_MEM_SPY //Тут можно подключить MemSpy даже без сборки дебаг-версии
...
#define DPTR_MEM_SIGNATURE_INIT(size) { __dead_ptr_signature[0] = DPTR_SIGNATURE0; __dead_ptr_signature[1] = DPTR_SIGNATURE1; __dead_ptr_signature[2] = DPTR_SIGNATURE2; __dead_ptr_signature[3] = size;}
...
namespace DeadPtr
{
    ...
    {
        int *signature = (int*)s;
        return signature[0] == DPTR_SIGNATURE0 &&
               signature[1] == DPTR_SIGNATURE1 &&
               signature[2] == DPTR_SIGNATURE2;
    }
...
__inline int CheckValidPtr(const void* ptr)
{
    ...
    if(((dword)ptr) < ((dword)si.lpMinimumApplicationAddress)) return -1;
    if(((dword)ptr) > ((dword)si.lpMaximumApplicationAddress)) return -1;

    VirtualQuery(ptr, &mbi, sizeof(mbi));
    if(mbi.State == MEM_FREE) return -1;
...
DTRACE();
    ...
    SMemHeader* h = nullptr;
    if(buf != nullptr) { h = SMemHeader::CalcBegin(buf); h->Release(); }
    uint sz = SMemHeader::CalcSize(size);
...
{
    void* buf;
    if(this == nullptr) buf = HeapAlloc(GetProcessHeap(), 0, size);
    else buf = HeapAlloc(m_Heap, m_Flags, size);

    if(buf == nullptr) AllocationError(size);
    return buf;
}
...
{
    void* buf;
    if(this == nullptr) buf = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size);
    else buf = HeapAlloc(m_Heap, m_Flags | HEAP_ZERO_MEMORY, size);
...
{
    if(this == nullptr) buf = HeapReAlloc(GetProcessHeap(), 0, buf, size);
    else buf = HeapReAlloc(m_Heap, m_Flags, buf, size);
...
{
    if(this == nullptr) buf = HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, buf, size);
    else buf = HeapReAlloc(m_Heap, m_Flags | HEAP_ZERO_MEMORY, buf, size);
...
inline void* CHeap::AllocEx(void* buf, size_t size)
{
    if(this == nullptr)
    {
        ...
        {
            if((buf = HeapReAlloc(GetProcessHeap(), 0, buf, size)) == nullptr) AllocationError(size);
        }
        else if((buf = HeapAlloc(GetProcessHeap(), 0, size)) == nullptr) AllocationError(size);
    }
    else
    {
        if(size <= 0 && buf != nullptr)
        ...
        {
            if((buf = HeapReAlloc(m_Heap, m_Flags, buf, size)) == nullptr) AllocationError(size);
        }
        else if((buf = HeapAlloc(m_Heap, m_Flags, size)) == nullptr) AllocationError(size);
    }
    return buf;
}
...
inline void* CHeap::AllocClearEx(void* buf, size_t size)
{
    if(this == nullptr)
    {
        ...
        {
            if((buf = HeapReAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, buf, size)) == nullptr) AllocationError(size);
        }
        else if((buf = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size)) == nullptr) AllocationError(size);
    }
    else
    {
        if(size <= 0 && buf != nullptr)
        ...
        {
            if((buf = HeapReAlloc(m_Heap, m_Flags | HEAP_ZERO_MEMORY, buf, size)) == nullptr) AllocationError(size);
        }
        else if((buf = HeapAlloc(m_Heap, m_Flags | HEAP_ZERO_MEMORY, size)) == nullptr) AllocationError(size);
    }
    return buf;
}
...
inline void CHeap::Free(void* buf)
{
    ...
    if(this == nullptr) HeapFree(GetProcessHeap(), 0, buf);
    else HeapFree(m_Heap, m_Flags, buf);
}

}

//inline void* operator new (size_t size) { return Base::Alloc(size, nullptr); }
//inline void operator delete (void* buf) { Base::Free(buf, nullptr); }


//lint -e1532
#ifdef MEM_SPY_ENABLE
inline BASE_API void* operator new (size_t size, const char* file, int line, Base::CHeap* heap) { return heap->Alloc(size, file, line); }
inline BASE_API void operator delete (void* buf, const char* file, int line, Base::CHeap* heap) { heap->Free(buf, file, line); }
#else
inline BASE_API void* operator new (size_t size, Base::CHeap* heap) { return heap->Alloc(size); }
inline BASE_API void operator delete (void* buf, Base::CHeap* heap) { heap->Free(buf); }
#endif
//lint +e1532
...
#define HDelete(aclass, buf, heap) { (buf)->~aclass(); operator delete((buf), __FILE__, __LINE__, (Base::CHeap*)(heap)); }
...
#define HNew(heap) new
#define HDelete(aclass, buf, heap) delete buf

//#define HNew(heap) new((Base::CHeap*)(heap))
//#define HDelete(aclass, buf, heap) { (buf)->~aclass(); operator delete((buf), ((Base::CHeap*)(heap))); }
...
{
    return !memcmp(s1, s2, len * sizeof(wchar_t));
}
