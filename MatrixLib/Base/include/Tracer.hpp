...
inline unsigned __int64 GetCPUTact(void)
{
...
#define SLOG(n,s) {FILE *f = fopen(n,"a"); fwrite(s, strlen(s), 1, f); fclose(f);}

#ifdef _DEBUG

#define DCNT(s) {static int ccc=1; CDText::T(s, CStr(ccc++)); }
...
#define DCPO_START() { g_CheckPointNumber = 0; }
#define DCPO(x) { if ((x)==(g_CheckPointNumber+1))  {  g_CheckPointNumber=x; } else if ((x)!=g_CheckPointNumber)  { __asm int 3 }; }
...
#define DTRACE()  /*lint -e1025 -e1703*/  CDebugTracer      call_trace_profiler(__FILE__, __LINE__, __FUNCSIG__, false ) /*lint +e1025 +e1703*/
#define DCP()     /*lint -e1025 -e1703*/  CDebugTracer      UNIQID(check_point)(__FILE__, __LINE__, __FUNCSIG__, true)   /*lint +e1025 +e1703*/
#else
...
//lint -e773
#define TAKT_BEGIN()    unsigned __int64 t___takt = GetCPUTact()
//lint +e773
#define TAKT_END(ss)    unsigned __int64 t___takt2 = GetCPUTact(); \
                        static int t___i = 0;   static int t___smooth[64]; \
                        t___smooth[t___i] = int(t___takt2 - t___takt); \
                        t___i = (t___i+1) & 63; int t___s = 0;  for (int t___ii=0;t___ii<64; ++t___ii) t___s+=t___smooth[t___ii]; CDText::T(ss, CStr(t___s/64))
...
struct CDText { static void T(const char*, const char*) {}; };

char* generate_trace_text();
