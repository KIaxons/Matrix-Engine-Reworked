...
#define WARNING_S(s1) { SFT_fun("\rPlanetary battle Warning! " + CStr(s1)); }
#define WARNING_S2(s1, s2) { SFT_fun("\rPlanetary battle Warning! " + CStr(s1) + CStr(s2)); }
#define WARNING_S3(s1, s2, s3) { SFT_fun("\rPlanetary battle Warning! " + CStr(s1) + CStr(s2) + CStr(s3)); }
#define WARNING_S4(s1, s2, s3, s4) { SFT_fun("\rPlanetary battle Warning! " + CStr(s1) + CStr(s2) + CStr(s3) + CStr(s4)); }
#define WARNING_S5(s1, s2, s3, s4, s5) { SFT_fun("\rPlanetary battle Warning! " + CStr(s1) + CStr(s2) + CStr(s3) + CStr(s4) + CStr(s5)); }

#ifdef ASSERT_OFF
	#define ASSERT(zn)
#else
#ifdef _TRACE
#define ASSERT(zn) { if(!(zn)) ERROR_S(L"ASSERT(zn) Error!"); }
#else
