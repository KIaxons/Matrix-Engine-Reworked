...
class BASE_API CStr : public CMain
{
	...
	void Init(const char* src, int len, CHeap* heap)
	{
		...
		if(len == 0) *m_Str = 0;
		else memcpy(m_Str, src, uint(len + 1));
	};
	void Init(const wchar* src, int len, CHeap* heap);

public:
	CStr() = default;
	explicit CStr(CHeap* heap) : m_Heap(heap) {}
	CStr(const CStr& s, CHeap* heap = nullptr) { Init(s.m_Str, s.Len(), heap ? heap : s.m_Heap); }
	explicit CStr(const CWStr& s, CHeap* heap = nullptr);
	explicit CStr(const wchar* s, CHeap* heap = nullptr);
	explicit CStr(const char* s, CHeap* heap = nullptr) { Init(s, int(strlen(s)), heap); }
	CStr(const char* s, int len, CHeap* heap = nullptr) { Init(s, len, heap); }
	explicit CStr(char sim, CHeap* heap = nullptr) : m_Heap(heap), m_MaxLen(16), m_Len(1) { m_Str = (char*)HAlloc(uint(m_MaxLen + 1), m_Heap); m_Str[0] = sim; m_Str[1] = 0; };
	CStr(char sim, int count, CHeap* heap = nullptr) : m_Heap(heap), m_MaxLen(count + 16), m_Len(count) { m_Str = (char*)HAlloc(uint(m_MaxLen + 1), m_Heap); memset(m_Str, int(sim), uint(count)); m_Str[count] = 0; };
	explicit CStr(int zn, CHeap* heap = nullptr);
	explicit CStr(dword zn, CHeap* heap = nullptr);
	...
	void ClearFull() { if(m_Str != nullptr) { HFree(m_Str, m_Heap); m_Str = nullptr; } m_Len = m_MaxLen = 0; }

	void SetLen(int len); // Установить длину строки (Выделение памяти если нужно)
    void SetZeroLenNoTream() { m_Len = 0; m_Str[0] = 0; };
	...
	const char* GetEx() const		 { if(IsEmpty()) return nullptr; return m_Str; }
    char* GetBuf()					 { return m_Str; }
	char* GetBufEx() const			 { if(IsEmpty()) return nullptr; return m_Str; }
	...
	bool IsEmpty() const			 { return m_Len < 1; }

	CStr& Trim();						// Удаляет в начале и в конце символы 0x20,0x9,0x0d,0x0a
	CStr& TrimFull();					// Trim() и в середине строки удоляет повторяющиеся 0x20,0x9
	...
	CStr& operator = (const CStr& s)  { if(this != &s) { Set(s); } return *this; }
	CStr& operator = (const CWStr& s) { Set(s); return *this; }
	CStr& operator = (const char* s)  { Set(s); return *this; }
	CStr& operator = (char zn)		  { Set(zn); return *this; }
	CStr& operator = (int zn)		  { Set(zn); return *this; }
	CStr& operator = (double zn)	  { Set(zn); return *this; }
	//CStr& operator = (void* zn)	  { Set(zn); return *this; }

	char  operator [] (int n) const { /*lint -e613*/ return *(m_Str + n); /*lint +e613*/ }
	char& operator [] (int n) { /*lint -e613*/ return *(m_Str + n); /*lint +e613*/ }
	...
	friend int operator < (const CStr& zn1, const CStr& zn2) { if(CStr::Compare(zn1, zn2) < 0)		    { return 1; } return 0; }
	friend int operator < (const CStr& zn1, const char* zn2) { if(CStr::Compare(zn1, CStr(zn2)) < 0)   { return 1; } return 0; }
	friend int operator < (const char* zn1, const CStr& zn2) { if(CStr::Compare(CStr(zn1), zn2) < 0)   { return 1; } return 0; }
	...
	CStr& operator +=(const CStr& str) { Add(str); return *this; }
	CStr& operator +=(const char* str) { Add(str); return *this; }
	CStr& operator +=(char sim) { Add(sim); return *this; }
	CStr& operator +=(int zn) { Add(zn); return *this; }
	CStr& operator +=(double zn) { Add(zn); return *this; }
	CStr& operator +=(void* zn) { Add(zn); return *this; }

	friend CStr operator + (const CStr& s1, const CStr& s2) { CStr str(s1); str += s2; return str; }
	friend CStr operator + (const CStr& s1, const char* s2) { CStr str(s1); str += s2; return str; }
	friend CStr operator + (const char* s1, const CStr& s2) { CStr str(s1); str += s2; return str; }
	friend CStr operator + (const CStr& s1, char sim)		{ CStr str(s1); str += sim; return str; }
	friend CStr operator + (char sim, const CStr& s2)		{ CStr str(sim); str += s2; return str; }
	friend CStr operator + (const CStr& s, int zn)			{ CStr str(s); str += zn; return str; }
	friend CStr operator + (int zn, const CStr& s)			{ CStr str(zn); str += s; return str; }
	friend CStr operator + (const CStr& s, double zn)		{ CStr str(s); str += zn; return str; }
	friend CStr operator + (double zn, const CStr& s)		{ CStr str(zn); str += s; return str; }
    ...
    //lint -e1763
    CHeap* GetHeap() const { return m_Heap; }
    //lint +e1763
};
