...
void CStorageRecord::Save(CBuf& buf, bool compression)
{
    ...
    for(int i = 0; i < m_ItemsCount; ++i) m_Items[i].Save(buf, compression);
}

dword CStorageRecord::CalcUniqID(dword xi)
{
    dword x = CalcCRC32_Buf(xi, m_Name.Get(), m_Name.GetLen() * sizeof(wchar));
    for(int i = 0; i < m_ItemsCount; ++i) x = m_Items[i].CalcUniqID(x);
    return x;
}
...
CStorage::CStorage(CHeap *heap):m_Heap(heap), m_Records(nullptr), m_RecordsCnt(0)
{
...
void CStorage::StoreBlockPar(const wchar *root, const CBlockPar &bp)
{
    ...
    CDataBuf *prop_key = GetBuf(root, L"0", ST_WCHAR);
    CDataBuf *prop_val = GetBuf(root, L"1", ST_WCHAR);

    int cnt = bp.ParCount();
    for (int i = 0;i<cnt;++i)
    {
        prop_key->AddWStr(bp.ParGetName(i));
        prop_val->AddWStr(bp.ParGet(i));
    }

    int uniq = 0;

    prop_key = GetBuf(root, L"2", ST_WCHAR);
    prop_val = GetBuf(root, L"3", ST_WCHAR);
    cnt = bp.BlockCount();

    CWStr uniq_s(m_Heap);
    for (int i = 0;i<cnt;++i)
    {
        prop_key->AddWStr(bp.BlockGetName(i));
...
void CStorage::RestoreBlockPar(const wchar *root, CBlockPar &bp)
{
    ...
    for (dword i=0;i<prop_key->GetArraysCount();++i)
    {
        bp.ParAdd(prop_key->GetAsWStr(i), prop_val->GetAsWStr(i));
    }
    ...
    for (dword i=0;i<prop_key->GetArraysCount();++i)
    {
        CBlockPar *bp1 = bp.BlockAdd(prop_key->GetAsWStr(i));
        RestoreBlockPar(prop_val->GetAsWStr(i).Get(), *bp1);
    }
