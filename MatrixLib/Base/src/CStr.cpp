...
CStr::CStr(dword zn, CHeap* heap) : CMain()
{
	m_Heap = heap;
...
void CStr::Set(const CWStr& cstr)
{
	...
	if(m_Len > 0)
	{
		if(!WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK | WC_DEFAULTCHAR,
			...
			cstr.GetLen(),
			m_Str,
			m_MaxLen, " ", nullptr)) ERROR_S(L"CStr::Set Error 1!");
	}
	m_Str[m_Len] = 0;
}
...
void CStr::Set(const wchar* wstr)
{
	...
	if(len > 0)
	{
		if(!WideCharToMultiByte(CP_ACP, WC_COMPOSITECHECK | WC_DEFAULTCHAR,
			...
			len,
			m_Str,
			m_MaxLen, " ", nullptr)) ERROR_S(L"CStr::Set Error 2!");
	}
	m_Str[m_Len] = 0;
}
...
void CStr::Set(int zn)
{
	...
	for(int8_t i = 0; i < int8_t(m_Len - 32); ++i) m_Str[i] = m_Str[m_Len - 1 - i];
	m_Len -= 32;
	m_Str[m_Len] = 0;
}

void CStr::Set(dword zn)
{
	Tream(32u);
	m_Len = 32;
	...
	while(zn > 0)
	{
		m_Str[m_Len] = char(zn - dword(zn / 10) * 10) + char('0');
		++m_Len;
		zn = zn / 10;
	}
	if(m_Len == 32)
...
{
	for(int i=0;i<m_Len;i++) if(m_Str[i]==0x9) m_Str[i]=' ';
}

void CStr::Del(int sme,int len)
{
	if(sme<0 || sme+len>m_Len) return;
	for(int i=sme;i<=(m_Len-len);i++) m_Str[i]=m_Str[i+len];
	m_Len-=len;
}
...
{
	if(substr.GetLen()<1 || GetLen()<1) return;

	int sme=0;
	for(;;) {
		int ff;
		if((ff=Find(substr,sme))==-1) break;
		sme+=ff;
...
{
    int i,u;
	if( (m_Len-sme<sublen) || (sublen<1) ) return -1;

	for(i=0;i<m_Len-sme;i++) {
		for(u=0;u<sublen;u++) {
			if(m_Str[sme+i+u]!=substr[u]) break;
		}
		if(u>=sublen) return i;
	}
	return -1;
}
...
{
    int i,u;
	if((m_Len<sublen) || (sublen<1)) return -1;

	for(i=m_Len-sublen;i>=0;--i)
    {
		for(u=0;u<sublen;u++)
        {
			if(m_Str[i+u]!=substr[u]) break;
		}
		if(u>=sublen) return i;
	}
	return -1;
...
int CStr::GetSmePar(int np, const char* ogsim) const
{
	...
	if(np > 0)
	{
		...
		if(i == tlen) ERROR_S(L"CStr::GetSmePar Error!");
	}
...
int CStr::GetLenPar(int smepar, const char* ogsim) const
{
	...
	int tlen = GetLen();
	int lenogsim = strlen(ogsim);
	if(tlen < 1 || lenogsim < 1 || smepar > tlen) ERROR_S(L"CStr::GetLenPar Error!");
...
{
	if(len < 0) len = Len() - sme;
	if((sme < 0) || (len <= 0) || ((sme + len) > Len())) return;
...
{
	if(str.m_Str==nullptr || m_Str==nullptr || str.m_Len>m_Len) return 0;
	
	for(int i=0;i<str.m_Len;i++) if(str.m_Str[i]!=m_Str[i]) return 0;
...
{
    int i;
    if(zn1.m_Len!=zn2.m_Len) return 0;
	for(i=0;i<zn1.m_Len;i++) if(zn1.m_Str[i]!=zn2.m_Str[i]) return 0;
    return 1;
}
...
{
    int i;
    if(zn1.m_Len!=strlen(zn2)) return 0;
	for(i=0;i<zn1.m_Len;i++) if(zn1.m_Str[i]!=zn2[i]) return 0;
    return 1;
}
