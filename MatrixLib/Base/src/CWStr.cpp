...
void CWStr::Set(const CStr& cstr)
{
	...
	if(!MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, cstr.Get(), m_Data->m_Len, m_Data->Data(), m_Data->m_Len))
    {
		...
		txt = "unknown error: ";
		if(err == ERROR_INSUFFICIENT_BUFFER) txt = "ERROR_INSUFFICIENT_BUFFER";
		else if(err == ERROR_INVALID_FLAGS) txt = "ERROR_INVALID_FLAGS";
		else if(err == ERROR_INVALID_PARAMETER) txt = "ERROR_INVALID_PARAMETER";
		else if(err == ERROR_NO_UNICODE_TRANSLATION) txt = "ERROR_NO_UNICODE_TRANSLATION";
		else txt.Add(int(err));
		...
		if(cstr.Get() == nullptr) fwrite("<Input is nullptr>", strlen("<Input is nullptr>"), 1, file);
		else fwrite(cstr.Get(), cstr.Len(), 1, file);

		fclose(file);

		ERROR_S(L"CWStr::Set Error!");
	}
...
void CWStr::Set(double zn, int zpz)
{
	...
	if(sign == 0) ModifyLenNoCopy(m_Data->m_Heap, 0);
	else Set((wchar)'-');
	...
	if(dec < 0)
	{
		...
		if(le > 0)
		{
			...
			if(le > count)
			{
				for(int yu = 0; yu < le - count; ++yu) Add(wchar(st[yu]));
			}
		}
	}
	else
	{
		if(dec > 0)
		{
			for(int yu = 0; yu < dec; ++yu) Add(wchar(st[yu]));
		}
		else Add(L'0');
		...
		{
			Add(L'.');
			for(int yu = 0; yu < le - count - dec; ++yu) Add(wchar(st[dec + yu]));
		}
	}
}

const wchar Str_CH[] = L"0123456789ABCDEF";
...
double CWStr::GetDouble() const
{
	...
	{
		ch = tstr[i];
		if(ch >= L'0' && ch <= L'9') zn = zn * 10.0 + (double)(ch - L'0');
		else if (ch == L'.') break;
	}
...
dword CWStr::GetHexUnsigned() const
{
	int tlen = GetLen();
	if(tlen < 1) return 0;
	const wchar* tstr = Get();
...
{
	int tlen = GetLen();
	if(tlen < 1) return *this;
	const wchar* tstr = Get();

	int i;
	for(i = 0; i < tlen; ++i)
	{
		if(tstr[i] != ' ' && tstr[i] != 0x9 && tstr[i] != 0x0d && tstr[i] != 0x0a) break;
	}
	if(i == tlen)
	{
	...
	for(u = tlen - 1; u >= 0; --u)
	{
		if(tstr[u] != ' ' && tstr[u] != 0x9 && tstr[u] != 0x0d && tstr[u] != 0x0a) break;
	}
	tlen = u - i + 1;
...
CWStr& CWStr::TrimFull()
{
	...
	for(int i = 2; i < tlen - 1; ++i)
	{
		...
		{
			for(int u = i; u < tlen; ++u) tstr[u] = tstr[u + 1];
			--tlen;
			--i;
		}
	}
...
void CWStr::TabToSpace()
{
	...
	for(int i = 0; i < tlen; ++i) if(tstr[i] == 0x9) tstr[i] = ' ';
}
...
CWStr& CWStr::Del(int sme, int len)
{
	...
	else
	{
		...
		m_Data->m_Len -= len;
		m_Data->Data()[m_Data->m_Len] = 0;
		if(m_Data->m_Len < 1) ModifyLenNoCopy(GetHeap(), 0);
	}
...
CWStr& CWStr::Insert(int sme, const wchar* str, int len)
{
	...
	wchar* tstr = GetBuf();
	if(sme > oldlen) sme = oldlen;

	for(int i = oldlen; i >= sme; --i) tstr[i + len] = tstr[i];

	for(int i = 0; i < len; ++i) tstr[sme + i] = str[i];
...
int CWStr::Find(const wchar* substr, int slen, int sme) const
{
	int tlen = GetLen();
	if(tlen < 1) return -1;
	if(slen < 1 || (tlen - sme) < slen) return -1;
	const wchar* tstr = Get();
...
{
	if(sme < 0) sme = GetLen() - 1;
	if((sme < 0) || (sme >= GetLen())) return -1;
...
{
	if(len < 0) len = GetLen() - sme;
	if((sme < 0) || (len <= 0) || ((sme + len) > GetLen())) return;

	ModifyLen(GetHeap(), GetLen());

	if(GetVersion() < 0x80000000) CharLowerBuffW(GetBuf() + sme, len);
	else
	{
...
void CWStr::UpperCase(int sme, int len)
{
	...
	if(GetVersion() < 0x80000000) CharUpperBuffW(GetBuf() + sme, len);
	else
	{
		CStr tstr(*this, GetHeap());
...
int CWStr::GetSmePar(int np, const wchar* og_sim) const
{
	int len_og_sim = WStrLen(og_sim);
	int t_len = GetLen();
	//if(t_len < 1 || len_og_sim < 1 || np < 0) ERROR_OK("Data in CWStr::GetSmePar()");
	//if((t_len < 1 || len_og_sim < 1 || np < 0))
	//ASSERT(1);
	ASSERT(!(t_len < 1 || len_og_sim < 1 || np < 0));
	int sme_par = 0;
	int tek_par = 0;
	...
	if(np > 0)
	{
		...
		{
			int u;
			for(u = 0; u < len_og_sim; ++u) if(tstr[i] == og_sim[u]) break;
			if(u < len_og_sim)
			{
				++tek_par;
				sme_par = i + 1;
		...
		if(i == t_len) ERROR_S(L"CWStr::GetSmePar Error!");
	}
...
bool CWStr::GetTrueFalsePar(int np, const wchar* og_sim) const
{
	...
	if(tstr == L"yes" || tstr == L"Yes" || tstr == L"YES") return 1;
	if(tstr == L"no" || tstr == L"No" || tstr == L"NO") return 0;

	if(tstr == L"on" || tstr == L"On" || tstr == L"ON") return 1;
	if(tstr == L"off" || tstr == L"Off" || tstr == L"OFF") return 0;

	ERROR_S(L"CWStr::GetTrueFalsePar Error!");
	return 0;
}

int CWStr::Compare(const wchar* zn1, int zn1len, const wchar* zn2, int zn2len)
{
	if(!zn1len && !zn2len) return 0;
	else if(!zn1len) return -1;
	else if(!zn2len) return 1;
