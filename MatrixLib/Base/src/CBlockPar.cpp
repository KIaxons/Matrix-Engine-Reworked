...
CBlockParUnit* CBlockPar::UnitGet(const wchar* path, int path_len)
{
	...
	while(true)
	{
		...
		if(ne == nullptr) ERROR_S2(L"Path not found: ", path);
		if(name_next >= path_len) break;
...
void CBlockPar::ParReplace(int index, const wchar* name, int namelen, const wchar* zn, int znlen)
{
	...
	ERROR_S(L"CBlockPar::ParReplace Error!");
}
...
void CBlockPar::ParDelete(int no) // нужно оптимизировать
{
	...
	ERROR_S(L"CBlockPar::ParDelete Error!");
}
...
const CWStr& CBlockPar::ParGet(int no) const
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::ParGet Error!");
	}
}
...
void CBlockPar::ParSet(int no, const wchar* zn, int znlen)
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::ParSet Error!");
	}
}
...
const CWStr& CBlockPar::ParGetName(int no) const
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::ParGetName Error!");
	}
}
...
void CBlockPar::BlockDelete(int no)
{
	...
	ERROR_S(L"CBlockPar::BlockDelete Error!");
}
...
CBlockPar* CBlockPar::BlockGet(int no)
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::BlockGet Error!");
	}
}
...
const CWStr& CBlockPar::BlockGetName(int no) const
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::BlockGetName Error!");
	}
}
...
const CWStr& CBlockPar::ParPathGet(const wchar* path, int pathlen)
{
	...
	if(el->m_Type != 1) ERROR_S(L"CBlockPar::ParPathGet Error!");
	return *el->m_Par;
}
...
void CBlockPar::ParPathSet(const wchar* path, int pathlen, const wchar* zn, int znlen)
{
	...
	if(te->m_Type != 1) ERROR_S(L"CBlockPar::ParPathSet Error!");
...
void CBlockPar::ParPathSetAdd(const wchar* path, int pathlen, const wchar* zn, int znlen)
{
	...
	{
		CBlockParUnit* te = UnitGet(path, pathlen);
		if(te->m_Type != 1) ERROR_S(L"CBlockPar::ParPathSetAdd Error!");
		te->m_Par->Set(zn, znlen);
	}
	catch(CException* ex)
	{
...
{
	CBlockParUnit* te = UnitGet(path, pathlen);
	if(te->m_Type != 1) ERROR_S(L"CBlockPar::ParPathDelete Error!");
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
CBlockPar* CBlockPar::BlockPathGet(const wchar* path, int path_len, bool skip_exception)
{
	CBlockParUnit* el = UnitGet(path, path_len);
	if(el->m_Type != 2)
	{
		if(!skip_exception)
		{
			ERROR_S(L"CBlockPar::BlockPathGet Error!");
		}
		else return nullptr;
	}
...
int CBlockPar::AllGetType(int no)
{
	...
	else
	{
		...
		ERROR_S(L"CBlockPar::AllGetType Error!");
	}
}
...
CBlockPar* CBlockPar::AllGetBlock(int no)
{
	...
	{
		el = m_Array[no];
		if(el->m_Type != 2) ERROR_S(L"CBlockPar::AllGetBlock Error 1!");
		return el->m_Block;
	}
	else
	{
		...
		while(el != nullptr)
		{
			if(!no)
			{
				if(el->m_Type != 2) ERROR_S(L"CBlockPar::AllGetType Error 2!");
				return el->m_Block;
			}
			--no;
			el = el->m_Next;
		...
		ERROR_S(L"CBlockPar::AllGetType Error 3!");
	}
}
...
const CWStr& CBlockPar::AllGetPar(int no)
{
	...
	{
		el = m_Array[no];
		if(el->m_Type != 1) ERROR_S(L"CBlockPar::AllGetPar Error 1!");
		return *el->m_Par;
	}
	else
	{
		...
		while(el != nullptr)
		{
			if(!no)
			{
				if(el->m_Type != 1) ERROR_S(L"CBlockPar::AllGetPar Error 2!");
				return *el->m_Par;
			}
		...
		ERROR_S(L"CBlockPar::AllGetPar Error 3!");
	}
}
...
const CWStr& CBlockPar::AllGetName(int no)
{
	...
	{
		el = m_Array[no];
		if(el->m_Type != 1 && el->m_Type != 2) ERROR_S(L"CBlockPar::AllGetName Error 1!");
		return el->m_Name;
	}
	else
	{
		...
		while(el != nullptr)
		{
			if(!no)
			{
				if(el->m_Type != 1 && el->m_Type != 2) ERROR_S(L"CBlockPar::AllGetName Error 2!");
				return el->m_Name;
			}
		...
		ERROR_S(L"CBlockPar::AllGetName Error 3!");
	}
}
...
{
public:
	CBlockPar* m_BP = nullptr;
	const wchar* m_Text = nullptr;
	int m_TextLen = 0;

	int m_Line = 0;
	int m_LineEnd = 0;
	int m_Com = 0, m_ComSpace = 0;
	int m_Block = 0;
	int m_Par = 0;

	int m_WordBegin = 0;
	int m_WordEnd = 0;
	int m_WordSearchEnd = 0;

	int m_FileBegin = 0, m_FileEnd = 0;

	BPCompiler(CBlockPar* bp, const wchar* text, int text_len) : m_BP(bp), m_Text(text), m_TextLen(text_len) {}
	~BPCompiler() = default;
