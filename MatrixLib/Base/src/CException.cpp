...
{ 
	...
	len = WStrLen(str2);
	if((sme + len) >= maxlen) len = maxlen - sme;
	if(len > 0) memmove(m_Str + sme, str2, len * 2);
	sme += len;

	len = WStrLen(str3);
	if((sme + len) >= maxlen) len = maxlen - sme;
	if(len > 0) memmove(m_Str + sme, str3, len * 2);
	sme += len;

	len = WStrLen(str4);
	if((sme + len) >= maxlen) len = maxlen - sme;
	if(len > 0) memmove(m_Str + sme, str4, len * 2);
	sme += len;

	SFT(m_Str); //Выводим собранную строку ошибки в игровой лог
}
