...
#include "stdafx.h"
#include "CBitmap.hpp"
#include "../../FilePNG/include/FilePNG.hpp"
...
void CBitmap::Make2xSmaller()
{
    ...
    {
        ERROR_S(L"CBitmap::Make2xSmaller Error 1!");
    }
    else if(m_BytePP == 3)
    {
        int y = m_Size.y;
...
{
	if(m_BytePP == 2) ERROR_S(L"CBitmap::Make2xSmaller Error 2!");
    ...
    else
    {
        ...
        for(int y = 0; y < size.y; ++y, des += desnl, sou += sounl)
        {
            for(int x = 0; x < size.x; ++x, des += desnp, sou += sounp)
            {
                ...
                if(alpha == 0) continue;
                else if(alpha == 255) ocolor = color;
                else
                {
                    float A = float(alpha) / 255.0f;
                    float nA = 1.0f - A;
...
void CBitmap::MergeGrayscaleWithAlpha(const Base::CPoint& pdes, const Base::CPoint& size, const CBitmap* data_src, const Base::CPoint& data_src_point, const CBitmap* alpha_src, const Base::CPoint& alpha_src_point)
{
	...
	if(data_src == nullptr) no_data = true;
    else
    {
        if(data_src->m_Format != BMF_FLAT || data_src->m_BytePP != 1 || data_src->m_BitPP != 8) ERROR_S(L"CBitmap::MergeGrayscaleWithAlpha Error 1!");
        dsou = (byte*)data_src->m_Data + data_src_point.x + data_src->m_Pitch * data_src_point.y;
        dsounl = data_src->m_Pitch - size.x;
    }
    if(alpha_src == nullptr) ERROR_S(L"CBitmap::MergeGrayscaleWithAlpha Error 2!");

    if(m_Format != BMF_FLAT || m_BytePP != 1 || m_BitPP != 8) ERROR_S(L"CBitmap::MergeGrayscaleWithAlpha Error 3!");
    ...
    if(no_data)
    {
        for(int y = 0; y < size.y; ++y, des += desnl, asou += asounl)
        {
            for(int x = 0; x < size.x; ++x, ++des, asou += 4)
            {
                ...
                if(alpha == 0) continue;
                else if(alpha == 255) odata = 0;
                else odata = byte((255.0f - alpha) * (*des) * (1.0f / 255.0f));
    ...
    for(int y = 0; y < size.y; ++y, des += desnl, asou += asounl, dsou += dsounl)
    {
        for(int x = 0; x < size.x; ++x, ++des, ++dsou, asou += 4)
        {
            ...
            if(alpha == 0) continue;
            else if(alpha == 255) odata = *dsou;
            else odata = byte(((255.0f - alpha) * (*des) + alpha * (*dsou)) * (1.0f / 255.0f));
...
void CBitmap::MergeGrayscaleWithAlphaPM(const Base::CPoint& pdes, const Base::CPoint& size, const CBitmap* data_src, const Base::CPoint& data_src_point, const CBitmap* alpha_src, const Base::CPoint& alpha_src_point)
{
    ...
    {
        if (data_src->m_Format!=BMF_FLAT || data_src->m_BytePP != 1 || data_src->m_BitPP != 8) ERROR_S(L"CBitmap::MergeGrayscaleWithAlphaPM Error 1!");
	    dsou = (byte *)data_src->m_Data + data_src_point.x + data_src->m_Pitch*data_src_point.y;
	    dsounl=data_src->m_Pitch-size.x;
    }

    if(alpha_src == nullptr) ERROR_S(L"CBitmap::MergeGrayscaleWithAlphaPM Error 2!");

    if (m_Format!=BMF_FLAT || m_BytePP != 1 || m_BitPP != 8) ERROR_S(L"CBitmap::MergeGrayscaleWithAlphaPM Error 3!");
    if (alpha_src->m_Format!=BMF_FLAT || alpha_src->m_BytePP != 4 || alpha_src->m_BitPP != 32) return;
...
{
    if(alpha_src == nullptr) ERROR_S(L"CBitmap::ModulateGrayscaleWithAlpha Error 1!");

    if(m_Format != BMF_FLAT || m_BytePP != 1 || m_BitPP != 8) ERROR_S(L"CBitmap::ModulateGrayscaleWithAlpha Error 2!");
    if(alpha_src->m_Format != BMF_FLAT || alpha_src->m_BytePP != 4 || alpha_src->m_BitPP != 32) return;
    ...
    for(int y = 0; y < size.y; ++y, des += desnl, asou += asounl)
    {
        for(int x = 0; x < size.x; ++x, ++des, asou += 4)
        {
            ...
            else *des = byte((alpha * (*des)) * (1.0f / 255.0f));
        }
    }
}
...
void CBitmap::WBM_Init()
{
    ...
    if(m_WindowBitmap == 0) ERROR_S(L"CBitmap::WBM_Init Error 1!");
    ...
    if(m_WindowDC == 0) ERROR_S(L"CBitmap::WBM_Init Error 2!");

    if(SelectObject(m_WindowDC, m_WindowBitmap) == 0) ERROR_S(L"CBitmap::WBM_Init Error 3!");
...
void CBitmap::WBM_Save(bool save16as32)
{
	...
	if(GetDIBits(m_WindowDC,m_WindowBitmap,0,0,nullptr,(LPBITMAPINFO)&b.bmi,DIB_RGB_COLORS)==0) return;
...
bool CBitmap::LoadFromPNG(void* buf, int buflen)
{
    ...
    else ERROR_S(L"CBitmap::LoadFromPNG Error!");
...
bool CBitmap::LoadFromPNG(const wchar* filename, bool crash_on_fail_to_load)
{
    CFile file(filename, m_Heap);
    if(!file.OpenRead(FILE_SHARE_READ, crash_on_fail_to_load)) return false;

    int size = file.Size();
    if(size < 0) return false;

    void* buf;
    if(size > 32768) buf = HAlloc(size, m_Heap);
    else
    {
        try
        {
        ...
        {
            ERROR_S(L"Not enough memory for buf after _alloca() call in CBitmap::LoadFromPNG function!");
        }
    }
